(function(){
  ...
  async function decodeSharedReview(value){
    value=String(value||'');if(value.length>900000)throw new Error('This review link is too large to open safely.');var dot=value.indexOf('.'),mode=dot>0?value.slice(0,dot):'j',bytes=reviewShareBytes(dot>0?value.slice(dot+1):value),text='';
    if(mode==='g'){if(typeof DecompressionStream==='undefined')throw new Error('This browser cannot unpack the shared review. Open it in a current Safari, Chrome, or Edge.');text=await new Response(new Blob([bytes]).stream().pipeThrough(new DecompressionStream('gzip'))).text();}else text=new TextDecoder().decode(bytes);
    if(text.length>2000000)throw new Error('This shared review is larger than Phloem can safely import.');var payload=JSON.parse(text);if(!payload||payload.kind!=='phloem-review'||payload.version!==1||!payload.paper||!/^[a-f0-9]{64}$/i.test(payload.paper.contentHash||'')||!Array.isArray(payload.comments)||!payload.comments.length||payload.comments.length>160)throw new Error('This is not a valid Phloem review link.');return payload;
  }
  ...
  async function copyReviewShareLink(ch){
    ...
    try{
      ...
      if(!ch.contentHash){var stored=await getPdf(ch.id);if(stored)await rememberPdfFingerprint(ch,stored);}if(!ch.contentHash)throw new Error('The original PDF is not available here, so Phloem cannot verify its fingerprint. Re-add the PDF first.');
      if(!ch.reviewShareId){ch.reviewShareId='review-layer-'+uid('').replace(/[^a-z0-9-]/gi,'');persist(false);}var encoded=await encodeSharedReview(reviewSharePayload(ch)),link=location.href.split('#')[0]+'#phloem-review='+encoded;await copyPlainText(link);status.textContent='Snapshot link copied. After later changes, copy a new link; it will replace this layer for recipients.';button.textContent='Copied';setTimeout(function(){if(button.textContent==='Copied')button.textContent=old;},1800);
  ...
  async function importSharedReview(){
    ...
    history.replaceState(history.state&&history.state.phloem?{phloem:history.state.phloem}:null,'',location.pathname+location.search);
    try{
      pendingSharedReview=await decodeSharedReview(match[1]);var dialog=byId('reviewShareDialog');refreshIncomingReviewShare();byId('reviewShareStatus').textContent='';if(!dialog.open)dialog.showModal();
      if(!reviewShareTarget(pendingSharedReview)&&state.chapters.some(function(ch){return ch.kind==='pdf'&&!ch.contentHash;})){byId('reviewShareStatus').textContent='Checking the PDFs already on this device…';await backfillPdfFingerprints();refreshIncomingReviewShare();byId('reviewShareStatus').textContent=reviewShareTarget(pendingSharedReview)?'Exact PDF verified.':'Choose the exact PDF to attach this review layer.';}
    }catch(e){pendingSharedReview=null;showError(e.message||'Phloem could not read this shared review.','Could not open shared review');}
  }
