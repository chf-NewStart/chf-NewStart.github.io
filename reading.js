(function(){
  ...
  function repairPdfReviewQuotes(ch,pageOnly){
    ...
    ch.reviewComments.forEach(function(comment){if(!reviewCanAutoLocate(comment))return;var anchors=Array.isArray(comment.pdfAnchors)?comment.pdfAnchors:[];anchors.forEach(function(anchor){var page=+anchor.page||0;if(anchor.quote||!page||pageOnly&&page!==+pageOnly)return;var quote=reviewLocalPdfQuote(ch.pageTexts[page-1],comment);if(!quote)return;anchor.quote=quote;anchor.method='local-pdf-quote';if(comment.page===page&&!comment.quote){comment.quote=quote;comment.anchorMethod=anchor.method;}changed=true;});});return changed;
  }
  ...
  function reviewLocationConfig(routeOverride){var route=routeOverride||activeAiRoute(false),local=route&&route.id==='browser';return{route:route,size:local?1:3,concurrency:local?1:2,label:route&&route.label||'AI'};}
  async function locateReviewBatchWithAi(ch,comments,onProgress,routeOverride){
    if(!comments.length)return 0;var prepared,system,user,result=null,data=null,items=[],byComment={},found=0,requestFailed=false;
    if(ch.kind==='pdf'){
      ...
      try{result=await runAi(system,user,Math.max(1050,comments.length*700),onProgress,routeOverride);data=reviewAiJson(result.text);items=Array.isArray(data&&data.items)?data.items:(comments.length===1&&Array.isArray(data&&data.matches)?[{commentId:prepared[0].id,matches:data.matches}]:[]);}catch(e){requestFailed=true;}
      items.forEach(function(item){if(item&&item.commentId)byComment[String(item.commentId)]=item;});prepared.forEach(function(item){var answer=byComment[item.id],matches=answer&&Array.isArray(answer.matches)?answer.matches:[];if(applyPdfReviewMatches(ch,item,matches,result&&result.provider))found++;});
      if(!requestFailed&&comments.length>1)for(var p=0;p<prepared.length;p++)if(!byComment[prepared[p].id]&&!prepared[p].comment.anchored)found+=await locateReviewBatchWithAi(ch,[prepared[p].comment],onProgress,routeOverride);return found;
    }
    ...
    try{result=await runAi(system,user,Math.max(650,comments.length*430),onProgress,routeOverride);data=reviewAiJson(result.text);items=Array.isArray(data&&data.items)?data.items:(comments.length===1&&data?[Object.assign({commentId:prepared[0].id},data)]:[]);}catch(e){requestFailed=true;}
    items.forEach(function(item){if(item&&item.commentId)byComment[String(item.commentId)]=item;});prepared.forEach(function(item){if(applyTextReviewMatch(ch,item,byComment[item.id],result&&result.provider))found++;});
    if(!requestFailed&&comments.length>1)for(var t=0;t<prepared.length;t++)if(!byComment[prepared[t].id]&&!prepared[t].comment.anchored)found+=await locateReviewBatchWithAi(ch,[prepared[t].comment],onProgress,routeOverride);return found;
  }
