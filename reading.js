(function(){
  ...
  function renderPdfReviewMarkers(ch,pageNum,view){
    view.reviewHits=[];var comments=(ch.reviewComments||[]).filter(function(comment){return !comment.resolved&&comment.anchored&&reviewPdfAnchorOnPage(ch,comment,pageNum);}),entries=[],groups=[];
  ...
  function reviewerPassageMarkup(ch,comment){
    ...
    if(!comment.anchored){var empty=comment.level==='general'?'Manuscript-wide · no single passage':comment.level==='section'&&!reviewHasLocationEvidence(comment)?'Section-wide · no single passage named':'No passage linked yet';return'<span class="reviewer-unlinked">'+empty+'</span>'+add;}
