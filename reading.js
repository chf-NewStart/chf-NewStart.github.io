(function(){
  ...
  function renderReviewerPanel(ch){
    ...
    locate.classList.toggle('hidden',!unlinked);locate.textContent='✦ Locate '+unlinked+' passage'+(unlinked===1?'':'s')+' with AI';var visible=comments.map(function(comment,index){return{comment:comment,index:index};}).filter(function(item){return reviewFilter==='all'||(reviewFilter==='needs'?reviewNeedsPassage(ch,item.comment):item.comment.level===reviewFilter);});
