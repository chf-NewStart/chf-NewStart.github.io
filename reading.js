(function(){
  ...
  function reviewLocationSummary(ch,comments){
    var matchable=(comments||[]).filter(reviewCanAutoLocate),confident=matchable.filter(function(comment){return reviewHasVerifiedPassage(ch,comment)&&comment.locationStatus!=='needs-checking'&&+comment.matchConfidence>=.72;}).length,exact=matchable.filter(function(comment){return comment.locatedProvider==='Exact manuscript text'&&reviewHasVerifiedPassage(ch,comment);}).length;
  ...
  async function importReviewerFile(ch,file,button,replaceCurrent){
    ...
    try{
      ...
      var comments=extracted.map(function(item){var record={id:uid('rr'),sourceId:reportId,author:item.author,date:'',text:item.text,level:normalizeReviewLevel(item.level),topic:normalizeReviewTopic(item.topic),locationHint:item.locationHint||'',classificationAudit:item.classificationAudit||null,classifiedProvider:item.classifiedProvider||plan.classification.label,para:null,page:null,start:0,end:0,quote:'',anchors:[],pdfAnchors:[],anchored:false,replies:[],response:item.response||'',resolved:false,sourceName:file.name,addedAt:now()},prior=priorByText[reviewNormalizedText(item.text)];if(prior){record.id=prior.id;record.response=prior.response||record.response;record.resolved=!!prior.resolved;record.replies=prior.replies||[];}return record;});var matchable=comments.filter(reviewCanAutoLocate),locationStarted=now(),location=await locateReviewsBalanced(ch,matchable,function(message,progress){status.textContent=message;setTaskProgress('reviewerProgress',55+(Number.isFinite(progress)?progress:0)*45);},plan),locationMs=now()-locationStarted;
