(function(){
  ...
  function renderReviewerInbox(){
    ...
    box.innerHTML='<div class="reviewer-inbox">'+items.map(function(item){var comment=item.comment,displayable=reviewHasDisplayablePassage(item.ch,comment);return '<article class="reviewer-inbox-card'+(comment.resolved?' resolved':'')+'"><span class="review-eyebrow">'+esc(comment.author||'Reviewer')+' · '+esc(item.ch.title||'Untitled')+'</span><div class="reviewer-classification"><span class="reviewer-chip level">'+esc(reviewLevelLabel(comment.level))+'</span><span class="reviewer-chip">'+esc(reviewTopicLabel(comment.topic))+'</span></div><p>'+esc(comment.text||'')+'</p>'+(comment.quote&&displayable?'<blockquote>'+esc(comment.quote)+'</blockquote>':'')+'<button class="text-button" type="button" data-open-review-paper="'+esc(item.ch.id)+'" data-open-review-comment="'+esc(comment.id)+'">Open beside draft →</button></article>';}).join('')+'</div>';
