(function(){
  ...
  function renderReviewerPanel(ch){
    ch=ch||find(currentId);var comments=ch&&ch.reviewComments||[],list=byId('readerReviewList'),count=byId('readerReviewerCount'),locate=byId('locateReviewsBtn'),filters=byId('reviewerFilters'),importButton=byId('importReviewFileBtn'),clearButton=byId('clearReviewCommentsBtn'),status=byId('reviewerLocateStatus'),legacyReport=(ch&&ch.reviewReports||[]).some(function(report){return !report.extractorVersion||report.extractorVersion<2;});count.textContent=comments.length?'· '+comments.filter(function(comment){return !comment.resolved;}).length:'';importButton.textContent=legacyReport?'↻ Re-import reviewer Word file':'＋ Import reviewer Word file';clearButton.classList.toggle('hidden',!comments.length);if(legacyReport&&!status.textContent)status.textContent='This review used the older summary importer. Re-import the same Word file to replace the incomplete list.';
    ...
    list.innerHTML=visible.map(function(item){var comment=item.comment,index=item.index,replies=(comment.replies||[]).map(function(reply){return '<div class="reviewer-reply"><b>'+esc(reply.author||'Reply')+'</b><p>'+esc(reply.text||'')+'</p></div>';}).join(''),where=reviewerPassageMarkup(ch,comment),location=comment.locationHint?'<p class="reviewer-location">⌖ '+esc(comment.locationHint)+'</p>':'';
      return '<article class="reviewer-comment-card'+(comment.resolved?' resolved':'')+(comment.id===reviewFocusId?' focused':'')+'" data-review-card="'+esc(comment.id)+'"><div class="reviewer-card-head"><span>R'+(index+1)+' · '+esc(comment.author||'Reviewer')+'</span><span>'+esc(reviewerDate(comment.date))+'</span></div><div class="reviewer-classification"><span class="reviewer-chip level review-level-'+esc(comment.level)+'">'+esc(reviewLevelLabel(comment.level))+'</span><span class="reviewer-chip">'+esc(reviewTopicLabel(comment.topic))+'</span></div>'+location+(comment.quote&&comment.anchored?'<blockquote>'+esc(comment.quote)+'</blockquote>':'')+'<p class="reviewer-comment-text">'+esc(comment.text||'')+'</p>'+replies+where+'<label class="field-label" for="review-response-'+esc(comment.id)+'">Response draft</label><textarea class="reviewer-response" id="review-response-'+esc(comment.id)+'" data-review-response="'+esc(comment.id)+'" placeholder="Draft your response to the reviewer…">'+esc(comment.response||'')+'</textarea><button class="reviewer-resolve" type="button" data-resolve-review="'+esc(comment.id)+'">'+(comment.resolved?'Reopen comment':'Mark resolved')+'</button></article>';
    }).join('');
  ...
  byId('clearReviewCommentsBtn').onclick=function(){var ch=find(currentId);if(!ch||(ch.reviewComments||[]).length)return;if(!confirm('Clear every reviewer comment, response draft, and linked PDF passage for this paper? Your paper, reading notes, and personal highlights will stay.'))return;ch.reviewComments=[];ch.reviewReports=[];reviewFocusId='';reviewFocusPage=0;touch(ch);if(readerMode==='text')renderText(ch);renderReviewerPanel(ch);refreshPdfReviewMarkers();clearPdfReviewFocus();updateReviewBadge();byId('reviewerLocateStatus').textContent='Current review cleared. Import the new reviewer file when ready.';};
