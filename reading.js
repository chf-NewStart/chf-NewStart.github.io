(function(){
  ...
  function normalize(ch){
    ...
    if(!Array.isArray(ch.reviewReports))ch.reviewReports=[];var legacyReviewReports={};ch.reviewReports.forEach(function(report){if(!report.extractorVersion||report.extractorVersion<2)legacyReviewReports[report.id]=1;});
  ...
  function renderShelf(){
    ...
    grouped.forEach(function(group,groupIndex){
      var categoryTab=document.createElement('div'),categoryGrip=document.createElement('span'),categoryOpen=document.createElement('button'),categoryEdit=document.createElement('button'),categoryReorder=document.createElement('span'),categoryPrev=document.createElement('button'),categoryNext=document.createElement('button'),noteGrid=document.createElement('div'),isOpen=group.label.toLowerCase()===selectedCategory;
      categoryTab.className='paper-category-tab'+(isOpen?' is-selected':'');categoryTab.dataset.categoryName=group.label;categoryTab.setAttribute('aria-label',group.label+' papers');
      ...
      categoryTab.appendChild(categoryGrip);categoryTab.appendChild(categoryOpen);categoryTab.appendChild(categoryEdit);categoryTab.appendChild(categoryReorder);categoryRail.appendChild(categoryTab);
      if(!isOpen)return;
  ...
  function pdfReviewSpanMatch(view,quote){
    if(!view||!view.text||!quote)return[];var entries=[],joined='',cursor=0;
    view.text.querySelectorAll('span').forEach(function(span){if(span.querySelector&&span.querySelector('span'))return;var value=reviewNormalizedText(span.textContent);if(!value)return;if(joined){joined+=' ';cursor++;}var start=cursor;joined+=value;cursor=joined.length;entries.push({span:span,start:start,end:cursor});});
  ...
  function renderPdfReviewFocus(pageNum){
    ...
    var tries=0;(function attempt(){
      ...
      pdfReviewSpanMatch(view,anchor.quote).forEach(function(span){span.classList.add('review-focus-span');});
    })();
  }
  ...
  function showReviewerComment(ch,id,pageOverride){
    reviewFocusId=id;reviewFocusPage=+pageOverride||reviewCommentPage(ch,reviewerById(ch,id))||0;renderReviewerPanel(ch);switchTab('reviewsPanel');if(innerWidth<=720)toggleSheet(true);else setNotebookCollapsed(false,true);
  ...
  async function locateOneReviewWithAi(ch,comment,onProgress){
    if(ch.kind==='pdf'){
      var pageCandidates=reviewCandidatePdfPages(ch,comment),explicit=reviewExplicitPages(comment,(ch.pageTexts||[]).length),pageText=pageCandidates.map(function(item){return '[PDF page '+item.page+']\n'+item.text;}).join('\n\n'),pdfSystem='You locate every manuscript passage directly criticized by reviewer feedback. Choose only from the candidate PDF pages. A comment may cite multiple distinct locations; return each necessary location, up to four. Prefer explicit page, figure, table, section, and equation references. Anchor the claim or method being criticized, not merely nearby words. Copy each quote exactly from its page. Return only JSON: {"matches":[{"page": number, "quote": "short exact verbatim quote or empty", "confidence": 0 to 1}]}. If the concern is manuscript-wide or only targets supplementary material that is not shown, return an empty matches array. Never invent a quote or choose a merely related passage.',pdfUser='Reviewer comment:\n'+comment.text+'\n\nLocation hint:\n'+(comment.locationHint||'none')+'\n\nCandidate PDF pages:\n'+pageText,pdfResult=null,pdfData=null,rawMatches=[];
      try{pdfResult=await runAi(pdfSystem,pdfUser,1050,onProgress);pdfData=reviewAiJson(pdfResult.text);rawMatches=Array.isArray(pdfData&&pdfData.matches)?pdfData.matches:(pdfData&&pdfData.page?[pdfData]:[]);}catch(e){if(!explicit.length)throw e;}
      var valid=[],seen={};rawMatches.slice(0,4).forEach(function(match){var page=match&&Number.isInteger(+match.page)?+match.page:null,allowed=pageCandidates.some(function(item){return item.page===page;}),confidence=match&&match.confidence!==undefined?+match.confidence:.65;if(!page||!allowed||confidence<.55)return;var full=String((ch.pageTexts||[])[page-1]||''),range=reviewQuoteRange(full,match.quote||''),key=page+'|'+reviewNormalizedText(range&&range.quote||'');if(seen[key])return;seen[key]=1;valid.push({page:page,quote:range?range.quote:'',confidence:Math.max(0,Math.min(1,confidence)),method:range?'ai-pdf-quote':'ai-pdf-page'});});
      explicit.forEach(function(page){if(!valid.some(function(anchor){return anchor.page===page;}))valid.push({page:page,quote:'',confidence:1,method:'review-page-reference'});});valid.sort(function(a,b){var aiA=a.method==='review-page-reference'?1:0,aiB=b.method==='review-page-reference'?1:0;return aiA-aiB||explicit.indexOf(a.page)-explicit.indexOf(b.page)||a.page-b.page;});if(!valid.length)return false;
      var primary=valid[0];comment.pdfAnchors=valid;comment.page=primary.page;comment.para=null;comment.start=0;comment.end=0;comment.quote=primary.quote;comment.anchors=[];comment.anchored=true;comment.anchorMethod=primary.method;comment.matchConfidence=Math.max.apply(null,valid.map(function(anchor){return anchor.confidence;}));comment.locatedProvider=pdfResult&&pdfResult.provider||'Reviewer reference';comment.updatedAt=now();return true;
    }
  ...
  function reviewUnitRecord(unit,classification){classification=classification||{};var text=String(unit.text||'').trim(),locationHint=String(classification.locationHint||classification.location||fallbackReviewLocation(unit)).trim();return{author:String(unit.reviewer||'Reviewer'),text:text,response:String(unit.response||'').trim(),level:stabilizedReviewLevel(classification.level||classification.scope||fallbackReviewLevel(unit),text,locationHint),topic:normalizeReviewTopic(classification.topic||classification.category||fallbackReviewTopic(unit)),locationHint:locationHint};}
  async function extractReviewerReportWithAi(text,onProgress){
    var units=reviewReportUnits(text),all=[],seen={},system='Classify pre-separated reviewer concerns. Return exactly one result for every inputId, in the same order; never merge, summarize away, or omit numbered concerns. Mark generic praise, section labels, private task notes, recommendations to the editor, and other non-concerns as actionable false. Classify level as exactly one of: general (manuscript-wide), section (a whole section or subsection), specific (a passage, claim, method, equation, figure, table, or result), editorial (typo, grammar, formatting, citation, or simple consistency correction). Requests to restructure, reorganize, separate broad parts, improve overall clarity, narrow novelty, or reframe validation are general or section-level, not specific merely because they mention technical terms. Classify topic as exactly one of: writing, structure, methods, statistics, modeling, evidence, figures, consistency, claims, references, other. Preserve explicit manuscript page, section, figure, table, and equation references in locationHint. Return only JSON: {"items":[{"inputId":"exact supplied id","actionable":true,"level":"general|section|specific|editorial","topic":"writing|structure|methods|statistics|modeling|evidence|figures|consistency|claims|references|other","locationHint":"explicit manuscript location or empty"}]}.',failed=0,batchSize=6;
    ...
    for(var start=0;start<units.length;start+=batchSize){
      ...
      try{var result=await runAi(system,payload,1900,onProgress),data=reviewAiJson(result.text),items=Array.isArray(data&&data.items)?data.items:[];items.forEach(function(item){if(item&&item.inputId)byId[String(item.inputId)]=item;});}
      catch(e){failed++;}
      batch.forEach(function(unit){var classification=byId[unit.inputId],keep=unit.number||!classification||classification.actionable!==false;if(!keep)return;var record=reviewUnitRecord(unit,classification),key=reviewNormalizedText(record.text);if(!key||seen[key])return;seen[key]=1;all.push(record);});
    }
  ...
  async function importReviewerFile(ch,file,button){
    ...
    try{
      ...
      var comments=extracted.map(function(item){var record={id:uid('rr'),sourceId:reportId,author:item.author,date:'',text:item.text,level:normalizeReviewLevel(item.level),topic:normalizeReviewTopic(item.topic),locationHint:item.locationHint||'',para:null,page:null,start:0,end:0,quote:'',anchors:[],pdfAnchors:[],anchored:false,replies:[],response:item.response||'',resolved:false,sourceName:file.name,addedAt:now()},prior=priorByText[reviewNormalizedText(item.text)];if(prior){record.id=prior.id;record.response=prior.response||record.response;record.resolved=!!prior.resolved;record.replies=prior.replies||[];}return record;});ch.reviewComments=(ch.reviewComments||[]).filter(function(comment){return comment.sourceId!==reportId;}).concat(comments);ch.reviewReports=ch.reviewReports.filter(function(report){return report.id!==reportId;});var linked=0,matchable=comments.filter(reviewCanAutoLocate);
      ...
      var levels={};comments.forEach(function(comment){levels[comment.level]=(levels[comment.level]||0)+1;});ch.reviewReports.push({id:reportId,name:file.name,addedAt:now(),comments:comments.length,levels:levels,extractorVersion:2});placeInReviewWorkspace(ch);touch(ch);if(readerMode==='text')renderText(ch);renderReviewerPanel(ch);refreshPdfReviewMarkers();renderPdfReviewFocus(currentPage);updateReviewBadge();status.textContent='Imported '+comments.length+' reviewer concern'+(comments.length===1?'':'s')+' · '+linked+' linked to the PDF · filed in '+REVIEW_WORKSPACE_CATEGORY+'.';
  ...
  async function locateUnlinkedReviews(ch,button){
    ...
    try{for(var i=0;i<pending.length;i++){status.textContent='Locating '+(i+1)+' of '+pending.length+'…';try{if(await locateOneReviewWithAi(ch,pending[i],function(message){status.textContent=message;}))found++;}catch(e){status.textContent=e.message||'AI could not locate that comment.';}}touch(ch);renderText(ch);renderReviewerPanel(ch);refreshPdfReviewMarkers();status.textContent=found?'Linked '+found+' comment'+(found===1?'':'s')+' to the manuscript.':'No confident passage matches were found.';}
  ...
  function renderReviewerPanel(ch){
    ch=ch||find(currentId);var comments=ch&&ch.reviewComments||[],list=byId('readerReviewList'),count=byId('readerReviewerCount'),locate=byId('locateReviewsBtn'),filters=byId('reviewerFilters'),importButton=byId('importReviewFileBtn'),status=byId('reviewerLocateStatus'),legacyReport=(ch&&ch.reviewReports||[]).some(function(report){return !report.extractorVersion||report.extractorVersion<2;});count.textContent=comments.length?'· '+comments.filter(function(comment){return !comment.resolved;}).length:'';importButton.textContent=legacyReport?'↻ Re-import reviewer Word file':'＋ Import reviewer Word file';if(legacyReport&&!status.textContent)status.textContent='This review used the older summary importer. Re-import the same Word file to replace the incomplete list.';
