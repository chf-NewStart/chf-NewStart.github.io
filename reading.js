(function(){
  ...
  /* Saved review comments are normalized while the library is loading, before the
     rest of the app initializes. Keep their label tables above loadState() so a
     reload cannot discard an otherwise valid paper just because it has reviews. */
  ...
  function loadSharedAiPass(){
    var pass=null;try{pass=JSON.parse(sessionStorage.getItem(AI_PASS_SESSION_KEY)||'null');}catch(e){}if(!pass||!normalizePassEndpoint(pass.endpoint)||!/^[A-Za-z0-9_-]{24,}$/.test(String(pass.token||'')))return null;var expires=+pass.activeUntil||+pass.claimBefore||0;if(expires&&expires<=now()){try{sessionStorage.removeItem(AI_PASS_SESSION_KEY);}catch(e){}return null;}return pass;
  }
  function saveSharedAiPass(pass){sharedAiPass=pass||null;try{if(pass)sessionStorage.setItem(AI_PASS_SESSION_KEY,JSON.stringify(pass));else sessionStorage.removeItem(AI_PASS_SESSION_KEY);}catch(e){}renderSharedAiPass();}
  function sharedReviewPassRoute(){
    sharedAiPass=loadSharedAiPass();if(!sharedAiPass)return null;if(!sharedAiPass.jobId){sharedAiPass.jobId=uid('review-job-');saveSharedAiPass(sharedAiPass);}return{id:'review-pass',label:'Shared DeepSeek pass',cfg:{endpoint:sharedAiPass.endpoint,token:sharedAiPass.token,jobId:sharedAiPass.jobId}};
  }
  function reviewPassExpiryLabel(pass){var expires=+pass.activeUntil||0;if(!expires)return'Available for one reviewer workflow.';var minutes=Math.max(1,Math.ceil((expires-now())/60000));return'Active for this review · '+minutes+' min remaining.';}
  function renderSharedAiPass(){
    var pass=loadSharedAiPass(),banner=byId('cloudPassBanner'),detail=byId('cloudPassBannerDetail');if(!banner)return;banner.classList.toggle('hidden',!pass);if(pass&&detail)detail.textContent=reviewPassExpiryLabel(pass);var status=byId('aiPassStatus');if(status&&pass)status.textContent='A shared high-accuracy pass is ready on this device. It will be removed after the review finishes.';
  }
  function updateSharedPassFromResponse(response){
    if(!sharedAiPass||!response)return;var activeUntil=+response.headers.get('X-Phloem-Pass-Expires')||0;if(activeUntil){sharedAiPass.activeUntil=activeUntil;saveSharedAiPass(sharedAiPass);}
  }
  async function finishSharedReviewPass(route){
    if(!route||route.id!=='review-pass')return;try{await fetch(normalizePassEndpoint(route.cfg.endpoint)+'/complete',{method:'POST',headers:{'Authorization':'Bearer '+route.cfg.token,'Content-Type':'application/json'},body:JSON.stringify({job_id:route.cfg.jobId})});}catch(e){}saveSharedAiPass(null);
  }
  ...
  function reviewAiPlan(){
    var shared=sharedReviewPassRoute();if(shared)return{mode:'shared',classification:shared,location:shared};aiSettings=loadAiSettings();var selected=aiSettings.provider||'auto',route=selected==='auto'?(browserLanguageModel()?{id:'browser',label:'Gemini Nano (on device)',cfg:{}}:null):cloudAiRoute(selected);
    return{mode:selected==='auto'?(route?'local':'none'):(route?'cloud':'none'),classification:route,location:route};
  }
  function hasReviewAiRoute(){return!!sharedReviewPassRoute()||hasAiRoute();}
  ...
  async function runCloudAi(route,messages,maxTokens){
    ...
    if(id==='review-pass'){
      response=await fetch(normalizePassEndpoint(cfg.endpoint)+'/review',{method:'POST',headers:{'Authorization':'Bearer '+cfg.token,'Content-Type':'application/json'},body:JSON.stringify({job_id:cfg.jobId,messages:messages,max_tokens:maxTokens})});
      if(!response.ok&&(response.status===401||response.status===403||response.status===410||response.status===429)){saveSharedAiPass(null);throw aiSetupError(response.status===429?'This shared review pass has reached its usage limit.':'This shared review pass has expired or was already used.');}
      await aiResponseError(response,route.label);updateSharedPassFromResponse(response);data=await response.json();text=String(((((data.choices||[])[0]||{}).message||{}).content)||'');
    }else if(id==='gemini'){
  ...
  function reviewResponseWasImported(comment,report){
  ...
  function normalizedPdfPosition(value){
    if(!value||typeof value!=='object')return null;
    var page=Math.max(1,Math.floor(+value.page||1)),x=Math.max(0,Math.min(1,Number.isFinite(+value.x)?+value.x:.5)),y=Math.max(0,Math.min(1,Number.isFinite(+value.y)?+value.y:0));
    var position={v:1,page:page,x:x,y:y,screenX:Math.max(0,Math.min(1,Number.isFinite(+value.screenX)?+value.screenX:.5)),screenY:Math.max(0,Math.min(1,Number.isFinite(+value.screenY)?+value.screenY:.4)),updatedAt:+value.updatedAt||0};
    if(Number.isFinite(+value.pdfX)&&Number.isFinite(+value.pdfY)){position.pdfX=+value.pdfX;position.pdfY=+value.pdfY;}
    if(Number.isFinite(+value.zoom))position.zoom=Math.max(0,Math.min(4,+value.zoom));
    if(['scroll','page','book'].indexOf(value.layout)>=0)position.layout=value.layout;
    if(['full','left','right','text','custom','fit'].indexOf(value.zoomMode)>=0)position.zoomMode=value.zoomMode;
    return position;
  }
  ...
  /* Interface light and paper appearance are intentionally independent. A dark room
     can therefore surround an unfiltered paper when a figure's real colors matter. */
  function applyTheme(dark){
    ...
    var themeLabel = dark ? 'Switch to light interface' : 'Switch to dark interface';
    tb.setAttribute('aria-label', themeLabel); zt.setAttribute('aria-label', themeLabel);
    tb.title = zt.title = dark ? 'Light interface' : 'Dark interface';
  ...
  try{var savedPaperAppearance=localStorage.getItem(PAPER_APPEARANCE_KEY);if(['original','cream','inverted'].indexOf(savedPaperAppearance)>=0)paperAppearance=savedPaperAppearance;}catch(e){}
  ...
  byId('readerBack').onclick = function(){
    ...
    if(!byId('touchDockMenu').classList.contains('hidden')){closeTouchDockMore(true);return;}
    if(!byId('comfortBar').classList.contains('hidden')){setComfortBarOpen(false,true);return;}
    if(byId('notebook').classList.contains('sheet-open')){toggleSheet(false);return;}
  ...
  async function refreshPhloem(){saveCurrentReadingPosition(false);persist(false);await flushStateSnapshot();location.reload();}
  byId('refreshBtn').onclick = refreshPhloem;
  /* A lifecycle flush writes only the cursor fields into the newest stored record. A
     second tab may have saved notes or reading preferences since this tab loaded; writing
     our whole in-memory chapter on pagehide would silently roll those newer fields back. */
  function flushReadingLifecycle(){
    ...
    try{
      ...
      if(ch.kind==='pdf'){
        /* Preserve a deliberate preference written by a newer same-browser tab while
           adding this tab's cursor. The cursor and its effective Scroll zoom travel as
           one unit on subsequent device merges. */
        if(ch.pdfPosition&&(+stored.zoomPreferenceV||0)>=PDF_ZOOM_PREFERENCE_VERSION&&Number.isFinite(+stored.zoom)&&+stored.zoom!==+ch.zoom)ch.pdfPosition.zoom=Math.max(0,Math.min(4,+stored.zoom));
        if(ch.pdfPosition)stored.pdfPosition=ch.pdfPosition;
        stored.readPage=ch.readPage;stored.readThroughPage=Math.max(+stored.readThroughPage||0,+ch.readThroughPage||0);
      }else stored.readerScroll=ch.readerScroll;
      stored.lastOpenedAt=Math.max(+stored.lastOpenedAt||0,+ch.lastOpenedAt||0);
      state.savedAt=latest.savedAt=now();serialized=JSON.stringify(latest);localStorage.setItem(KEY,serialized);
      stateLoadFailed=false;storageWarned=false;queueStateSnapshot(serialized,true);
  ...
  var localSourceReady=Object.create(null),localSourceScanDone=false,localSourceScanPromise=null,localSourceMutation=0,localAvailabilityRenderTimer=null;
  function db(){
  ...
  function scheduleLocalAvailabilityRender(){
    clearTimeout(localAvailabilityRenderTimer);localAvailabilityRenderTimer=setTimeout(function(){if(byId('libraryPage')&&!byId('libraryPage').classList.contains('hidden'))renderShelf();},80);
  }
  function refreshLocalSourceStates(shouldRender){
    if(localSourceScanPromise)return localSourceScanPromise;
    localSourceScanPromise=(async function(){
      var keys=[],stamp;
      do{
        stamp=localSourceMutation;
        try{var d=await db();keys=await new Promise(function(res,rej){var request=d.transaction('pdfs').objectStore('pdfs').getAllKeys();request.onsuccess=function(){res(request.result||[]);};request.onerror=function(){rej(request.error);};});}
        catch(e){keys=[];}
      }while(stamp!==localSourceMutation);
      var next=Object.create(null);keys.forEach(function(id){next[String(id)]=true;});localSourceReady=next;localSourceScanDone=true;syncOfflineFilterUi();if(shouldRender)renderShelf();return next;
    })();
  ...
  async function downloadForOfflineReading(ch,button){
    if(!ch||ch.kind!=='pdf')return;var original=button&&button.textContent;if(button){button.disabled=true;button.textContent='Downloading…';}
    try{
      var stored=await getPdf(ch.id),kept=stored?await putPdf(ch.id,stored):false;
      if(!kept&&gdriveOn()){
        stored=await gdriveFetchSource(ch,true,function(loaded,total){if(button&&total)button.textContent=Math.min(99,Math.round(loaded/total*100))+'%';});
        kept=!!localSourceReady[ch.id];if(stored&&!kept)kept=await putPdf(ch.id,stored);
      }
      async function sourceBytes(){
        var attempts=[];if(ch.sourceUrl)attempts.push({url:ch.sourceUrl,options:{redirect:'follow',cache:'no-store'}});
        if(ch.sourcePath&&syncCfg&&syncCfg.repo&&syncCfg.token)attempts.push({url:ghUrl(ch.sourcePath),options:{headers:{Authorization:'Bearer '+syncCfg.token,Accept:'application/vnd.github.raw+json'},cache:'no-store'}});
        if(ch.sourcePath&&location.protocol!=='file:')attempts.push({url:new URL(String(ch.sourcePath).replace(/^\/+/,''),location.origin+'/').href,options:{cache:'no-store'}});
        for(var attempt=0;attempt<attempts.length;attempt++)try{var response=await fetch(attempts[attempt].url,attempts[attempt].options);if(!response.ok)continue;var bytes=await response.arrayBuffer();if(bytes&&bytes.byteLength>=100&&looksLikePdf(bytes))return bytes;}catch(e){}
        return null;
      }
      if(!kept){stored=await sourceBytes();if(stored)kept=await putPdf(ch.id,stored);}
      if(!kept)throw new Error('Phloem could not keep a durable offline copy. The original may be unavailable, or this browser may need storage space.');
      if(!ch.contentHash&&stored)rememberPdfFingerprint(ch,stored);renderShelf();showReaderToast('Downloaded for offline reading on this device');
    }catch(e){showError(e.message||'The paper could not be downloaded for offline reading.','Offline download did not finish');}
    finally{if(button&&button.isConnected){button.disabled=false;button.textContent=original;}}
  }
  async function seedStarterGuide(){
    if(!pristineLibrary||state.chapters.length||syncCfg||gdriveOn()||location.protocol==='file:'||/^#(?:(?:phloem|carrel|margin)-setup|phloem-review|phloem-ai-pass)=/.test(location.hash))return false;
    try{
  ...
  async function deletePdf(id){
    localSourceMutation++;delete localSourceReady[id];scheduleLocalAvailabilityRender();delete memoryPdfs[id];delete memoryDerived[id];clearTimeout(derivedTimers[id]);delete derivedTimers[id];
  ...
  function librarySearchText(value){return String(value||'').replace(/\s+/g,' ').trim().toLowerCase();}
  function librarySearchScore(value,query){
    var text=librarySearchText(value),needle=librarySearchText(query);if(!text||!needle)return 0;
    if(text.indexOf(needle)>=0)return 3;
    var words=needle.split(' ').filter(Boolean);return words.length>1&&words.every(function(word){return text.indexOf(word)>=0;})?1:0;
  }
  function librarySearchSnippet(value,query,limit){
    var text=String(value||'').replace(/\s+/g,' ').trim(),max=Math.max(80,+limit||240);if(text.length<=max)return text;
    var lower=text.toLowerCase(),needle=librarySearchText(query),at=lower.indexOf(needle);
    if(at<0){var words=needle.split(' ').filter(Boolean);for(var i=0;i<words.length;i++){at=lower.indexOf(words[i]);if(at>=0)break;}}
    if(at<0)at=0;var start=Math.max(0,Math.min(text.length-max,at-Math.round(max*.28))),end=Math.min(text.length,start+max),snippet=text.slice(start,end).trim();
    return(start?'…':'')+snippet+(end<text.length?'…':'');
  }
  function librarySearchMarkup(value,query,limit){
    var snippet=librarySearchSnippet(value,query,limit),needle=librarySearchText(query);if(!snippet||!needle)return esc(snippet);
    var lower=snippet.toLowerCase(),terms=lower.indexOf(needle)>=0?[needle]:needle.split(' ').filter(Boolean);terms=terms.filter(function(term,index){return term&&terms.indexOf(term)===index;}).sort(function(a,b){return b.length-a.length;});
    if(!terms.length)return esc(snippet);var pattern=terms.map(function(term){return term.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');}).join('|'),matcher;
    try{matcher=new RegExp('('+pattern+')','ig');}catch(e){return esc(snippet);}
    var out='',cursor=0,match;while((match=matcher.exec(snippet))){out+=esc(snippet.slice(cursor,match.index))+'<mark>'+esc(match[0])+'</mark>';cursor=match.index+match[0].length;if(!match[0].length)matcher.lastIndex++;}return out+esc(snippet.slice(cursor));
  }
  function paperHaystack(ch){return librarySearchText([ch.title,ch.authors,shelfPaperCategory(ch),(ch.tags||[]).join(' '),ch.sourceName,ch.sourceUrl,(ch.reviewComments||[]).map(function(comment){return [comment.author,comment.text,comment.response].join(' ');}).join(' ')].join(' '));}
  function libraryParagraphPage(ch,para){
    if(!ch||ch.kind!=='pdf'||!Number.isInteger(+para)||+para<0)return 0;var page=0;
    (paraPageStarts(ch)||[]).forEach(function(start){if(start.para<=+para)page=start.page;});return +page||0;
  }
  function paperThinkingHits(ch,query){
    var hits=[],sourceParas=null,readerParas=null;
    function sourceParagraph(para){if(!sourceParas)sourceParas=paras(ch.fr||'');return sourceParas[para]||'';}
    function readerParagraph(para){if(!readerParas)readerParas=paras(readerSourceText(ch));return readerParas[para]||'';}
    function add(hit){
      hit.excerpt=String(hit.excerpt||'').trim();hit.note=String(hit.note||'').trim();var noteScore=librarySearchScore(hit.note,query),excerptScore=librarySearchScore(hit.excerpt,query),combinedScore=librarySearchScore(hit.excerpt+' '+hit.note,query);if(!combinedScore)return;
      hit.paperId=ch.id;hit.score=Math.max(noteScore?400+noteScore*10:0,excerptScore?300+excerptScore*10:0,250+combinedScore*10);hit.at=+hit.at||+ch.updatedAt||+ch.addedAt||0;hits.push(hit);
    }
    Object.keys(ch.pageNotes||{}).forEach(function(key){var page=key==='document'?0:+key||0;add({kind:key==='document'?'Paper note':'Page note',page:page,note:ch.pageNotes[key]});});
    Object.keys(ch.notes||{}).forEach(function(key){var para=+key;if(!Number.isInteger(para)||para<0)return;add({kind:'Margin note',para:para,excerpt:sourceParagraph(para),note:ch.notes[key]});});
    Object.keys(ch.readerNotes||{}).forEach(function(key){var para=+key;if(!Number.isInteger(para)||para<0)return;add({kind:'Reader note',para:para,page:libraryParagraphPage(ch,para),excerpt:readerParagraph(para),note:ch.readerNotes[key]});});
    Object.keys(ch.highlights||{}).forEach(function(page){(ch.highlights[page]||[]).forEach(function(mark){add({kind:'Highlight',page:+page||0,excerpt:mark.text,note:mark.note,color:mark.color,at:mark.at});});});
    (ch.textHighlights||[]).forEach(function(mark){var para=+mark.para;add({kind:'Highlight',para:Number.isInteger(para)&&para>=0?para:null,excerpt:mark.text,note:mark.note,color:mark.color,at:mark.at});});
    (ch.readerHighlights||[]).forEach(function(mark){var para=+mark.para;add({kind:'Reader highlight',para:Number.isInteger(para)&&para>=0?para:null,page:libraryParagraphPage(ch,para),excerpt:mark.text,note:mark.note,color:mark.color,at:mark.at});});
    return hits.sort(function(a,b){return b.score-a.score||b.at-a.at||(+a.page||0)-(+b.page||0)||(+a.para||0)-(+b.para||0);});
  }
  function paperSearchMatch(ch,query){
    var metadata=[ch.title,ch.authors,shelfPaperCategory(ch),(ch.tags||[]).join(' '),ch.sourceName,ch.sourceUrl].join(' '),metadataScore=librarySearchScore(metadata,query),best=null;
    (ch.reviewComments||[]).forEach(function(comment){var review=[comment.author,comment.text,comment.response].join(' '),score=librarySearchScore(review,query);if(!score||best&&best.score>=score)return;best={kind:'Reviewer work',commentId:comment.id,page:+comment.page||reviewCommentPage(ch,comment)||0,excerpt:comment.text||comment.quote||'',note:comment.response||'',score:100+score,at:+comment.updatedAt||+comment.addedAt||0};});
    if(metadataScore&&( !best||200+metadataScore>=best.score))return{kind:'Paper details',excerpt:'',note:'',score:200+metadataScore,at:+ch.updatedAt||+ch.addedAt||0};return best;
  }
  ...
  function localReadingAvailability(ch){
    ...
    if(ch.sourceType==='docx')return{available:true,known:true,state:durable?'readable-original':'readable',label:durable?'Readable offline · Word file downloaded':known?'Readable offline · original Word file not downloaded':'Readable offline'};
    return{available:true,known:true,state:'readable',label:'Readable offline'};
  }
  function syncOfflineFilterUi(){
    var button=byId('libraryOfflineFilter');if(!button)return;button.setAttribute('aria-pressed',String(libraryOfflineOnly));button.disabled=!localSourceScanDone;button.classList.toggle('active',libraryOfflineOnly);button.title=localSourceScanDone?(libraryOfflineOnly?'Show every paper':'Show only papers readable without a network connection'):'Checking downloads on this device';
  }
  function openedDate(ch){
    var stamp=+ch.lastOpenedAt||0;if(!stamp)return'Not yet';var days=Math.floor((now()-stamp)/86400000);if(days<=0)return'Today';if(days===1)return'Yesterday';if(days<14)return days+' days ago';try{return new Intl.DateTimeFormat(undefined,{month:'short',day:'numeric'}).format(new Date(stamp));}catch(e){return'Recently';}
  }
  function renderContinueReading(query,statsFor){
    ...
    var cards=section.querySelector('.continue-reading-cards');recent.forEach(function(ch){
      ...
      button.innerHTML='<span class="continue-reading-kicker">'+esc(ch.kind==='pdf'?'PDF':ch.sourceType==='docx'?'Word':'Text')+' · '+esc(openedDate(ch))+'</span><strong>'+esc(ch.title||'Untitled')+'</strong><small>'+esc(ch.authors||ch.sourceName||'')+'</small><span class="continue-reading-progress"><i style="--continue-progress:'+stats.progress+'%"></i></span><span class="continue-reading-meta"><b>'+esc(progress)+'</b></span><span class="continue-reading-availability"><em>'+esc(local.label)+'</em>'+(backup?'<em>'+esc(backup.label)+'</em>':'')+'</span>';
      button.onclick=function(){librarySelectionId=ch.id;openReader(ch.id);};cards.appendChild(button);
    });
  }
  syncOfflineFilterUi();
  syncLibraryViewControl();
  function shelfPaperStats(ch){
    ...
    var questions=(ch.questions||[]).length,page=ch.kind==='pdf'?Math.max(+ch.readPage||1,+ch.readThroughPage||0):(+ch.readPage||1),total=ch.kind==='pdf'?(+ch.pageCount||0):paras(ch.fr).length;
  ...
  function renderReviewWorkspaceCard(ch,isSelected){
    ...
    card.querySelector('.review-workspace-continue').onclick=function(){openReader(ch.id).then(function(opened){if(opened&&currentId===ch.id)revealRevisionDesk();});};
  ...
  document.addEventListener('click',function(event){var path=typeof event.composedPath==='function'?event.composedPath():[];document.querySelectorAll('.review-workspace-manage[open],.library-list-actions[open],.library-list-bulk-move[open]').forEach(function(menu){if(path.indexOf(menu)<0&&!menu.contains(event.target))menu.removeAttribute('open');});});
  document.addEventListener('keydown',function(event){if(event.key!=='Escape')return;document.querySelectorAll('.review-workspace-manage[open],.library-list-actions[open],.library-list-bulk-move[open]').forEach(function(menu){menu.removeAttribute('open');var toggle=menu.querySelector('summary');if(toggle)toggle.focus();});});
  ...
  function revealSelectedShelfCategory(categoryRail){
    var active=categoryRail&&categoryRail.querySelector('.paper-category-tab.is-selected');if(!active)return;
    var start=active.offsetLeft,end=start+active.offsetWidth,visibleStart=categoryRail.scrollLeft,visibleEnd=visibleStart+categoryRail.clientWidth,pad=7;
    if(start<visibleStart+pad)categoryRail.scrollLeft=Math.max(0,start-pad);
    else if(end>visibleEnd-pad)categoryRail.scrollLeft=Math.max(0,end-categoryRail.clientWidth+pad);
  }
  ...
  function compareShelfPapers(a,b,statsFor){
    ...
    else if(mode==='progress')cmp=(statsFor?statsFor(b):shelfPaperStats(b)).progress-(statsFor?statsFor(a):shelfPaperStats(a)).progress;
  ...
  function renderLibraryList(shelf,list,statsFor){
    var section=document.createElement('section'),categoriesNav=document.createElement('nav'),categoriesLabel=document.createElement('span'),bulkbar=document.createElement('div'),head=document.createElement('div'),rows=document.createElement('ul'),categoryNames=shelfCategoryNames(),categoryCounts={},validPaperIds={};
    state.chapters.forEach(function(ch){var key=shelfPaperCategory(ch).toLowerCase();categoryCounts[key]=(categoryCounts[key]||0)+1;validPaperIds[ch.id]=true;});
    Object.keys(libraryListSelected).forEach(function(id){if(!validPaperIds[id])delete libraryListSelected[id];});
    if(libraryListCategory!=='all'&&!categoryNames.some(function(name){return 'category:'+name.toLowerCase()===libraryListCategory;}))libraryListCategory='all';
    var visibleList=libraryListCategory==='all'?list:list.filter(function(ch){return 'category:'+shelfPaperCategory(ch).toLowerCase()===libraryListCategory;});
    section.className='library-list';section.setAttribute('aria-label','Papers in your library');
    categoriesNav.className='library-list-categories';categoriesNav.setAttribute('aria-label','Filter list by category');categoriesLabel.className='library-list-categories-label';categoriesLabel.textContent='Categories';categoriesNav.appendChild(categoriesLabel);
    function restoreListFocus(paperId){setTimeout(function(){var target;if(paperId){Array.prototype.some.call(byId('shelf').querySelectorAll('.library-list-row'),function(row){if(row.dataset.libraryPaper!==paperId)return false;target=row.querySelector('.library-list-select');return true;});}else target=byId('shelf').querySelector('.library-list-select-all');if(target)target.focus();},0);}
    function addCategoryFilter(label,dataValue,filterValue,count){
      var button=document.createElement('button');button.type='button';button.className='library-list-category-filter';button.dataset.libraryCategory=dataValue;button.setAttribute('aria-pressed',String(libraryListCategory===filterValue));button.innerHTML='<span>'+esc(label)+'</span> <b>'+count+'</b>';button.onclick=function(){if(libraryListCategory===filterValue)return;libraryListSelected={};libraryListCategory=filterValue;renderShelf();setTimeout(function(){var next=Array.prototype.find.call(byId('shelf').querySelectorAll('[data-library-category]'),function(item){return item.dataset.libraryCategory===dataValue;});if(next)next.focus();},0);};categoriesNav.appendChild(button);
    }
    addCategoryFilter('All','all','all',state.chapters.length);categoryNames.forEach(function(name){addCategoryFilter(name,name,'category:'+name.toLowerCase(),categoryCounts[name.toLowerCase()]||0);});
    function moveSelectedPapers(destination){
      var moving=state.chapters.filter(function(ch){return libraryListSelected[ch.id];});if(!moving.length)return;moving.forEach(function(ch){setShelfPaperCategory(ch,destination);});librarySelectionId=moving[0].id;libraryListSelected={};persist();renderShelf();
    }
    var selectedCount=Object.keys(libraryListSelected).length;
    if(selectedCount){
      bulkbar.className='library-list-bulkbar';var selectedStatus=document.createElement('strong'),bulkMove=document.createElement('details'),bulkSummary=document.createElement('summary'),bulkPanel=document.createElement('div'),bulkNew=document.createElement('button'),bulkClear=document.createElement('button');
      selectedStatus.className='library-list-selected-count';selectedStatus.setAttribute('aria-live','polite');selectedStatus.textContent=selectedCount+' selected';
      bulkMove.className='library-list-bulk-move';bulkSummary.className='library-list-bulk-summary';bulkSummary.textContent='Move to…';bulkPanel.className='library-list-bulk-panel';bulkPanel.innerHTML='<strong>Move selected papers to</strong>';
      var bulkNames=categoryNames.slice();if(!bulkNames.some(function(name){return name.toLowerCase()==='unsorted';}))bulkNames.push('Unsorted');
      bulkNames.forEach(function(name){var choice=document.createElement('button');choice.type='button';choice.className='library-list-bulk-category';choice.dataset.bulkCategory=name;choice.textContent=name;choice.onclick=function(){moveSelectedPapers(name);};bulkPanel.appendChild(choice);});
      bulkMove.ontoggle=function(){if(bulkMove.open)closeOtherActions(bulkMove);};bulkMove.appendChild(bulkSummary);bulkMove.appendChild(bulkPanel);
      bulkNew.type='button';bulkNew.className='library-list-bulk-new';bulkNew.textContent='＋ New category';bulkNew.onclick=function(){var answer=prompt('New category for '+selectedCount+' selected paper'+(selectedCount===1?'':'s'),'');if(answer===null||!String(answer).trim())return;moveSelectedPapers(answer);};
      bulkClear.type='button';bulkClear.className='library-list-bulk-clear';bulkClear.textContent='Clear';bulkClear.onclick=function(){libraryListSelected={};renderShelf();restoreListFocus();};
      bulkbar.appendChild(selectedStatus);bulkbar.appendChild(bulkMove);bulkbar.appendChild(bulkNew);bulkbar.appendChild(bulkClear);
    }
    head.className='library-list-head';head.innerHTML='<label class="library-list-select-all-wrap"><input class="library-list-select-all" type="checkbox"><span class="sr-only">Select all visible papers</span></label><span aria-hidden="true">Paper</span><span aria-hidden="true">Category</span><span aria-hidden="true">Progress</span><span aria-hidden="true">Last touched</span><span></span>';
    var selectAll=head.querySelector('.library-list-select-all'),visibleSelected=visibleList.filter(function(ch){return libraryListSelected[ch.id];}).length;selectAll.checked=!!visibleList.length&&visibleSelected===visibleList.length;selectAll.indeterminate=visibleSelected>0&&visibleSelected<visibleList.length;selectAll.onchange=function(){var shouldSelect=this.checked;visibleList.forEach(function(ch){if(shouldSelect)libraryListSelected[ch.id]=true;else delete libraryListSelected[ch.id];});renderShelf();restoreListFocus();};
    rows.className='library-list-rows';
    function closeOtherActions(keep){
      section.querySelectorAll('.library-list-actions[open],.library-list-bulk-move[open]').forEach(function(details){if(details!==keep)details.removeAttribute('open');});
    }
    function actionButton(panel,label,className,handler){
      var button=document.createElement('button');button.type='button';button.className='library-list-action'+(className?' '+className:'');button.textContent=label;button.onclick=handler;panel.appendChild(button);return button;
    }
    function showMoveChoices(ch,details,panel,summary){
      panel.innerHTML='';panel.classList.add('choosing-category');
      var bar=document.createElement('div'),back=document.createElement('button'),label=document.createElement('strong');
      bar.className='library-list-action-head';back.type='button';back.className='library-list-action-back';back.textContent='←';back.setAttribute('aria-label','Back to paper actions');label.textContent='Move to';bar.appendChild(back);bar.appendChild(label);panel.appendChild(bar);
      var names=shelfCategoryNames();if(!names.some(function(name){return name.toLowerCase()==='unsorted';}))names.push('Unsorted');
      names=names.filter(function(name){return name.toLowerCase()!==shelfPaperCategory(ch).toLowerCase();});
      var search=document.createElement('input'),choices=document.createElement('div'),empty=document.createElement('span');
      search.type='search';search.className='library-list-action-search';search.placeholder='Find category…';search.setAttribute('aria-label','Find category');choices.className='library-list-category-choices';empty.className='library-list-action-empty';empty.textContent=names.length?'No matching category':'No other categories yet';empty.hidden=!!names.length;
      names.forEach(function(name){
        var choice=document.createElement('button');choice.type='button';choice.className='library-list-category-choice';choice.textContent=name;choice.dataset.categorySearch=name.toLowerCase();
        choice.onclick=function(){librarySelectionId=ch.id;setShelfPaperCategory(ch,name);persist();details.removeAttribute('open');renderShelf();};choices.appendChild(choice);
      });
      choices.appendChild(empty);search.hidden=!names.length;search.oninput=function(){var query=this.value.trim().toLowerCase(),shown=0;choices.querySelectorAll('.library-list-category-choice').forEach(function(choice){var match=!query||choice.dataset.categorySearch.indexOf(query)>=0;choice.hidden=!match;if(match)shown++;});empty.hidden=shown>0;empty.textContent='No matching category';};
      panel.appendChild(search);panel.appendChild(choices);
      var makeCategory=actionButton(panel,'＋ New category…','new-category',function(){var answer=prompt('New category name','');if(answer===null||!String(answer).trim())return;librarySelectionId=ch.id;setShelfPaperCategory(ch,answer);persist();details.removeAttribute('open');renderShelf();});
      back.onclick=function(){var first=showActions(ch,details,panel,summary);if(first)first.focus();};
      setTimeout(function(){(names.length?search:makeCategory).focus();},0);
    }
    function showActions(ch,details,panel,summary){
      ...
      actionButton(panel,'Remove from library','danger',function(){details.removeAttribute('open');removePaper(ch,false);});return move;
    }
    visibleList.forEach(function(ch,index){
      var stats=statsFor(ch),review=reviewWorkspaceStats(ch),title=String(ch.title||'Untitled'),author=String(ch.authors||ch.journal||ch.venue||ch.sourceName||(ch.kind==='pdf'?'Unknown author':'Personal note')),category=shelfPaperCategory(ch),kind=ch.kind==='pdf'?'PDF':(ch.sourceType==='docx'?'Word draft':'Text'),facts=[kind],progressContext=(ch.kind==='pdf'?'Page ':'Paragraph ')+stats.page+(stats.total?' of '+stats.total:''),touched=shelfDate(ch),local=localReadingAvailability(ch),backup=binarySourceSpec(ch)?gdrivePaperStatus(ch):null;
      facts.push(local.label);if(backup)facts.push(backup.label);
      if(stats.notes)facts.push(stats.notes+' mark'+(stats.notes===1?'':'s'));if(stats.questions)facts.push(stats.questions+' question'+(stats.questions===1?'':'s'));if(review.open)facts.push(review.open+' review '+(review.open===1?'note':'notes')+' open');
      var row=document.createElement('li'),selectWrap=document.createElement('label'),select=document.createElement('input'),open=document.createElement('button'),details=document.createElement('details'),summary=document.createElement('summary'),panel=document.createElement('div'),titleId='library-list-title-'+index,authorId='library-list-author-'+index,categoryId='library-list-category-'+index,progressId='library-list-progress-'+index,touchedId='library-list-touched-'+index;
      row.className='library-list-row'+(libraryListSelected[ch.id]?' is-selected':'');row.dataset.libraryPaper=ch.id;
      selectWrap.className='library-list-select-wrap';select.type='checkbox';select.className='library-list-select';select.checked=!!libraryListSelected[ch.id];select.setAttribute('aria-label','Select '+title);select.onchange=function(){if(this.checked)libraryListSelected[ch.id]=true;else delete libraryListSelected[ch.id];renderShelf();restoreListFocus(ch.id);};selectWrap.appendChild(select);
      open.type='button';open.className='library-list-open';open.dataset.libraryPaper=ch.id;open.setAttribute('aria-labelledby',titleId);open.setAttribute('aria-describedby',[authorId,categoryId,progressId,touchedId].join(' '));
      open.innerHTML='<span class="library-list-identity"><strong class="library-list-title" id="'+titleId+'">'+esc(title)+'</strong><span class="library-list-author" id="'+authorId+'">'+esc(author)+'</span><small class="library-list-facts">'+esc(facts.join(' · '))+'</small></span><span class="library-list-category'+(category.toLowerCase()===REVIEW_WORKSPACE_CATEGORY.toLowerCase()?' is-review':'')+'" id="'+categoryId+'">'+esc(category)+'</span><span class="library-list-progress" id="'+progressId+'"><span class="library-list-progress-copy"><b>'+stats.progress+'%</b><small>'+esc(progressContext)+'</small></span><span class="library-list-progress-track" aria-hidden="true"><i style="--library-progress:'+stats.progress+'%"></i></span></span><span class="library-list-touched" id="'+touchedId+'">'+esc(touched)+'</span>';
      open.onclick=function(){librarySelectionId=ch.id;var continueReview=category.toLowerCase()===REVIEW_WORKSPACE_CATEGORY.toLowerCase()&&(ch.reviewComments||[]).length;openReader(ch.id).then(function(opened){if(opened&&continueReview&&currentId===ch.id)revealRevisionDesk();});};
      details.className='library-list-actions';summary.className='library-list-more';summary.textContent='⋯';summary.setAttribute('aria-label','Actions for '+title);panel.className='library-list-action-panel';details.appendChild(summary);details.appendChild(panel);
      details.ontoggle=function(){if(details.open){closeOtherActions(details);showActions(ch,details,panel,summary);}};
      row.appendChild(selectWrap);row.appendChild(open);row.appendChild(details);rows.appendChild(row);
    });
    if(!visibleList.length){var empty=document.createElement('li');empty.className='library-list-filter-empty';empty.textContent='No papers in this category match the current search.';rows.appendChild(empty);}
    if(libraryListCategory!=='all'||byId('librarySearch').value.trim())byId('libraryCount').textContent=visibleList.length+' of '+state.chapters.length+(state.chapters.length===1?' paper':' papers');
    section.appendChild(categoriesNav);if(selectedCount)section.appendChild(bulkbar);section.appendChild(head);section.appendChild(rows);shelf.appendChild(section);
  }
  function libraryThinkingLocation(ch,hit){
    var page=+hit.page||0,hasPara=Number.isInteger(hit.para)&&hit.para>=0;
    if(page)return{label:hit.kind+' · page '+page,action:'Open on page '+page,aria:'Open '+(ch.title||'paper')+' on page '+page};
    if(hasPara&&ch.kind!=='pdf')return{label:hit.kind+' · paragraph '+(hit.para+1),action:'Open at paragraph '+(hit.para+1),aria:'Open '+(ch.title||'paper')+' at paragraph '+(hit.para+1)};
    if(hasPara)return{label:hit.kind+' · Reader passage',action:'Open at passage',aria:'Open '+(ch.title||'paper')+' at this Reader passage'};
    return{label:hit.kind,action:'Open paper',aria:'Open '+(ch.title||'paper')};
  }
  async function openLibraryThinkingHit(ch,hit){
    if(!ch)return;librarySelectionId=ch.id;var opened=await openReader(ch.id);if(!opened||currentId!==ch.id)return;
    if(hit.commentId){var comment=reviewerById(ch,hit.commentId);showReviewerComment(ch,hit.commentId);if(comment&&reviewHasDisplayablePassage(ch,comment)){await focusReviewerPassage(ch,hit.commentId);return;}}
    if(+hit.page>0)await gotoPdfPage(+hit.page,'auto');else if(Number.isInteger(hit.para)&&hit.para>=0)jumpToParagraph(hit.para);
  }
  function renderLibrarySearchResults(shelf,papers,hitsByPaper,paperMatches,query){
    var section=document.createElement('section'),groups=document.createElement('div'),intro=document.createElement('header'),managing=libraryViewMode==='list',categoryNames=shelfCategoryNames(),categoryCounts={},validPaperIds={},visiblePapers;
    papers.forEach(function(ch){var key=shelfPaperCategory(ch).toLowerCase();categoryCounts[key]=(categoryCounts[key]||0)+1;validPaperIds[ch.id]=true;});
    Object.keys(libraryListSelected).forEach(function(id){if(!validPaperIds[id])delete libraryListSelected[id];});
    if(managing&&libraryListCategory!=='all'&&!categoryNames.some(function(name){return 'category:'+name.toLowerCase()===libraryListCategory;}))libraryListCategory='all';
    visiblePapers=!managing||libraryListCategory==='all'?papers:papers.filter(function(ch){return 'category:'+shelfPaperCategory(ch).toLowerCase()===libraryListCategory;});
    var annotationCount=0,metadataOnly=0;visiblePapers.forEach(function(ch){var hits=hitsByPaper[ch.id]||[];annotationCount+=hits.length;if(!hits.length&&paperMatches[ch.id])metadataOnly++;});
    var total=annotationCount+metadataOnly,summary=total+' '+(total===1?'match':'matches')+' in '+visiblePapers.length+' '+(visiblePapers.length===1?'paper':'papers');
    byId('libraryCount').textContent=summary;section.className='library-thinking-results';section.setAttribute('aria-label','Search results');intro.className='library-thinking-intro';
    intro.innerHTML='<div><span>Search across your thinking</span><h3>'+esc(summary)+'</h3></div><p>Paper details, highlighted passages, your notes, and reviewer work—kept beside the place they came from.</p>';
    function restoreThinkingFocus(paperId){setTimeout(function(){var shelf=byId('shelf'),target;if(paperId){Array.prototype.some.call(shelf.querySelectorAll('[data-thinking-group]'),function(group){if(group.dataset.thinkingGroup!==paperId)return false;target=group.querySelector('.library-thinking-select');return true;});}else target=shelf.querySelector('.library-thinking-select-all');if(target)target.focus();},0);}
    function moveSelectedPapers(destination){var moving=state.chapters.filter(function(ch){return libraryListSelected[ch.id];});if(!moving.length)return;moving.forEach(function(ch){setShelfPaperCategory(ch,destination);});librarySelectionId=moving[0].id;libraryListSelected={};persist();renderShelf();}
    if(managing){
      var categoriesNav=document.createElement('nav'),categoriesLabel=document.createElement('span');categoriesNav.className='library-list-categories';categoriesNav.setAttribute('aria-label','Filter search results by category');categoriesLabel.className='library-list-categories-label';categoriesLabel.textContent='Categories';categoriesNav.appendChild(categoriesLabel);
      function addCategoryFilter(label,dataValue,filterValue,count){var button=document.createElement('button');button.type='button';button.className='library-list-category-filter';button.dataset.libraryCategory=dataValue;button.setAttribute('aria-pressed',String(libraryListCategory===filterValue));button.innerHTML='<span>'+esc(label)+'</span> <b>'+count+'</b>';button.onclick=function(){if(libraryListCategory===filterValue)return;libraryListSelected={};libraryListCategory=filterValue;renderShelf();};categoriesNav.appendChild(button);}
      addCategoryFilter('All','all','all',papers.length);categoryNames.forEach(function(name){addCategoryFilter(name,name,'category:'+name.toLowerCase(),categoryCounts[name.toLowerCase()]||0);});section.appendChild(categoriesNav);
      var selectedCount=Object.keys(libraryListSelected).length;
      if(selectedCount){
        var bulkbar=document.createElement('div'),selectedStatus=document.createElement('strong'),bulkMove=document.createElement('details'),bulkSummary=document.createElement('summary'),bulkPanel=document.createElement('div'),bulkNew=document.createElement('button'),bulkClear=document.createElement('button');
        bulkbar.className='library-list-bulkbar';selectedStatus.className='library-list-selected-count';selectedStatus.setAttribute('aria-live','polite');selectedStatus.textContent=selectedCount+' selected';bulkMove.className='library-list-bulk-move';bulkSummary.className='library-list-bulk-summary';bulkSummary.textContent='Move to…';bulkPanel.className='library-list-bulk-panel';bulkPanel.innerHTML='<strong>Move selected papers to</strong>';
        var bulkNames=categoryNames.slice();if(!bulkNames.some(function(name){return name.toLowerCase()==='unsorted';}))bulkNames.push('Unsorted');bulkNames.forEach(function(name){var choice=document.createElement('button');choice.type='button';choice.className='library-list-bulk-category';choice.dataset.bulkCategory=name;choice.textContent=name;choice.onclick=function(){moveSelectedPapers(name);};bulkPanel.appendChild(choice);});bulkMove.appendChild(bulkSummary);bulkMove.appendChild(bulkPanel);
        bulkNew.type='button';bulkNew.className='library-list-bulk-new';bulkNew.textContent='＋ New category';bulkNew.onclick=function(){var answer=prompt('New category for '+selectedCount+' selected paper'+(selectedCount===1?'':'s'),'');if(answer===null||!String(answer).trim())return;moveSelectedPapers(answer);};bulkClear.type='button';bulkClear.className='library-list-bulk-clear';bulkClear.textContent='Clear';bulkClear.onclick=function(){libraryListSelected={};renderShelf();restoreThinkingFocus();};bulkbar.appendChild(selectedStatus);bulkbar.appendChild(bulkMove);bulkbar.appendChild(bulkNew);bulkbar.appendChild(bulkClear);section.appendChild(bulkbar);
      }
      var controls=document.createElement('div'),selectAllLabel=document.createElement('label'),selectAll=document.createElement('input');controls.className='library-thinking-controls';selectAllLabel.className='library-thinking-select-all-wrap';selectAll.type='checkbox';selectAll.className='library-thinking-select-all';var visibleSelected=visiblePapers.filter(function(ch){return libraryListSelected[ch.id];}).length;selectAll.checked=!!visiblePapers.length&&visibleSelected===visiblePapers.length;selectAll.indeterminate=visibleSelected>0&&visibleSelected<visiblePapers.length;selectAll.setAttribute('aria-label','Select all visible search results');selectAll.onchange=function(){var shouldSelect=this.checked;visiblePapers.forEach(function(ch){if(shouldSelect)libraryListSelected[ch.id]=true;else delete libraryListSelected[ch.id];});renderShelf();restoreThinkingFocus();};selectAllLabel.appendChild(selectAll);selectAllLabel.appendChild(document.createTextNode(' Select visible papers'));controls.appendChild(selectAllLabel);section.appendChild(controls);
    }
    section.appendChild(intro);groups.className='library-thinking-groups';
    function appendHit(container,ch,hit,isMetadata){
      var location=libraryThinkingLocation(ch,hit),article=document.createElement('article'),color=['yellow','mint','coral','blue'].indexOf(hit.color)>=0?hit.color:'yellow';
      article.className='library-thinking-hit'+(isMetadata?' is-paper-match':'')+' thought-'+color;article.dataset.thinkingPaper=ch.id;article.dataset.thinkingType=hit.kind;
      article.innerHTML='<div class="library-thinking-copy"><div class="library-thinking-hit-meta"><span>'+esc(location.label)+'</span><strong class="library-thinking-paper">'+esc(ch.title||'Untitled')+'</strong></div>'+(hit.excerpt?'<blockquote class="library-thinking-excerpt">'+librarySearchMarkup(hit.excerpt,query,280)+'</blockquote>':'')+(hit.note?'<p class="library-thinking-note"><b>'+(isMetadata&&hit.kind==='Reviewer work'?'Your revision note':'Your note')+'</b><span>'+librarySearchMarkup(hit.note,query,230)+'</span></p>':'')+(!hit.excerpt&&!hit.note?'<p class="library-thinking-context">Matched '+(hit.kind==='Paper details'?'the title, author, category, tags, or source details.':'reviewer work attached to this paper.')+'</p>':'')+'</div>';
      var open=document.createElement('button');open.type='button';open.className='soft-button library-thinking-open';open.textContent=location.action;open.setAttribute('aria-label',location.aria);open.onclick=function(){openLibraryThinkingHit(ch,hit);};article.appendChild(open);container.appendChild(article);
    }
    visiblePapers.forEach(function(ch){
      var hits=hitsByPaper[ch.id]||[],match=paperMatches[ch.id],group=document.createElement('section'),list=document.createElement('div'),shown=Math.min(3,hits.length),author=String(ch.authors||ch.sourceName||'').trim();
      group.className='library-thinking-group'+(libraryListSelected[ch.id]?' is-selected':'');group.dataset.thinkingGroup=ch.id;group.innerHTML='<header class="library-thinking-group-head">'+(managing?'<label class="library-thinking-select-wrap"><input class="library-thinking-select" type="checkbox"'+(libraryListSelected[ch.id]?' checked':'')+' aria-label="Select '+esc(ch.title||'Untitled')+'"><span class="sr-only">Select '+esc(ch.title||'Untitled')+'</span></label>':'')+'<div class="library-thinking-identity"><h3>'+esc(ch.title||'Untitled')+'</h3>'+(author?'<p>'+esc(author)+'</p>':'')+'</div><span>'+esc(shelfPaperCategory(ch))+'</span></header>';list.className='library-thinking-hit-list';
      var select=group.querySelector('.library-thinking-select');if(select)select.onchange=function(){if(this.checked)libraryListSelected[ch.id]=true;else delete libraryListSelected[ch.id];renderShelf();restoreThinkingFocus(ch.id);};
      for(var i=0;i<shown;i++)appendHit(list,ch,hits[i],false);
      if(!hits.length&&match)appendHit(list,ch,match,true);
      if(hits.length>shown){var more=document.createElement('button');more.type='button';more.className='text-button library-thinking-more';more.textContent='Show '+(hits.length-shown)+' more '+(hits.length-shown===1?'match':'matches');more.onclick=function(){var start=shown;for(var index=start;index<hits.length;index++)appendHit(list,ch,hits[index],false);more.remove();};list.appendChild(more);}
      group.appendChild(list);groups.appendChild(group);
    });
    if(!visiblePapers.length)groups.innerHTML='<div class="library-thinking-empty"><b>No matches in this category</b><span>Choose All or another category to see matching papers.</span></div>';
    section.appendChild(groups);shelf.appendChild(section);
  }
  function renderShelf(){
    ...
    var q=librarySearchText(byId('librarySearch').value),thinkingHitsByPaper={},paperMatches={},resultCount=0;
    var statsCache={};function statsFor(ch){var key=String(ch.id||'');if(!statsCache[key])statsCache[key]=shelfPaperStats(ch);return statsCache[key];}
    var list=state.chapters.filter(function(ch){
      if(libraryOfflineOnly&&!localReadingAvailability(ch).available)return false;
      if(!q)return true;var hits=paperThinkingHits(ch,q),match=paperSearchMatch(ch,q);if(hits.length)thinkingHitsByPaper[ch.id]=hits;if(match)paperMatches[ch.id]=match;if(!hits.length&&!match)return false;resultCount+=hits.length||1;return true;
    }).sort(function(a,b){return compareShelfPapers(a,b,statsFor);});
    renderContinueReading(q,statsFor);syncOfflineFilterUi();
    byId('libraryCount').textContent=!state.chapters.length&&libraryDriveRestoreArmed?'Drive library waiting':(q?(resultCount+' '+(resultCount===1?'match':'matches')+' in '+list.length+' '+(list.length===1?'paper':'papers')):libraryOfflineOnly?(list.length+' readable offline'):state.chapters.length+(state.chapters.length===1?' paper':' papers'));renderDuplicateNotice();
    var shelf=byId('shelf'),oldCategoryRail=shelf.querySelector('.paper-category-rail'),categoryScroll=oldCategoryRail?oldCategoryRail.scrollLeft:0;shelf.classList.toggle('library-list-mode',libraryViewMode==='list'&&!q);shelf.classList.toggle('library-search-mode',!!q);shelf.classList.remove('review-workspace-open');shelf.innerHTML='';
    if(!list.length){
      ...
      if(!state.chapters.length&&libraryDriveRestoreArmed&&gdriveOn()){shelf.innerHTML='<div class="shelf-drive-restore" role="status" aria-live="polite"><span class="shelf-drive-mark" aria-hidden="true">☁</span><div><b>Your library is in Google Drive</b><span>'+(libraryDriveRestoreMessage?esc(libraryDriveRestoreMessage):'Google needs one tap before Phloem can restore it on this device.')+'</span><button class="button" id="restoreDriveLibraryBtn" type="button">Restore library</button><small>Your papers and review work are not gone. This gives Google permission to open the private app folder.</small></div></div>';return;}
      shelf.innerHTML='<div class="shelf-empty"><div><b>'+(state.chapters.length?(libraryOfflineOnly?'No offline papers yet':'Nothing found'):'Start with a paper')+'</b>'+(state.chapters.length?(libraryOfflineOnly?'Download a PDF for offline reading, or turn off the Offline filter.':'Try another title, passage, note, author, or tag.'):'Add a PDF or Word draft, or bring a manuscript and its reviewer comments together.')+'</div></div>';return;
    }
    if(q){renderLibrarySearchResults(shelf,list,thinkingHitsByPaper,paperMatches,q);return;}
    if(!librarySelectionId||!list.some(function(ch){return ch.id===librarySelectionId;}))librarySelectionId=list[0].id;
    if(libraryViewMode==='list'){renderLibraryList(shelf,list,statsFor);return;}
    var selected=list.find(function(ch){return ch.id===librarySelectionId;})||list[0],stats=statsFor(selected),selectedCategory=shelfPaperCategory(selected).toLowerCase(),reviewWorkspaceOpen=selectedCategory===REVIEW_WORKSPACE_CATEGORY.toLowerCase();
    ...
    noteSurface.appendChild(pile);caseEl.appendChild(categoryRail);caseEl.appendChild(noteSurface);shelf.appendChild(caseEl);categoryRail.scrollLeft=categoryScroll;if(revealShelfCategoryOnRender){revealShelfCategoryOnRender=false;revealSelectedShelfCategory(categoryRail);}if(!reviewWorkspaceOpen)shelf.appendChild(renderOpenPaper(selected,stats,BOOK_SPINES[paperVisualHash(selected)%BOOK_SPINES.length]));
  }
  byId('librarySearch').oninput=function(){libraryListSelected={};clearTimeout(librarySearchTimer);librarySearchTimer=setTimeout(renderShelf,90);};
  byId('libraryOfflineFilter').onclick=function(){if(!localSourceScanDone)return;libraryOfflineOnly=!libraryOfflineOnly;libraryListSelected={};try{localStorage.setItem(LIBRARY_OFFLINE_KEY,libraryOfflineOnly?'1':'0');}catch(e){}renderShelf();};
  ...
  var REVIEW_STEPS=[1,3,7,14,30,60], reviewQueue=[], reviewIndex=0, reviewPageMode='reviewers', reviewInboxFilter='open', reviewInboxQuery='', reviewInboxLimit=12;
  ...
  function updateReviewBadge(){
    ...
    document.querySelectorAll('[data-view="reviewPage"]').forEach(function(b){b.textContent=open?'Revision · '+open:'Revision';});
  }
  ...
  function reviewSourceLabel(comment){var number=String(comment&&comment.sourceNumber||'').trim(),group=String(comment&&comment.sourceGroup||'').trim();return number?'Concern '+number:group;}
  ...
  function renderReviewerInbox(){
    var box=byId('reviewerDesk'),items=reviewerReviewItems(),open=items.filter(function(item){return !item.comment.resolved;}).length,needs=items.filter(function(item){return !item.comment.resolved&&reviewNeedsPassage(item.ch,item.comment);}).length,resolved=items.length-open;byId('reviewCount').textContent=items.length?(open+' open · '+items.length+' total'):'';byId('reviewerCount').textContent=items.length?'· '+open:'';
    ...
    var query=reviewInboxQuery.trim().toLowerCase(),filtered=items.filter(function(item){var comment=item.comment;if(reviewInboxFilter==='open'&&comment.resolved)return false;if(reviewInboxFilter==='needs'&&(comment.resolved||!reviewNeedsPassage(item.ch,comment)))return false;if(reviewInboxFilter==='resolved'&&!comment.resolved)return false;if(!query)return true;return [item.ch.title,item.ch.authors,item.ch.sourceName,comment.author,comment.text,comment.locationHint,comment.sourceGroup,comment.sourceNumber,reviewLevelLabel(comment.level),reviewTopicLabel(comment.topic)].join(' ').toLowerCase().indexOf(query)>=0;}),shown=filtered.slice(0,reviewInboxLimit),groups=[],groupMap={};
    shown.forEach(function(item){var groupName=String(item.comment.sourceGroup||'').trim(),key=item.ch.id+'|'+String(item.comment.author||'Reviewer')+'|'+groupName;if(!groupMap[key]){groupMap[key]={ch:item.ch,author:item.comment.author||'Reviewer',group:groupName,items:[]};groups.push(groupMap[key]);}groupMap[key].items.push(item);});
    var filters='<div class="revision-inbox-tools"><div class="revision-inbox-filters" role="group" aria-label="Filter reviewer comments"><button type="button" data-revision-filter="open" aria-pressed="'+String(reviewInboxFilter==='open')+'">Open · '+open+'</button>'+(needs?'<button type="button" data-revision-filter="needs" aria-pressed="'+String(reviewInboxFilter==='needs')+'">Needs passage · '+needs+'</button>':'')+(resolved?'<button type="button" data-revision-filter="resolved" aria-pressed="'+String(reviewInboxFilter==='resolved')+'">Resolved · '+resolved+'</button>':'')+'<button type="button" data-revision-filter="all" aria-pressed="'+String(reviewInboxFilter==='all')+'">All · '+items.length+'</button></div><input class="revision-inbox-search" id="revisionInboxSearch" type="search" value="'+esc(reviewInboxQuery)+'" placeholder="Search comments or papers…" aria-label="Search reviewer comments"></div>';
    var body=!shown.length?'<div class="empty revision-inbox-empty">No comments match this view.</div>':groups.map(function(group){return '<section class="revision-inbox-group"><header><div><span>'+esc(group.author)+(group.group?' · '+esc(group.group):'')+'</span><h3>'+esc(group.ch.title||group.ch.sourceName||'Untitled')+'</h3></div><small>'+group.items.length+' shown</small></header><div class="reviewer-inbox">'+group.items.map(function(item){var comment=item.comment,displayable=reviewHasDisplayablePassage(item.ch,comment),longComment=String(comment.text||'').length>320,source=reviewSourceLabel(comment),cardKey=item.ch.id+'-'+comment.id;return '<article class="reviewer-inbox-card review-level-'+esc(comment.level)+(comment.resolved?' resolved':'')+'"><div class="revision-card-meta"><span>R'+reviewOrdinal(item.ch,comment)+(source?' · '+esc(source):'')+'</span>'+(reviewNeedsPassage(item.ch,comment)?'<span class="revision-needs-passage">Needs passage</span>':'')+'</div><div class="reviewer-classification"><span class="reviewer-chip level review-level-'+esc(comment.level)+'">'+esc(reviewLevelLabel(comment.level))+'</span><span class="reviewer-chip">'+esc(reviewTopicLabel(comment.topic))+'</span></div><p class="revision-card-copy'+(longComment?' is-collapsed':'')+'" id="revision-card-copy-'+esc(cardKey)+'">'+esc(comment.text||'')+'</p>'+(longComment?'<button class="text-button revision-card-toggle" type="button" data-revision-toggle="'+esc(cardKey)+'" aria-expanded="false">Show full comment</button>':'')+'<button class="text-button revision-open-paper" type="button" data-open-review-paper="'+esc(item.ch.id)+'" data-open-review-comment="'+esc(comment.id)+'">'+(displayable?'Open passage':'Open comment')+' →</button></article>';}).join('')+'</div></section>';}).join('');
    box.innerHTML=filters+body+(filtered.length>shown.length?'<button class="soft-button revision-show-more" id="revisionShowMore" type="button">Show '+Math.min(12,filtered.length-shown.length)+' more · '+(filtered.length-shown.length)+' remaining</button>':'');
    box.querySelectorAll('[data-revision-filter]').forEach(function(button){button.onclick=function(){reviewInboxFilter=button.dataset.revisionFilter;reviewInboxLimit=12;renderReviewerInbox();};});
    var search=byId('revisionInboxSearch');search.oninput=function(){reviewInboxQuery=search.value;reviewInboxLimit=12;renderReviewerInbox();var next=byId('revisionInboxSearch');if(next){next.focus();next.setSelectionRange(next.value.length,next.value.length);}};
    var more=byId('revisionShowMore');if(more)more.onclick=function(){reviewInboxLimit+=12;renderReviewerInbox();};
    box.querySelectorAll('[data-revision-toggle]').forEach(function(button){button.onclick=function(){var copy=byId('revision-card-copy-'+button.dataset.revisionToggle),expanded=button.getAttribute('aria-expanded')==='true';if(!copy)return;copy.classList.toggle('is-collapsed',expanded);button.setAttribute('aria-expanded',String(!expanded));button.textContent=expanded?'Show full comment':'Show less';};});
    box.querySelectorAll('[data-open-review-paper]').forEach(function(button){button.onclick=function(){var chapterId=button.dataset.openReviewPaper,commentId=button.dataset.openReviewComment;openReader(chapterId).then(function(opened){if(!opened||currentId!==chapterId)return;var ch=find(chapterId),comment=reviewerById(ch,commentId);showReviewerComment(ch,commentId);if(comment&&reviewHasDisplayablePassage(ch,comment))return focusReviewerPassage(ch,commentId);});};});
  }
  ...
  function showReviewCard(){
    ...
    byId('reviewOpen').onclick=function(){openReader(item.ch.id).then(function(opened){if(opened&&currentId===item.ch.id&&item.page)gotoPdfPage(item.page);});};
  ...
  byId('reviewLayerFileBtn').onclick=function(){byId('reviewLayerFile').click();};
  byId('reviewLayerFile').onchange=async function(){
    var file=this.files&&this.files[0],status=byId('reviewImportStatus'),button=byId('reviewLayerFileBtn');this.value='';if(!file)return;button.disabled=true;status.textContent='Opening the shared review layer…';
    try{var payload=await decodeSharedReviewFile(file);byId('reviewImportDialog').close();await openIncomingSharedReview(payload);}
    catch(e){status.textContent=e.message||'Phloem could not read that shared review file.';}
  ...
  /* A PDF's Title field is often a prepress job name rather than its human title
     (for example "OP-PLPH210546 1709..1723"). Those codes are short, digit-heavy,
     and either carry a page range or are composed almost entirely of capitals and
     production punctuation. Only distrust them when page one offers a real heading. */
  ...
  var DEFAULT_COMFORT={size:100,measure:780,leading:1.7,typeface:'book',airy:false,focus:false,guide:'yellow',guideOrientation:'row',pdfLayout:'scroll',guideScope:'page',guideSize:'m',guideDim:55,guideX:.72,guideY:.38,guideLock:false,tone:'cream',driftSpeed:4};
  ...
  /* The old "Vertical book" setting mixed page direction with a crop-heavy layout.
     Carry its explicit choices forward, but do not turn a column-shaped Guide into a
     book preference: Guide direction and page layout are independent concerns. */
  if(!savedComfort||!Object.prototype.hasOwnProperty.call(savedComfort,'pdfLayout')){
    comfort.pdfLayout=savedComfort&&savedComfort.pdfDirection==='vertical'?(savedComfort.verticalPages==='two'?'book':'page'):'scroll';
  }
  /* The retired crop layout stored its automatic fit as if it were the reader's chosen
     Scroll zoom. Only the last-open paper is known to have been laid out under the active
     legacy setting; preserve zoom choices on every other paper rather than guessing. */
  if(migratedLegacyPagedComfort){
    var legacyPagedPaperId='';try{legacyPagedPaperId=resolvedPaperId(localStorage.getItem(LAST_OPEN_KEY));}catch(e){}
    var legacyPagedPaper=state.chapters.find(function(ch){return ch.kind==='pdf'&&ch.id===legacyPagedPaperId;});
    if(legacyPagedPaper&&+legacyPagedPaper.zoom>0){legacyPagedPaper.zoom=0;legacyPagedPaper.zoomPreferenceV=PDF_ZOOM_PREFERENCE_VERSION;persist(false);}
  }
  ...
  /* `tone` was the old two-state Paper control. Migrate it once into the separate
     preference without coupling it to the interface theme. */
  if(!savedPaperAppearance){paperAppearance=comfort.tone==='white'?'original':'cream';try{localStorage.setItem(PAPER_APPEARANCE_KEY,paperAppearance);}catch(e){}}
  ...
  function showGuideDiscovery(chapterId){
    ...
    note.textContent='Focus one line—or a vertical column—with Guide.';
  ...
  function setPaperAppearance(value,announce){
    ...
    try{localStorage.setItem(PAPER_APPEARANCE_KEY,value);}catch(e){}
  ...
  function setPdfLayout(layout,restoreSpot){
    ...
    var position=restoreSpot===false?null:capturePdfReadingPosition(),layoutDoc=pdfDoc,layoutId=currentId,layoutEpoch=pdfOpenEpoch;
    if(layout!=='scroll'){columnZoomOperation++;columnZoomState='full';}
    comfort.pdfLayout=layout;pagedFitToken='';pagedManualZoom=false;guideOffset=null;applyComfort();
    if(position&&pdfDoc)requestAnimationFrame(function(){Promise.resolve(renderPdfPage()).then(function(){if(pdfDoc===layoutDoc&&currentId===layoutId&&pdfOpenEpoch===layoutEpoch)return placePdfReadingPosition(position,false,true);});});
  ...
  byId('comfortReset').onclick=function(){Object.assign(comfort,DEFAULT_COMFORT);paperAppearance='cream';try{localStorage.setItem(PAPER_APPEARANCE_KEY,paperAppearance);}catch(e){}if(driftSpeed)driftSpeed=DEFAULT_COMFORT.driftSpeed;applyComfort();showReaderToast('Reading settings reset · cream paper · guide off');};
  var comfortReturnFocus=null;
  function setComfortBarOpen(open,returnFocus,trigger){
    var bar=byId('comfortBar'),btn=byId('comfortBtn'),touchButton=byId('touchSettings');
  ...
  byId('documentPane').addEventListener('pointermove',function(e){
    ...
    if(bookCurlOwned||!comfort.focus||comfort.guideLock||e.pointerType==='touch')return;setReadingGuide(e.clientX,e.clientY,e.target);
  });
  /* A released page curl can synthesize a click over whatever the folded corner lands
     on. Stop that click in capture phase, before a PDF link/highlight sees it. */
  byId('documentPane').addEventListener('click',function(e){
    if(Date.now()<pagedSuppressClickUntil){e.preventDefault();e.stopImmediatePropagation();}
  },true);
  byId('documentPane').addEventListener('click',function(e){
    if(Date.now()<pagedSuppressClickUntil){e.preventDefault();e.stopImmediatePropagation();return;}
  ...
  /* Column reading is a named, dependable mode instead of a mystery zoom shortcut.
     The button says where it will go next, and Page/Book explicitly offer to enter
     Scroll first because a cropped column is incompatible with a full-page layout. */
  var columnZoomState='full',columnBandCache=Object.create(null),columnZoomOperation=0;
  function syncColumnZoomUi(){
    var button=byId('colZoomBtn');if(!button)return;
    if(pagedPdfFlow()){
      button.textContent='Column · Start';button.setAttribute('aria-label','Start column reading in Scroll view');button.setAttribute('aria-pressed','false');button.dataset.columnState='start';return;
    }
    var labels={full:'Column · Full width',left:'Column · Left',right:'Column · Right',text:'Column · Text'};
    button.textContent=labels[columnZoomState]||labels.full;button.dataset.columnState=columnZoomState;
    button.setAttribute('aria-label',columnZoomState==='full'?'Zoom to the first reading column':'Column reading: '+(columnZoomState==='text'?'text width':columnZoomState)+'; activate for the next view');
    button.setAttribute('aria-pressed',String(columnZoomState!=='full'));
  }
  async function columnBands(pageNo){
    var bandDoc=pdfDoc,bandId=currentId,bandEpoch=pdfOpenEpoch,key=bandId+'|'+pageNo;
    if(columnBandCache[key])return columnBandCache[key];
    var pg=await bandDoc.getPage(pageNo),content=await pg.getTextContent(),vp=pg.getViewport({scale:1});
    if(pdfDoc!==bandDoc||currentId!==bandId||pdfOpenEpoch!==bandEpoch)throw new Error('stale column request');
    ...
    var result={twoCol:left.length>=6&&right.length>=6,left:band(left),right:band(right),text:band(lines.filter(function(l){return (l.endX-l.x)>pageW*.12;}))};
    columnBandCache[key]=result;return result;
  }
  async function zoomToBand(fracs,pageNo,anchorX,anchorY,label,stateName,operation){
    if(!fracs||operation!==columnZoomOperation)return;
    var verticalAnchor=capturePdfReadingPosition();
    var z=Math.max(1.15,Math.min(1.73,.985/(fracs[1]-fracs[0])));
    await setUserZoom(z,anchorX,anchorY,0,0,undefined,true);
    if(operation!==columnZoomOperation)return;
    if(verticalAnchor)await placePdfReadingPosition(verticalAnchor,false,false);
    ...
    columnZoomState=stateName||'text';syncColumnZoomUi();savePdfReadingPosition(false);
    if(label)showReaderToast(label);
  }
  async function zoomToFullColumn(operation,label){
    var verticalAnchor=capturePdfReadingPosition();
    await setUserZoom(1,undefined,undefined,0,0,undefined,true);if(operation!==columnZoomOperation)return;
    if(verticalAnchor)await placePdfReadingPosition(verticalAnchor,false,false);
    if(operation!==columnZoomOperation)return;columnZoomState='full';syncColumnZoomUi();savePdfReadingPosition(false);if(label)showReaderToast(label);
  }
  async function columnZoomAt(e){
    if(!pdfDoc)return;
    var holder=e.target.closest?e.target.closest('.pdf-page'):null;
    if(!holder)return;
    var operation=++columnZoomOperation;
    if(columnZoomState!=='full'||currentZoom()>1.25){await zoomToFullColumn(operation,'Full width');return;}
    ...
    try{
      ...
      if(operation!==columnZoomOperation)return;
      if(bands.twoCol)await zoomToBand(fx<.5?bands.left:bands.right,pageNo,e.clientX,e.clientY,'Reading the '+(fx<.5?'left':'right')+' column · Column advances',fx<.5?'left':'right',operation);
      else await zoomToBand(bands.text,pageNo,e.clientX,e.clientY,'Reading at text width · Column returns to full','text',operation);
    }catch(err){}
  }
  async function cycleColumnZoom(){
    ...
    var operation=++columnZoomOperation;
    if(pagedPdfFlow()){
      var saved=capturePdfReadingPosition();setPdfLayout('scroll',false);
      await zoomChain;await renderPdfPage();if(operation!==columnZoomOperation)return;
      if(saved)await placePdfReadingPosition(saved,false,true);
      columnZoomState='full';syncColumnZoomUi();showReaderToast('Scroll view · starting column reading');
    }
    if(currentZoom()<=1.05&&columnZoomState!=='full')columnZoomState='full';
    try{
      var pageNo=currentPage,bands=await columnBands(pageNo);
      if(operation!==columnZoomOperation)return;
      if(bands.twoCol){
        if(columnZoomState==='full'||columnZoomState==='text')await zoomToBand(bands.left,pageNo,undefined,undefined,'Left column · Column advances right','left',operation);
        else if(columnZoomState==='left')await zoomToBand(bands.right,pageNo,undefined,undefined,'Right column · Column returns to full','right',operation);
        else await zoomToFullColumn(operation,'Full width');
      }else if(columnZoomState==='full')await zoomToBand(bands.text,pageNo,undefined,undefined,'Text width · Column returns to full','text',operation);
      else await zoomToFullColumn(operation,'Full width');
    }catch(err){}
  }
  ...
  try{tabletNotesPinned=localStorage.getItem(TOUCH_NOTES_PIN_KEY)==='1';}catch(e){}
  function touchTabletUi(){return innerWidth>720&&(matchMedia('(any-pointer: coarse)').matches||matchMedia('(pointer: coarse)').matches||navigator.maxTouchPoints>1);}
  function tabletSidePanelAvailable(){return touchTabletUi()&&innerWidth>=980&&matchMedia('(orientation: landscape)').matches;}
  ...
  function syncTabletReaderUi(){
    ...
    var tablet=touchTabletUi(),sidePanel=tablet&&tabletNotesPinned&&tabletSidePanelAvailable()&&!zenOn,overlay=tablet&&!sidePanel;
    document.body.classList.toggle('touch-tablet-reader',tablet);document.body.classList.toggle('tablet-notes-overlay',overlay);document.body.classList.toggle('tablet-notes-pinned',sidePanel);
    document.body.setAttribute('data-tablet-dock-side',touchDockSide);byId('touchDock').dataset.side=touchDockSide;
    var move=byId('touchDockMove');move.innerHTML='<span aria-hidden="true">⇆</span> Move dock '+(touchDockSide==='right'?'left':'right');move.setAttribute('aria-label','Move reading controls to '+(touchDockSide==='right'?'left':'right')+' side');
    var pin=byId('touchPinNotes'),canPin=tabletSidePanelAvailable();pin.classList.toggle('hidden',!canPin);pin.setAttribute('aria-pressed',String(sidePanel));pin.innerHTML='<span aria-hidden="true">▥</span> '+(sidePanel?'Use temporary notes':'Keep notes beside page');
    if(sidePanel&&wasOverlay&&sheetWasOpen){notebook.classList.remove('sheet-open');byId('sheetScrim').classList.add('hidden');notebookCollapsed=false;}
    else if(overlay&&wasPinned&&!notebookCollapsed){notebook.classList.add('sheet-open');byId('sheetScrim').classList.remove('hidden');}
    var temporary=temporaryNotebookMode();notebook.setAttribute('role',temporary?'dialog':'complementary');
    if(temporary)notebook.setAttribute('aria-modal','true');else notebook.removeAttribute('aria-modal');
    if(temporary)byId('readerLayout').classList.remove('notebook-collapsed');
    syncTouchDockStates();
  }
  ...
  function setNotebookWidth(value,save){
    var layout=byId('readerLayout'),resizer=byId('notebookResizer');if(temporaryNotebookMode()){layout.style.removeProperty('--notebook-width');return;}
  ...
  byId('notebookResizer').onpointerdown=function(e){
    if(temporaryNotebookMode())return;notebookDrag=true;try{this.setPointerCapture(e.pointerId);}catch(err){}document.body.classList.add('resizing-notebook');e.preventDefault();
  };
  ...
  function syncMobileSheetButtons(){
    var open=byId('notebook').classList.contains('sheet-open'),aiOpen=!byId('aiPanel').classList.contains('hidden'),ch=find(currentId),hasReviews=!!(ch&&(ch.reviewComments||[]).length),primaryId=hasReviews?'reviewsPanel':'notesPanel',primaryOpen=!byId(primaryId).classList.contains('hidden'),primary=byId('mNotes');
    byId('mAsk').setAttribute('aria-expanded',String(open&&aiOpen));
    primary.textContent=hasReviews?'R Reviews':'✎ Notes';primary.setAttribute('aria-label',hasReviews?'Open reviewer comments':'Open notes');primary.setAttribute('aria-expanded',String(open&&primaryOpen));
    syncTouchDockStates();
  }
  ...
  function toggleMobileTab(id){var same=!byId(id).classList.contains('hidden');if(same&&byId('notebook').classList.contains('sheet-open'))toggleSheet(false);else{switchTab(id);toggleSheet(true);
    ...
    if(id==='notesPanel'&&temporaryNotebookMode())requestAnimationFrame(function(){try{byId('pageNote').scrollIntoView({block:'center'});}catch(e){}});
  }}
  byId('mAsk').onclick=function(){toggleMobileTab('aiPanel');};
  byId('mNotes').onclick=function(){var ch=find(currentId);toggleMobileTab(ch&&(ch.reviewComments||[]).length?'reviewsPanel':'notesPanel');};
  byId('sheetScrim').onclick=function(){toggleSheet(false);};
  byId('sheetClose').onclick=function(){toggleSheet(false);};
  function revealRevisionDesk(){reviewPageMode='reviewers';switchTab('reviewsPanel');if(temporaryNotebookMode())toggleSheet(true);else setNotebookCollapsed(false,true);}
  ...
  function toggleTouchPanel(id){
    if(temporaryNotebookMode())toggleMobileTab(id);
      ...
      if(same&&open)setNotebookCollapsed(true,true);else{switchTab(id);setNotebookCollapsed(false,true);}
    }
    syncTouchDockStates();
  }
  ...
  byId('touchDockMove').onclick=function(){touchDockSide=touchDockSide==='right'?'left':'right';try{localStorage.setItem(TOUCH_DOCK_SIDE_KEY,touchDockSide);}catch(e){}syncTabletReaderUi();closeTouchDockMore(false);showReaderToast('Reading controls moved '+touchDockSide);};
  byId('touchPinNotes').onclick=function(){
    var pinning=!tabletNotesPinned;
    if(pinning)toggleSheet(false);
    tabletNotesPinned=pinning;try{localStorage.setItem(TOUCH_NOTES_PIN_KEY,pinning?'1':'0');}catch(e){}
    syncTabletReaderUi();switchTab('notesPanel');
    if(pinning&&tabletSidePanelAvailable())setNotebookCollapsed(false,true);else toggleSheet(true);
    closeTouchDockMore(false);showReaderToast(pinning?'Notes will stay beside wide pages':'Notes will float over the page');
  };
  document.addEventListener('pointerdown',function(e){
  ...
  function updateProgress(){
    ...
    if(ch){
      if(readerMode==='pdf'&&pdfDoc&&pdfDoc.numPages){var pdfPane=byId('documentPane'),pdfSpan=pdfPane.scrollHeight-pdfPane.clientHeight,visibleThrough=pagedPdfFlow()?pagedPageNos(currentPage).slice(-1)[0]:currentPage;pct=pagedPdfFlow()?visibleThrough/pdfDoc.numPages*100:(pdfSpan>4?pdfPane.scrollTop/pdfSpan*100:currentPage/pdfDoc.numPages*100);}
      else{
  ...
  var findMatches=[], findIndex=-1, findTimer=null, findPaintToken=0;
  function clearFindTargets(root){
    root=root||document;
    root.querySelectorAll('.find-span').forEach(function(span){span.classList.remove('find-span','find-current-span','find-piece-host');});
    root.querySelectorAll('.find-flash').forEach(function(section){section.classList.remove('find-flash');});
    root.querySelectorAll('span.find-target').forEach(function(target){
      var parent=target.parentNode;if(!parent)return;
      parent.replaceChild(document.createTextNode(target.textContent||''),target);parent.normalize();
    });
  }
  function findOccurrenceRanges(value,query){
    value=findCanonicalValue(value);query=findCanonicalValue(query);if(!value||!query)return[];
    var ranges=[],at=0;while((at=value.indexOf(query,at))>=0){ranges.push({start:at,end:at+query.length});at+=Math.max(1,query.length);}return ranges;
  }
  function findCanonicalValue(value){
    value=String(value||'');try{value=value.normalize('NFKD').replace(/[\u0300-\u036f]/g,'');}catch(e){}
    try{return value.toLowerCase().replace(/[^\p{L}\p{N}]+/gu,' ').trim().replace(/\s+/g,' ');}catch(e){return value.toLowerCase().replace(/[^a-z0-9]+/g,' ').trim().replace(/\s+/g,' ');}
  }
  function findCanonicalTextMap(value){
    value=String(value||'');var text='',points=[];
    for(var offset=0;offset<value.length;){
      var code=value.codePointAt(offset),unit=String.fromCodePoint(code),size=unit.length,folded=unit;
      try{folded=unit.normalize('NFKD').replace(/[\u0300-\u036f]/g,'');}catch(e){}
      var word='';for(var at=0;at<folded.length;){var foldedCode=folded.codePointAt(at),foldedUnit=String.fromCodePoint(foldedCode);try{if(/[\p{L}\p{N}]/u.test(foldedUnit))word+=foldedUnit.toLowerCase();}catch(e){if(/[a-z0-9]/i.test(foldedUnit))word+=foldedUnit.toLowerCase();}at+=foldedUnit.length;}
      if(word){for(var index=0;index<word.length;index++){text+=word[index];points.push({start:offset,end:offset+size});}}
      else if(folded&&text&&text[text.length-1]!==' '){text+=' ';points.push({start:offset,end:offset+size});}
      offset+=size;
    }
    while(text.endsWith(' ')){text=text.slice(0,-1);points.pop();}
    return{text:text,points:points};
  }
  function findPdfSpanSeparator(state,span,value){
    var rect=span.getBoundingClientRect(),height=Math.max(0,rect.height),center=(rect.top+rect.bottom)/2,needs=false;
    if(!state.previous){state.previous=span;state.center=center;state.height=height;state.end=rect.right;return false;}
    if(!rect.width||!height||!state.height)needs=pdfReviewSpanNeedsSpace(state.previous,span);
    else{
      var newLine=Math.abs(center-state.center)>Math.max(2,height*.55,state.height*.55),gap=rect.left-state.end;
      needs=newLine||(gap>height*.12&&!/^[,.;:!?%)\]]/.test(value));
      if(newLine){state.center=center;state.height=height;state.end=rect.right;}
      else{state.height=Math.max(state.height,height);state.end=Math.max(state.end,rect.right);}
    }
  ...
  function findTextMap(root,isPdf){
    ...
    if(isPdf){
      ...
      root.querySelectorAll('span').forEach(function(span){
        ...
        var walker=document.createTreeWalker(span,NodeFilter.SHOW_TEXT),node;while((node=walker.nextNode()))appendNode(node,span);
      });
    }else{
      var walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT),node;while((node=walker.nextNode()))appendNode(node,null);
    }
  ...
  function findTextGroups(root,query,isPdf){
    ...
    return findOccurrenceRanges(canonical.text,query).map(function(range){
      ...
      return map.entries.filter(function(entry){return entry.end>rawStart&&entry.start<rawEnd;}).map(function(entry){
        return{node:entry.node,start:Math.max(0,rawStart-entry.start),end:Math.min(entry.end-entry.start,rawEnd-entry.start),span:entry.span};
      }).filter(function(piece){return piece.end>piece.start;});
    }).filter(function(group){return group.length;});
  }
  function wrapFindGroups(groups,activeIndex){
    var pieces=[];
    groups.forEach(function(group,groupIndex){group.forEach(function(piece){piece.group=groupIndex;pieces.push(piece);});});
    pieces.sort(function(left,right){if(left.node===right.node)return right.start-left.start;return 0;});
    var active=[];
    pieces.forEach(function(piece){
      var node=piece.node;if(!node.parentNode||piece.end>node.nodeValue.length)return;
      if(piece.end<node.nodeValue.length)node.splitText(piece.end);
      var matchNode=piece.start?node.splitText(piece.start):node,target=document.createElement('span');
      target.className='find-target'+(piece.group===activeIndex?' find-current':'');
      matchNode.parentNode.insertBefore(target,matchNode);target.appendChild(matchNode);
      if(piece.span){piece.span.classList.add('find-span','find-piece-host');target.classList.add('find-span');if(piece.group===activeIndex)target.classList.add('find-current-span');}
      if(piece.group===activeIndex)active.push(target);
    });
    return active;
  }
  function paintFindTarget(root,query,isPdf,occurrence){
    if(!root)return{groups:[],active:[]};clearFindTargets(root);
    var groups=findTextGroups(root,query,isPdf),at=Math.max(0,Math.min(groups.length-1,+occurrence||0));
    return{groups:groups,active:wrapFindGroups(groups,at)};
  }
  function queueFindRefresh(){
    if(byId('findBar').classList.contains('hidden'))return;
    findPaintToken++;clearFindTargets();findMatches=[];findIndex=-1;clearTimeout(findTimer);
    var query=byId('findInput').value.trim();byId('findCount').textContent=query.length>=2?'Searching…':query?'Type a bit more':'';
  ...
  function toggleFindBar(open){
    ...
    if(willOpen){byId('findInput').focus({preventScroll:true});byId('findInput').select();if(byId('findInput').value.trim().length>=2)runFind();}
    else{findPaintToken++;clearTimeout(findTimer);clearFindTargets();findMatches=[];findIndex=-1;byId('findCount').textContent='';}
  }
  ...
  function runFind(){
    ...
    if(ch.kind==='pdf'&&readerMode==='pdf'){
      ...
      paras(source).forEach(function(p,i){findOccurrenceRanges(p,q).forEach(function(range,occurrence){findMatches.push({para:i,occurrence:occurrence});});});
    }
  ...
  async function gotoFindMatch(index){
    ...
    var m=findMatches[findIndex],where=m.page!==undefined?'p. '+m.page:'¶ '+(m.para+1);
    byId('findCount').textContent=where+' · '+(findIndex+1)+' / '+findMatches.length;
    if(m.page!==undefined){await gotoPdfPage(m.page,'auto');if(token!==findPaintToken)return;await flashPdfFind(m.page,byId('findInput').value.trim(),m.occurrence,token);return;}
    var list=paraSections(),section=list[m.para];if(!section)return;
    var original=section.querySelector('.original'),result=paintFindTarget(original,byId('findInput').value.trim(),false,m.occurrence),hiddenEquation=section.classList.contains('eq-figured');
    if(!result.active.length||hiddenEquation)section.classList.add('find-flash');
    (hiddenEquation?(section.querySelector('.reader-fig')||section):(result.active[0]||section)).scrollIntoView({block:'center',behavior:'smooth'});
  }
  /* PDF.js often breaks one phrase into several positioned spans. Build a continuous
     text map for the page, then ink every piece of the chosen occurrence. */
  function flashPdfFind(page,q,occurrence,token){
    if(!q)return Promise.resolve(false);
    return new Promise(function(resolve){var tries=0;
      (function attempt(){
      if(token!==findPaintToken){resolve(false);return;}
      var view=pdfViews[page-1];
      if(!view||!view.rendered){if(tries++<24){setTimeout(attempt,180);return;}resolve(false);return;}
      var result=paintFindTarget(view.text,q,true,occurrence);
      if(!result.active.length){pdfReviewSpanMatch(view,q,occurrence).forEach(function(span){span.classList.add('find-span','find-current-span');});}
      if(result.active[0]&&!pagedPdfFlow())result.active[0].scrollIntoView({block:'center',inline:'center',behavior:'smooth'});
      resolve(!!result.active.length);
    })();});
  }
  byId('findInput').oninput=function(){
    findPaintToken++;clearFindTargets();findMatches=[];findIndex=-1;clearTimeout(findTimer);
    var query=this.value.trim();byId('findCount').textContent=query.length>=2?'Searching…':query?'Type a bit more':'';
    if(query.length>=2)findTimer=setTimeout(function(){findTimer=null;runFind();},260);
  };
  byId('findInput').onkeydown=function(e){
    if(e.key==='Enter'){e.preventDefault();if(findTimer){clearTimeout(findTimer);findTimer=null;runFind();return;}findMatches.length?gotoFindMatch(findIndex+(e.shiftKey?-1:1)):runFind();}
  ...
  async function gotoPdfPage(page,behavior){
    ...
    return pdfDoc===navDoc&&currentId===navId&&pdfOpenEpoch===navEpoch;
  }
  ...
  async function tocGo(item){
    var tocDoc=pdfDoc,tocId=currentId,tocEpoch=pdfOpenEpoch;
    try{
      if(item.phloemPage){byId('tocDialog').close();gotoPdfPage(item.phloemPage);return;}
      var dest=item.dest;
      if(typeof dest==='string')dest=await tocDoc.getDestination(dest);
      if(pdfDoc!==tocDoc||currentId!==tocId||pdfOpenEpoch!==tocEpoch)return;
      if(!Array.isArray(dest)||!dest.length)throw new Error('no destination');
      var first=dest[0];
      var index=first&&typeof first==='object'?await tocDoc.getPageIndex(first):+first;
      if(pdfDoc!==tocDoc||currentId!==tocId||pdfOpenEpoch!==tocEpoch)return;
      byId('tocDialog').close();
      gotoPdfPage(index+1);
    }catch(e){if(pdfDoc===tocDoc&&currentId===tocId&&pdfOpenEpoch===tocEpoch)showReaderToast('Could not open that chapter');}
  }
  ...
  function restorePdfZoom(ch){
    var cursor=ch&&normalizedPdfPosition(ch.pdfPosition),saved=ch&&ch.kind==='pdf'?(cursor&&Number.isFinite(cursor.zoom)?cursor.zoom:+ch.zoom):0;
  ...
  async function openReader(id,preparedDoc){
    ...
    currentId=id;reviewFocusId='';reviewFocusPage=0;reviewFocusAnchorIndex=-1;reviewPassageGroupIds=[];reviewLinkTargetId='';reviewLinkUndo=null; currentPage=ch.pdfPosition&&ch.pdfPosition.page||ch.readPage||1; pdfOutline=null; pdfViews=[]; pdfBuildKey='';lastStablePdfPosition=null;lastStablePdfPositionId='';pagedFits=true;pagedMinLeft=0;pagedMaxLeft=0;pagedManualZoom=false;pagedFitToken='';columnBandCache=Object.create(null);columnZoomOperation++;columnZoomState=ch.pdfPosition&&['left','right','text'].indexOf(ch.pdfPosition.zoomMode)>=0?ch.pdfPosition.zoomMode:'full';ch.lastOpenedAt=now();persist(false); try{localStorage.setItem(LAST_OPEN_KEY,id);}catch(e){} restorePdfZoom(ch); setHighlightMode(false); clearPendingSelection(); hideHighlightCard(); highlightHistory=[]; highlightFuture=[];syncTouchDockStates(); lastAskSelection=null; toggleFindBar(false); linkReturnSpot=null; byId('linkReturn').classList.add('hidden');
    ...
    if(ch.kind==='pdf'){
      try{
        ...
        if(readerMode==='text'&&(!ch.readerText||ch.readerV!==READER_V)){await ensureReaderData(pdfDoc,ch);if(openEpoch!==pdfOpenEpoch||currentId!==id)return false;updateReaderMode();}
        else if(readerMode==='pdf'){
          await renderPdfPage();if(openEpoch!==pdfOpenEpoch||currentId!==id)return false;
          var restored=ch.pdfPosition?await placePdfReadingPosition(ch.pdfPosition,true,true):false;
          if(openEpoch!==pdfOpenEpoch||currentId!==id)return false;scheduleGuideDiscovery(ch.id);
          if(!restored&&currentPage>1)showReaderToast('Back on page '+currentPage);
        }
      }catch(e){if(openEpoch!==pdfOpenEpoch||currentId!==id)return false;showPdfProblem(ch,e);}
    } else renderText(ch);
    if(openEpoch!==pdfOpenEpoch||currentId!==id)return false;
    restoreReaderPosition(ch,true); loadPageNote(); updateProgress();byId('pdfFrame').dataset.positionReady='true';
    return true;
  }
  ...
  function updateReaderMode(){
    ...
    if(!pdf&&ch) renderText(ch);applyPdfFlowMode();applyFocus();loadPageNote();updateProgress();queueFindRefresh();
  }
  byId('reflowBtn').onclick=async function(){
    var ch=find(currentId);if(!ch||ch.kind!=='pdf')return;clearPendingSelection();if(readerMode==='pdf')savePdfReadingPosition(false);
    readerMode=readerMode==='pdf'?'text':'pdf';updateReaderMode();
    if(readerMode==='pdf'&&pdfDoc){await renderPdfPage();if(ch.pdfPosition)await placePdfReadingPosition(ch.pdfPosition,false,true);else scrollToPdfPage(currentPage,'auto');return;}
  ...
  function applyPaperAppearance(){
    var frame=byId('pdfFrame'),doc=byId('textDocument'),cream=paperAppearance==='cream',inverted=paperAppearance==='inverted';
    if(frame){frame.classList.toggle('cream',cream);frame.classList.toggle('dark-paper',inverted);frame.dataset.paperAppearance=paperAppearance;}
  ...
  var pagedTurning=false,pagedTurnQueue=[],pagedTurnEpoch=0,pagedTurnPromise=Promise.resolve(),pagedFlowSignature='',pagedFits=true,pagedMinLeft=0,pagedMaxLeft=0,pagedManualZoom=false,pagedFitToken='',pagedFitEpoch=0,pagedFitRunId=0,pagedReadyFitEpoch=-1,pagedReadyFitRunId=-1,pagedReadyBuildId=-1,pagedSuppressClickUntil=0;
  /* The direct Book curl lives with the gesture handlers below, but layout/open/zoom
     code must be able to tear it down synchronously. The no-op is replaced once those
     handlers are installed. */
  var bookCurlOwned=false,automaticBookCurlRunning=false,cancelActiveBookCurl=function(){return false;},runAutomaticBookCurl=function(){return Promise.resolve(null);};
  var scrollPdfFit=true,scrollPdfZoom=1;
  function pagedPdfFlow(){return readerMode==='pdf'&&comfort.pdfLayout!=='scroll';}
  /* Touch must never fold holders left over from the previous mode, zoom, or rotation.
     Publish readiness only after the current build's final layout frame has landed. */
  function setPagedReady(ready,fitEpoch,fitRunId,buildId){
    var frame=byId('pdfFrame');
    if(ready&&pagedPdfFlow()){
      pagedReadyFitEpoch=fitEpoch;pagedReadyFitRunId=fitRunId;pagedReadyBuildId=buildId;
      if(frame){frame.dataset.pagedReady='true';frame.removeAttribute('aria-busy');}
    }else{
      pagedReadyFitEpoch=-1;pagedReadyFitRunId=-1;pagedReadyBuildId=-1;
      if(frame){
        if(pagedPdfFlow()){frame.dataset.pagedReady='false';frame.setAttribute('aria-busy','true');}
        else{delete frame.dataset.pagedReady;frame.removeAttribute('aria-busy');}
      }
    }
  }
  function invalidatePagedFit(){pagedFitEpoch++;pagedFitRunId++;setPagedReady(false);return pagedFitEpoch;}
  function pagedCurlGeometryReady(){
    var frame=byId('pdfFrame');
    return !!frame&&frame.dataset.pagedReady==='true'&&pagedReadyFitEpoch===pagedFitEpoch&&pagedReadyFitRunId===pagedFitRunId&&pagedReadyBuildId===pdfBuildId&&pdfBuildKey===currentBuildKey()&&frame.classList.contains('book-spread')===bookSpread()&&Math.abs(layoutZoom-currentZoom())<.005;
  }
  /* A requested Book becomes a single complete page when the actual reading pane is too
     small. The preference stays Book, so tucking the notebook or rotating restores the
     spread without another trip to Settings. */
  function bookSpread(){var pane=byId('documentPane'),minimumWidth=touchTabletUi()?900:720;return pagedPdfFlow()&&comfort.pdfLayout==='book'&&pane&&pane.clientWidth>=minimumWidth&&pane.clientHeight>=480;}
  function bookSpreadStart(pageNo){pageNo=Math.max(1,Math.min(pdfDoc?pdfDoc.numPages:pageNo,+pageNo||1));return pageNo<=1?1:pageNo-(pageNo%2);}
  function pagedPageNos(pageNo){
    if(!bookSpread())return [Math.max(1,Math.min(pdfDoc?pdfDoc.numPages:pageNo,+pageNo||1))];
    var start=bookSpreadStart(pageNo),pages=[start];if(start>1&&pdfDoc&&start<pdfDoc.numPages)pages.push(start+1);return pages;
  }
  function samePages(a,b){return a.length===b.length&&a.every(function(n,i){return n===b[i];});}
  function pagedNextTarget(){
    if(!pdfDoc)return currentPage;
    if(!bookSpread())return Math.min(pdfDoc.numPages,currentPage+1);
    var start=bookSpreadStart(currentPage);return Math.min(pdfDoc.numPages,start===1?2:start+2);
  }
  function pagedPreviousTarget(){
    if(!bookSpread())return Math.max(1,currentPage-1);
    var start=bookSpreadStart(currentPage);return start<=2?1:start-2;
  }
  ...
  function syncPagedPages(){
    if(!pdfViews)return;
    var on=pagedPdfFlow(),spread=bookSpread(),pages=on?pagedPageNos(currentPage):[],start=pages[0]||currentPage;
    var cover=spread&&start===1,lastBlank=spread&&start>1&&pages.length===1,frame=byId('pdfFrame'),pane=byId('documentPane'),blank=byId('bookBlankLeaf');
    frame.classList.toggle('paged-pdf-flow',on);pane.classList.toggle('paged-pdf-flow',on);
    frame.classList.toggle('book-spread',spread);pane.classList.toggle('book-spread',spread);
    frame.classList.toggle('book-cover',cover);frame.classList.toggle('book-last-blank',lastBlank);
    if(blank){blank.className='book-blank-leaf'+(cover?' book-placeholder-left':lastBlank?' book-blank-right':'');blank.setAttribute('aria-hidden','true');}
    pdfViews.forEach(function(view,index){
      var pageNo=index+1,active=on&&pages.indexOf(pageNo)>=0,left=spread&&!cover&&pageNo===pages[0],right=spread&&(cover?pageNo===1:pageNo===pages[1]);
      view.holder.classList.toggle('book-active',active);
      view.holder.classList.toggle('book-current',active&&pageNo===currentPage);
      view.holder.classList.toggle('book-spread-left',!!left);
      view.holder.classList.toggle('book-spread-right',!!right);
      view.holder.classList.toggle('book-single',active&&!spread);
      if(!on){resetFullBookView(view);view.holder.classList.remove('book-turning','book-flip-next','book-flip-prev','book-flip-single','book-flip-cover','book-stay-left','book-stay-right');}
    });
  }
  function layoutPagedPages(pageNos,spread,center,fitEpoch,fitRunId){
    var layoutBuildId=pdfBuildId;
    if((fitEpoch!==undefined&&fitEpoch!==pagedFitEpoch)||(fitRunId!==undefined&&fitRunId!==pagedFitRunId)||!pagedPdfFlow()||pdfBuildKey!==currentBuildKey()||bookSpread()!==spread||!samePages(pageNos,pagedPageNos(currentPage)))return Promise.resolve(false);
    var views=pageNos.map(function(n){return pdfViews[n-1];});views.forEach(resetFullBookView);syncPagedPages();
    var blank=byId('bookBlankLeaf'),reference=views[0];
    if(blank&&reference&&(blank.classList.contains('book-placeholder-left')||blank.classList.contains('book-blank-right'))){blank.style.width=reference.pageWidth+'px';blank.style.height=reference.pageHeight+'px';}
    var pane=byId('documentPane'),keptLeft=pane.scrollLeft,keptTop=pane.scrollTop;return new Promise(function(resolve){requestAnimationFrame(function(){
      if((fitEpoch!==undefined&&fitEpoch!==pagedFitEpoch)||(fitRunId!==undefined&&fitRunId!==pagedFitRunId)||layoutBuildId!==pdfBuildId||!pagedPdfFlow()||pdfBuildKey!==currentBuildKey()||bookSpread()!==spread||!samePages(pageNos,pagedPageNos(currentPage))){resolve(false);return;}
      pagedMinLeft=0;pagedMaxLeft=Math.max(0,pane.scrollWidth-pane.clientWidth);var maxTop=Math.max(0,pane.scrollHeight-pane.clientHeight);
      pagedFits=pagedMaxLeft<=2&&maxTop<=2;
      pane.scrollTo({left:center===false?Math.max(0,Math.min(pagedMaxLeft,keptLeft)):pagedMaxLeft/2,top:center===false?Math.max(0,Math.min(maxTop,keptTop)):maxTop/2,behavior:'auto'});
      setPagedReady(true,fitEpoch,fitRunId,layoutBuildId);if(comfort.focus)requestAnimationFrame(placeGuide);resolve(true);
    });});
  }
  /* Page and Book fit the authored PDF boxes themselves. Nothing samples, trims, masks or
     translates page content, so light illustrations and generous margins remain intact. */
  async function fitPagedPages(pageNo,force,retryCount,fitEpoch,fitRunId){
    if(fitEpoch===undefined)fitEpoch=pagedFitEpoch;
    if(fitEpoch!==pagedFitEpoch||!pdfDoc||!pagedPdfFlow()||!pdfViews.length)return false;
    if(fitRunId===undefined){fitRunId=++pagedFitRunId;setPagedReady(false);}
    if(fitRunId!==pagedFitRunId)return false;
    /* A resize can overtake a queued fit. Rebuild the page-width baseline first; fitting
       stale phone-sized holders inside a new desktop pane would briefly enlarge them. */
    if(pdfBuildKey!==currentBuildKey()){await renderPdfPage();if(fitEpoch!==pagedFitEpoch||fitRunId!==pagedFitRunId||!pdfDoc||!pagedPdfFlow()||pdfBuildKey!==currentBuildKey())return false;return fitPagedPages(pageNo,force,retryCount,fitEpoch,fitRunId);}
    var spread=bookSpread(),pageNos=pagedPageNos(pageNo);await Promise.all(pageNos.map(renderPdfPageAt));
    if(fitEpoch!==pagedFitEpoch||fitRunId!==pagedFitRunId||!pagedPdfFlow()||bookSpread()!==spread||!samePages(pageNos,pagedPageNos(currentPage)))return false;
    var views=pageNos.map(function(n){return pdfViews[n-1];});
    if(views.some(function(view){return !view||!view.rendered;})){
      retryCount=+retryCount||0;
      if(retryCount>=3||views.some(function(view){return view&&view.renderFailures>=2;})){showReaderToast('This PDF page could not be rendered');return false;}
      return new Promise(function(resolve){setTimeout(function(){if(fitEpoch!==pagedFitEpoch||fitRunId!==pagedFitRunId){resolve(false);return;}Promise.resolve(fitPagedPages(currentPage,force,retryCount+1,fitEpoch,fitRunId)).then(resolve);},90);});
    }
    views.forEach(resetFullBookView);
    ...
    if(shouldZoom&&Math.abs(desired-nowZoom)>.002){
      ...
      return setZoom(desired,undefined,undefined,0,0,undefined,false,true).then(function(valid){if(!valid||fitEpoch!==pagedFitEpoch||fitRunId!==pagedFitRunId)return false;return Promise.all(pageNos.map(renderPdfPageAt)).then(function(){if(fitEpoch!==pagedFitEpoch||fitRunId!==pagedFitRunId)return false;return layoutPagedPages(pageNos,spread,true,fitEpoch,fitRunId);});});
    }else{if(!pagedManualZoom)updatePageChrome();return layoutPagedPages(pageNos,spread,!pagedManualZoom,fitEpoch,fitRunId);}
  }
  function pagedSignature(){return pagedPdfFlow()?comfort.pdfLayout+'|'+(bookSpread()?'spread':'page'):'scroll';}
  function invalidatePagedTurns(){
    pagedTurnEpoch++;invalidatePagedFit();pagedTurnQueue=[];pagedTurning=false;
    cancelActiveBookCurl(true,'invalidated');
    var pane=byId('documentPane');if(pane)pane.classList.remove('turning-book-leaf');
    clearPagedTurnClasses(pdfViews||[]);
  }
  function applyPdfFlowMode(forceFit){
    var pane=byId('documentPane'),frame=byId('pdfFrame'),on=pagedPdfFlow(),was=frame.classList.contains('paged-pdf-flow');
    var nextSignature=pagedSignature();if(pagedFlowSignature&&nextSignature!==pagedFlowSignature)invalidatePagedTurns();pagedFlowSignature=nextSignature;
    ...
    if(pdfDoc&&pdfViews&&pdfViews.length){
      ...
      else if(was){pagedManualZoom=false;pagedFitToken='';pagedFits=true;setZoom(scrollPdfFit?1:scrollPdfZoom,undefined,undefined,0,0,function(){scrollToPdfPage(currentPage,'auto');},false);}
    }
    if(pdfDoc)updatePageChrome();
  }
  function savePagedPosition(){savePdfReadingPosition();}
  ...
  async function turnPagedPage(target,animate,epoch,turnStyle,turnOriginYRatio){
    ...
    if(samePages(fromPages,toPages)){currentPage=target;syncPagedPages();updatePageChrome();savePagedPosition();return fitPagedPages(target,false);}
    await Promise.all(toPages.map(renderPdfPageAt));
    ...
    currentPage=target;syncPagedPages();updatePageChrome();updateProgress();savePagedPosition();
    /* The compact card is only the continuation of touch's own swipe preview. If a
       physical arrow curl cannot start (for example during a resize/rebuild), swap the
       page cleanly instead of reviving the old stiff single-page animation. */
    var compactAnimation=animate!==false&&adjacent&&!reduceMotion&&turnStyle==='compact';
    if(compactAnimation){
      if(oldViews.length>1){
        var stay=forward?oldViews[0]:oldViews[oldViews.length-1],flip=forward?oldViews[oldViews.length-1]:oldViews[0];
        stay.holder.classList.add('book-turning',forward?'book-stay-left':'book-stay-right');
        flip.holder.classList.add('book-turning',forward?'book-flip-next':'book-flip-prev');
      }else if(oldViews[0])oldViews[0].holder.classList.add('book-turning',turnSpread&&fromStart===1&&forward?'book-flip-cover':'book-flip-single',forward?'book-flip-next':'book-flip-prev');
    }
    var duration=compactAnimation?310:0;
    await new Promise(function(resolve){setTimeout(resolve,duration);});
    if(!pagedTurnStillValid(epoch,turnDoc,turnId,turnLayout,turnSpread))return;
    clearPagedTurnClasses(oldViews);syncPagedPages();await fitPagedPages(currentPage,false);
  }
  function drainPagedTurns(){
    if(pagedTurning)return pagedTurnPromise;
    var epoch=pagedTurnEpoch;pagedTurning=true;
  ...
  async function buildPdfScroll(){
    ...
    frame.innerHTML='<div class="book-blank-leaf" id="bookBlankLeaf" aria-hidden="true"></div>';pdfViews=[];renderedPages=[];
    for(var i=1;i<=buildDoc.numPages;i++){
      ...
      holder.innerHTML='<div class="pdf-sheet"><canvas></canvas><div class="highlight-layer"></div><div class="text-layer"></div></div><span class="book-curl-grip" aria-hidden="true"></span>';
      frame.appendChild(holder);
      var sheet=holder.children[0];sheet.style.width=holder.style.width;sheet.style.height=holder.style.height;
      pdfViews.push({holder:holder,sheet:sheet,canvas:sheet.children[0],highlights:sheet.children[1],text:sheet.children[2],viewport:null,pageWidth:Math.ceil(natural.width*scale),pageHeight:Math.ceil(natural.height*scale),crop:null,rendered:false,rendering:false,renderPromise:null,renderFailures:0,buildId:buildId});
      if(carry[i]){
    ...
    syncPagedPages();
    applyPaperAppearance();pdfBuildKey=currentBuildKey();layoutZoom=builtZoom;
    ...
    updatePageChrome();
    if(pagedPdfFlow())requestAnimationFrame(function(){fitPagedPages(currentPage,!pagedFitToken);});
    if(comfort.focus)requestAnimationFrame(placeGuide);
  }
  async function renderPdfPageAt(n){
    var view=pdfViews[n-1];if(!view||view.rendered||!pdfDoc||view.buildId!==pdfBuildId)return;
    if(view.rendering)return view.renderPromise;
    var settleRender;view.renderPromise=new Promise(function(resolve){settleRender=resolve;});
    var renderDoc=pdfDoc,renderId=currentId,renderBuildId=view.buildId;view.rendering=true;
    try{
      var page=await renderDoc.getPage(n),natural=page.getViewport({scale:1}),scale=pageScale(natural);
    ...
    finally{view.rendering=false;view.renderPromise=null;settleRender();}
  }
  ...
  function updatePageChrome(){
    ...
    var pageLabel=(lastVisible>firstVisible?firstVisible+'–'+lastVisible:currentPage)+' / '+pdfDoc.numPages;
    byId('pageNumber').textContent=pageLabel;byId('mPageLabel').textContent=pageLabel;
    var atStart=paged?firstVisible<=1:currentPage<=1,atEnd=paged?lastVisible>=pdfDoc.numPages:currentPage>=pdfDoc.numPages;
    byId('prevPage').disabled=atStart;byId('nextPage').disabled=atEnd;byId('mPrev').disabled=atStart;byId('mNext').disabled=atEnd;
  ...
  var restoringPdfPosition=false,pdfPositionRestoreSerial=0,lastStablePdfPosition=null,lastStablePdfPositionId='';
  function cancelPdfPositionRestore(){pdfPositionRestoreSerial++;restoringPdfPosition=false;}
  function pdfLayoutFrames(){return new Promise(function(resolve){requestAnimationFrame(function(){requestAnimationFrame(resolve);});});}
  function capturePdfReadingPosition(){
    if(readerMode!=='pdf'||!pdfDoc||!pdfViews.length)return null;
    var pane=byId('documentPane'),paneRect=pane.getBoundingClientRect(),paged=pagedPdfFlow(),screenX=.5,screenY=paged?.5:.4;
    var targetX=paneRect.left+pane.clientWidth*screenX,targetY=paneRect.top+pane.clientHeight*screenY,pageNo=currentPage;
    if(!paged)pageNo=pdfPageIndexAtY(pane.scrollTop+pane.clientHeight*screenY)+1;
    pageNo=Math.max(1,Math.min(pdfDoc.numPages,pageNo));var view=pdfViews[pageNo-1],rect=view&&view.holder.getBoundingClientRect();
    /* In Book, currentPage is the leaf the reader last touched. Sampling the visual
       midpoint instead would always break a seam tie toward the left leaf when changing
  ...
  function rememberStablePdfPosition(position){
    position=normalizedPdfPosition(position);if(!position)return null;lastStablePdfPosition=position;lastStablePdfPositionId=currentId;return position;
  }
  function stablePdfPositionForRebuild(){
    /* During resize, the pane has its new dimensions while the PDF holders still have
  ...
  function savePdfReadingPosition(schedule,writeState){
    if(restoringPdfPosition)return false;var ch=find(currentId),position=stablePdfPositionForRebuild();if(!ch||ch.kind!=='pdf'||!position)return false;
    ch.pdfPosition=position;ch.readPage=position.page;ch.readThroughPage=Math.max(+ch.readThroughPage||0,pagedPdfFlow()?pagedPageNos(currentPage).slice(-1)[0]:position.page);if(writeState!==false)persist(schedule);return true;
  }
  function saveCurrentReadingPosition(schedule,writeState){
    var ch=find(currentId);if(!ch)return false;if(readerMode==='pdf')return savePdfReadingPosition(schedule,writeState);
    var pane=byId('documentPane'),span=pane.scrollHeight-pane.clientHeight;ch.readerScroll=span>4?pane.scrollTop/span:0;if(writeState!==false)persist(schedule);return true;
  }
  function trackCurrentPage(){
    if(readerMode!=='pdf'||!pdfViews.length||pagedPdfFlow())return;
    var pane=byId('documentPane');
  ...
  async function renderPdfPage(preservedPosition){
    if(!pdfDoc)return;
    var requestedDoc=pdfDoc,requestedId=currentId;
    var flowSignature=pagedSignature();if(pagedFlowSignature&&flowSignature!==pagedFlowSignature)invalidatePagedTurns();pagedFlowSignature=flowSignature;
    if(pdfBuildKey!==currentBuildKey()||!pdfViews.length){
      ...
      var pane=byId('documentPane'),hadViews=pdfViews.length>0,rebuildPosition=normalizedPdfPosition(preservedPosition)||(hadViews?stablePdfPositionForRebuild():null);
      try{await buildPdfScroll();}catch(e){if(pdfDoc===requestedDoc&&currentId===requestedId)showError(e.message||'This PDF could not be laid out.');return;}
      if(pdfDoc!==requestedDoc||currentId!==requestedId)return;
      if(hadViews&&rebuildPosition&&!pagedTurning)await placePdfReadingPosition(rebuildPosition,false,true);
      else if(pagedPdfFlow()){syncPagedPages();updatePageChrome();await fitPagedPages(currentPage,false);}
      else scrollToPdfPage(currentPage,'auto');
      if(!rebuildPosition)pdfLayoutFrames().then(function(){if(pdfDoc===requestedDoc&&currentId===requestedId&&pdfBuildKey===currentBuildKey())rememberStablePdfPosition(capturePdfReadingPosition());});
    }
    else updatePageChrome();
  }
  byId('prevPage').onclick=function(){if(!pdfDoc)return;cancelPdfPositionRestore();clearPendingSelection();if(pagedPdfFlow())requestPagedStep(-1,true);else if(currentPage>1)scrollToPdfPage(currentPage-1);};
  byId('nextPage').onclick=function(){if(!pdfDoc)return;cancelPdfPositionRestore();clearPendingSelection();if(pagedPdfFlow())requestPagedStep(1,true);else if(currentPage<pdfDoc.numPages)scrollToPdfPage(currentPage+1);};
  ...
  function setZoom(value,anchorX,anchorY,shiftX,shiftY,onSettled,persistChoice,exactFit){
    if(!pdfDoc){if(onSettled)onSettled();return Promise.resolve(false);}
    var zoomDoc=pdfDoc,zoomId=currentId,pane=byId('documentPane'),rect=pane.getBoundingClientRect();
    ...
    zoomChain=zoomChain.then(async function(){
      ...
      if(serial!==zoomSerial||pdfDoc!==zoomDoc||currentId!==zoomId){if(pdfDoc===zoomDoc&&currentId===zoomId&&onSettled)onSettled();return false;}
      var f=newZoom/layoutZoom;
      var targetLeft=(pane.scrollLeft+ax)*f-ax-(shiftX||0),targetTop=(pane.scrollTop+ay)*f-ay-(shiftY||0);
      if(pdfBuildKey!==currentBuildKey()||!pdfViews.length){
        try{await buildPdfScroll();}catch(e){if(pdfDoc===zoomDoc&&currentId===zoomId){if(onSettled)onSettled();showError(e.message||'This PDF could not be laid out.');}return false;}
      }
      if(serial!==zoomSerial||pdfDoc!==zoomDoc||currentId!==zoomId)return false;
      pane.scrollLeft=Math.max(0,targetLeft);pane.scrollTop=Math.max(0,targetTop);
      if(onSettled)onSettled();
      if(comfort.focus)requestAnimationFrame(placeGuide);savePdfReadingPosition(false);return true;
    }).catch(function(){if(pdfDoc===zoomDoc&&currentId===zoomId&&onSettled)onSettled();return false;});
    return zoomChain;
  }
  function setUserZoom(value,anchorX,anchorY,shiftX,shiftY,onSettled,preserveColumn){
    cancelPdfPositionRestore();
  ...
  byId('documentPane').addEventListener('wheel',function(e){
    ...
    if(!pagedPdfFlow()||Math.abs(e.deltaX)<12||Math.abs(e.deltaX)<=Math.abs(e.deltaY)*1.12)return;
    var pane=byId('documentPane'),forward=e.deltaX>0;
    var atTurnEdge=pagedFits||(forward?pane.scrollLeft>=pagedMaxLeft-3:pane.scrollLeft<=pagedMinLeft+3);
    clearTimeout(pagedWheelTimer);
    pagedWheelTimer=setTimeout(function(){pagedWheelSum=0;pagedWheelConsumed=false;},210);
    if(!atTurnEdge||pagedWheelConsumed)return;
    e.preventDefault();
    if(pagedWheelSum&&Math.sign(pagedWheelSum)!==Math.sign(e.deltaX))pagedWheelSum=0;
    pagedWheelSum+=e.deltaX;
    if(Math.abs(pagedWheelSum)<72)return;
  ...
  (function(){
    ...
    function startPan(t,ts,allowEdge){
      var direction=pagedPdfFlow()&&allowEdge!==false?touchEdgeDirection(t):0,maxLeft=Math.max(0,pane.scrollWidth-pane.clientWidth);
      var ownsEdge=direction>0?pane.scrollLeft>=maxLeft-2:direction<0?pane.scrollLeft<=2:false;
      return {target:t.target,lastX:t.clientX,lastY:t.clientY,lastT:ts,startX:t.clientX,startY:t.clientY,startT:ts,vx:0,vy:0,edgePull:0,edgeDirection:ownsEdge?direction:0,claimed:false};
    }
    function activeBookHolder(dx){
      var pages=pagedPageNos(currentPage),pageNo=currentPage;
      if(bookSpread()&&pages.length>1)pageNo=dx<0?pages[pages.length-1]:pages[0];
      return pdfViews[pageNo-1]&&pdfViews[pageNo-1].holder;
    }
    /* In a spread, the leaf the reader touches becomes the page-note and discussion
    ...
    /* A Book leaf is manipulated like paper rather than treated as a swipe target. The
       ...
       the live fold while the next physical page is reflected into its underside. */
    function curlBlockedTarget(e){
      if(!e.target.closest||e.target.closest('a,button,.pdf-link,.text-layer span,mark'))return true;
      if(e.metaKey||e.ctrlKey||e.altKey||e.shiftKey||highlightMode||reviewLinkTargetId)return true;
      var selection=window.getSelection&&window.getSelection();return !!(selection&&!selection.isCollapsed);
    }
    function touchInteractiveTarget(target){return !target||!target.closest||!!target.closest('a,button,.pdf-link,mark');}
    function touchCurlBlockedTarget(target){
      /* Horizontal intent is decided only after movement, so ordinary text remains a
         valid swipe surface while long-press selection stays native. Links, controls,
         and existing marks keep their own touch interaction. */
      if(touchInteractiveTarget(target))return true;
      if(highlightMode||reviewLinkTargetId)return true;
      var selection=window.getSelection&&window.getSelection();return !!(selection&&!selection.isCollapsed);
    }
    function makeCurlPlan(direction,target,leaf,rect){
      var fromPages=pagedPageNos(currentPage),toPages=pagedPageNos(target),spread=bookSpread();
      if(samePages(fromPages,toPages))return null;
      ...
      return{direction:direction,target:target,sourceNo:sourceNo,backNo:backNo,underNo:underNo,underSide:spread?(direction>0?'right':'left'):'single',spread:spread,layout:comfort.pdfLayout,leaf:leaf,rect:rect};
    }
    function curlPlanAt(e){
      if(!pdfDoc||!pagedPdfFlow()||!pagedFits||pagedTurning||!pagedCurlGeometryReady()||curlBlockedTarget(e))return null;
      var leaf=e.target.closest('.pdf-page.book-active');if(!leaf)return null;
      var rect=leaf.getBoundingClientRect(),edge=Math.min(88,Math.max(34,rect.width*.18)),direction=0,target=0,spread=bookSpread();
      if((spread?leaf.classList.contains('book-spread-right'):leaf.classList.contains('book-single'))&&e.clientX>=rect.right-edge){direction=1;target=pagedNextTarget();}
      else if((spread?leaf.classList.contains('book-spread-left'):leaf.classList.contains('book-single'))&&e.clientX<=rect.left+edge){direction=-1;target=pagedPreviousTarget();}
      if(!direction)return null;
      return makeCurlPlan(direction,target,leaf,rect);
    }
    function clearCurlReady(){
      if(curlReadyLeaf)curlReadyLeaf.classList.remove('book-curl-ready-next','book-curl-ready-prev','book-curl-ready-top','book-curl-ready-bottom');
      curlReadyLeaf=null;
      if(!bookCurl&&frame.dataset.curlState==='ready'){
        delete frame.dataset.curlState;delete frame.dataset.curlDirection;delete frame.dataset.curlSource;delete frame.dataset.curlProgress;
      }
    }
    function warmCurlPlan(plan){
      if(!plan)return Promise.resolve();
      var pages=[plan.backNo];if(plan.underNo)pages.push(plan.underNo);
      return Promise.all(pages.filter(function(page,index){return page&&pages.indexOf(page)===index;}).map(function(page){return Promise.resolve(renderPdfPageAt(page));}));
    }
    function showCurlReady(plan,e){
      if(!plan){clearCurlReady();return;}
      if(curlReadyLeaf!==plan.leaf)clearCurlReady();
      curlReadyLeaf=plan.leaf;
      plan.leaf.classList.add(plan.direction>0?'book-curl-ready-next':'book-curl-ready-prev');
      var halfway=plan.rect.top+plan.rect.height/2;
      plan.leaf.classList.toggle('book-curl-ready-top',e.clientY<=halfway);
      plan.leaf.classList.toggle('book-curl-ready-bottom',e.clientY>halfway);
      frame.dataset.curlState='ready';frame.dataset.curlDirection=plan.direction>0?'next':'prev';frame.dataset.curlSource=String(plan.sourceNo);
      warmCurlPlan(plan);
    }
    function copyCurlCanvas(g){
      var view=pdfViews[g.backNo-1],ready=!!(view&&view.rendered&&view.canvas&&view.canvas.width&&view.canvas.height);
      var source=ready?view.canvas:g.sourceView&&g.sourceView.canvas,target=g.backCanvas;if(!source||!target||!source.width||!source.height)return;
      if(g.overlay)g.overlay.classList.toggle('book-curl-back-fallback',!ready);
      /* The moving copy is transient and filtered every frame. Cap it well below the
         real PDF canvas (which may be 14 MP) to avoid a ~56 MB allocation/GPU upload on
         every turn; its CSS box stays full-size and remains crisp while moving. */
      var pixelScale=Math.min(1,Math.sqrt(32e5/Math.max(1,source.width*source.height)));
      var width=Math.max(1,Math.round(source.width*pixelScale)),height=Math.max(1,Math.round(source.height*pixelScale));
      if(target.width!==width||target.height!==height){target.width=width;target.height=height;}
      try{var context=target.getContext('2d');context.clearRect(0,0,target.width,target.height);context.drawImage(source,0,0,width,height);}catch(err){}
    }
    function stageBookCurl(g){
      if(g.staged)return;g.staged=true;
      var source=g.sourceView.holder,sourceRect=source.getBoundingClientRect(),frameRect=frame.getBoundingClientRect();
      g.rect=sourceRect;g.width=sourceRect.width;g.height=sourceRect.height;g.left=sourceRect.left-frameRect.left;g.top=sourceRect.top-frameRect.top;
      g.grabY=Math.max(0,Math.min(g.height,g.startY-sourceRect.top));
      g.corner={x:g.direction>0?g.width:0,y:g.grabY};
      var overlay=document.createElement('div');overlay.className='book-curl-overlay'+(g.spread?'':' book-curl-single');overlay.dataset.page=String(g.sourceNo);overlay.dataset.backPage=String(g.backNo);
      overlay.setAttribute('aria-hidden','true');
      overlay.style.left=g.left+'px';overlay.style.top=g.top+'px';overlay.style.width=g.width+'px';overlay.style.height=g.height+'px';
      overlay.innerHTML='<div class="book-curl-back-sheet"><canvas class="book-curl-back-canvas"></canvas><span class="book-curl-back-wash"></span></div>';
      var fx=document.createElement('div');fx.className='book-curl-fx';fx.setAttribute('aria-hidden','true');fx.innerHTML='<i class="book-curl-cast"></i><i class="book-curl-ridge"></i><i class="book-curl-tip"></i>';
      frame.appendChild(overlay);frame.appendChild(fx);
      g.overlay=overlay;g.backCanvas=overlay.querySelector('canvas');g.fx=fx;g.cast=fx.children[0];g.ridge=fx.children[1];g.tip=fx.children[2];
      g.backCanvas.style.width=g.width+'px';g.backCanvas.style.height=g.height+'px';
      source.classList.add('book-curl-front');
      var under=g.underNo&&pdfViews[g.underNo-1]&&pdfViews[g.underNo-1].holder;
      /* The last forward turn and the first backward turn both uncover an intentionally
         blank board-side leaf. It is still paper, so stage it instead of showing desk. */
      if(!under)under=byId('bookBlankLeaf');
      if(under){
        g.underHadAria=under.hasAttribute('aria-hidden');g.underAria=under.getAttribute('aria-hidden');g.underHadInert=under.hasAttribute('inert');under.setAttribute('aria-hidden','true');under.setAttribute('inert','');
        under.classList.add('book-curl-under','book-curl-under-'+g.underSide);g.underHolder=under;
        /* A book opened directly near an even-numbered ending may never have shown its
           placeholder before. Give that final blank the outgoing leaf's real size now,
           so the desk cannot flash through beneath the last turn. */
        if(under.classList.contains('book-blank-leaf')){under.style.width=g.width+'px';under.style.height=g.height+'px';}
      }
      frame.classList.add('book-curl-active');pane.classList.add('turning-book-leaf');pane.classList.toggle('dragging-book-leaf',!g.automatic);
      frame.dataset.curlState='dragging';frame.dataset.curlDirection=g.direction>0?'next':'prev';frame.dataset.curlSource=String(g.sourceNo);frame.dataset.curlBack=String(g.backNo);
      copyCurlCanvas(g);
      Promise.resolve(g.preparePromise).then(function(){if(bookCurl===g){copyCurlCanvas(g);if(g.underNo)renderPdfPageAt(g.underNo);}});
    }
    function clipCurlHalf(width,height,nx,ny,line,positive){
      var points=[{x:0,y:0},{x:width,y:0},{x:width,y:height},{x:0,y:height}],out=[];
      for(var i=0;i<points.length;i++){
        var a=points[i],b=points[(i+1)%points.length],av=nx*a.x+ny*a.y-line,bv=nx*b.x+ny*b.y-line;
        var ain=positive?av>=-.01:av<=.01,bin=positive?bv>=-.01:bv<=.01;
        if(ain)out.push(a);
        if(ain!==bin){var t=av/(av-bv);out.push({x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t});}
      }
      return out;
    }
    function curlPolygon(points){
      if(points.length<3)return 'polygon(0 0,0 0,0 0)';
      return 'polygon('+points.map(function(p){return (Math.round(p.x*10)/10)+'px '+(Math.round(p.y*10)/10)+'px';}).join(',')+')';
    }
    function curlCreaseEnds(width,height,nx,ny,line){
      var points=[];
      function add(x,y){if(x>=-.1&&x<=width+.1&&y>=-.1&&y<=height+.1&&!points.some(function(p){return Math.hypot(p.x-x,p.y-y)<.5;}))points.push({x:Math.max(0,Math.min(width,x)),y:Math.max(0,Math.min(height,y))});}
      if(Math.abs(ny)>.00001){add(0,line/ny);add(width,(line-nx*width)/ny);}
      if(Math.abs(nx)>.00001){add(line/nx,0);add((line-ny*height)/nx,height);}
      if(points.length<2)return null;
      var pair=[points[0],points[1]],distance=0;
    ...
    function drawBookCurl(g,freeX,freeY){
      ...
      var depth=Math.sin(Math.PI*progress),ends=curlCreaseEnds(g.width,g.height,nx,ny,line);
      g.overlay.style.filter='drop-shadow('+(g.direction>0?-1:1)*Math.round(6+depth*16)+'px '+Math.round(4+depth*7)+'px '+Math.round(5+depth*10)+'px rgba(29,24,17,'+(.12+depth*.28).toFixed(3)+'))';
      if(ends){
        var centerX=g.left+(ends.a.x+ends.b.x)/2,centerY=g.top+(ends.a.y+ends.b.y)/2,angle=Math.atan2(ends.b.y-ends.a.y,ends.b.x-ends.a.x)*180/Math.PI;
        [g.cast,g.ridge].forEach(function(el){el.style.left=centerX+'px';el.style.top=centerY+'px';el.style.width=(ends.length+26)+'px';el.style.transform='translate(-50%,-50%) rotate('+angle+'deg)';});
      }
      g.fx.style.opacity='1';g.cast.style.opacity=(depth*.74).toFixed(3);g.ridge.style.opacity=(.12+depth*.8).toFixed(3);
      g.tip.style.left=(g.left+freeX)+'px';g.tip.style.top=(g.top+freeY)+'px';g.tip.style.opacity=(.18+depth*.72).toFixed(3);g.tip.style.transform='translate(-50%,-50%) rotate('+(Math.atan2(freeY-cy,freeX-cx)*180/Math.PI)+'deg)';
    }
    function pointBookCurl(g,clientX,clientY){
      var localStart=g.startX-g.rect.left,freeX=g.direction>0?clientX-g.rect.left+(g.width-localStart):clientX-g.rect.left-localStart;
      drawBookCurl(g,freeX,clientY-g.rect.top);
    }
    function queueBookCurlPoint(g,clientX,clientY){
      g.pendingPoint={x:clientX,y:clientY};if(g.pointRaf)return;
      g.pointRaf=requestAnimationFrame(function(){g.pointRaf=null;if(!g.pendingPoint)return;var point=g.pendingPoint;g.pendingPoint=null;pointBookCurl(g,point.x,point.y);});
    }
    function finishAutomaticBookCurl(g,completed){
      if(!g||!g.autoResolve)return;var resolve=g.autoResolve;g.autoResolve=null;resolve({started:true,committed:!!completed,reason:g.cancelReason||''});
    }
    function cleanupBookCurl(g,deferAutomaticDone){
      if(!g)return;
      if(g.pointRaf)cancelAnimationFrame(g.pointRaf);if(g.settleRaf)cancelAnimationFrame(g.settleRaf);
      if(g.sourceView&&g.sourceView.holder){g.sourceView.holder.style.clipPath='';g.sourceView.holder.style.visibility='';g.sourceView.holder.classList.remove('book-curl-front');}
      if(g.underHolder){
        g.underHolder.classList.remove('book-curl-under','book-curl-under-left','book-curl-under-right','book-curl-under-single');
        if(g.underHadAria)g.underHolder.setAttribute('aria-hidden',g.underAria);else g.underHolder.removeAttribute('aria-hidden');
        if(!g.underHadInert)g.underHolder.removeAttribute('inert');
      }
    ...
    function commitBookCurl(g){
      Promise.resolve(g.preparePromise).then(function(){
        if(bookCurl!==g)return;
        if(pdfDoc!==g.doc||currentId!==g.documentId||g.buildId!==pdfBuildId||!pagedPdfFlow()||comfort.pdfLayout!==g.layout||bookSpread()!==g.spread||(g.turnEpoch!==undefined&&g.turnEpoch!==pagedTurnEpoch)){cleanupBookCurl(g);return;}
        copyCurlCanvas(g);currentPage=g.target;syncPagedPages();updatePageChrome();updateProgress();savePagedPosition();
        requestAnimationFrame(function(){
          if(bookCurl!==g)return;
          cleanupBookCurl(g,true);
          Promise.resolve(fitPagedPages(currentPage,false)).then(function(){finishAutomaticBookCurl(g,true);},function(){finishAutomaticBookCurl(g,true);});
        });
      }).catch(function(){if(bookCurl===g)cleanupBookCurl(g);});
    }
    function settleBookCurl(g,commit){
      if(bookCurl!==g)return;g.settling=true;frame.dataset.curlState='settling';
      var startX=Number.isFinite(g.freeX)?g.freeX:g.corner.x,startY=Number.isFinite(g.freeY)?g.freeY:g.corner.y;
      var endX=commit?(g.direction>0?-g.width:g.width*2):g.corner.x,endY=g.corner.y;
      var reduced=matchMedia('(prefers-reduced-motion: reduce)').matches,distance=Math.abs(endX-startX)/(g.width*2),duration=reduced?0:g.automatic?460:Math.max(130,Math.min(340,130+distance*280));
      function finish(){drawBookCurl(g,endX,endY);if(commit)commitBookCurl(g);else cleanupBookCurl(g);}
      if(!duration){finish();return;}
      var began=performance.now();
      function tick(now){
        if(bookCurl!==g)return;var t=Math.min(1,(now-began)/duration),ease=g.automatic?t*t*(3-2*t):commit?1-Math.pow(1-t,3):1-Math.pow(1-t,2);
        var bow=g.automatic?-Math.sin(Math.PI*t)*g.height*.025:0;
        drawBookCurl(g,startX+(endX-startX)*ease,startY+(endY-startY)*ease+bow);
        if(t<1)g.settleRaf=requestAnimationFrame(tick);else{g.settleRaf=null;finish();}
      }
      g.settleRaf=requestAnimationFrame(tick);
    }
    cancelActiveBookCurl=function(immediate,reason){
      var g=bookCurl;if(!g){clearCurlReady();return false;}
      if(g.automatic&&reason)g.cancelReason=reason;
    ...
    runAutomaticBookCurl=function(target,turnEpoch,originYRatio){
      ...
      target=Math.max(1,Math.min(pdfDoc.numPages,target));
      var fromPages=pagedPageNos(currentPage),toPages=pagedPageNos(target);
      if(samePages(fromPages,toPages))return Promise.resolve(null);
      var direction=toPages[0]>fromPages[0]?1:-1;
    ...
    pane.addEventListener('pointerdown',function(e){
      if((e.pointerType!=='mouse'&&e.pointerType!=='pen')||e.button!==0||bookCurl)return;
      var plan=curlPlanAt(e);if(!plan)return;
      clearCurlReady();
      var g=Object.assign(plan,{serial:++curlSerial,pointerId:e.pointerId,startX:e.clientX,startY:e.clientY,lastX:e.clientX,lastY:e.clientY,lastT:e.timeStamp,vx:0,moved:false,staged:false,settling:false,doc:pdfDoc,documentId:currentId,buildId:pdfBuildId,sourceView:pdfViews[plan.sourceNo-1]});
      if(!g.sourceView||g.sourceView.buildId!==pdfBuildId||!g.sourceView.holder.isConnected)return;
      bookCurl=g;bookCurlOwned=true;frame.dataset.curlState='armed';frame.dataset.curlDirection=g.direction>0?'next':'prev';frame.dataset.curlSource=String(g.sourceNo);
      g.preparePromise=warmCurlPlan(g);
      /* This is the deliberately reserved outer-edge handle (links and text spans were
         rejected above), so prevent the canvas/image drag ghost from competing with it. */
      e.preventDefault();
      try{pane.setPointerCapture(e.pointerId);}catch(err){}
    });
    pane.addEventListener('pointermove',function(e){
      var g=bookCurl;
      if(!g){if(e.pointerType==='mouse'||e.pointerType==='pen')showCurlReady(curlPlanAt(e),e);return;}
      if(g.pointerId!==e.pointerId||g.settling)return;
      var dx=e.clientX-g.startX,dy=e.clientY-g.startY,directional=g.direction>0?-dx:dx;
      var dt=Math.max(1,e.timeStamp-g.lastT),instant=(e.clientX-g.lastX)/dt;g.vx=g.vx*.55+instant*.45;g.lastX=e.clientX;g.lastY=e.clientY;g.lastT=e.timeStamp;
      if(!g.moved){
        if(directional<=8||Math.abs(dx)<=Math.abs(dy)*.22)return;
        g.moved=true;stageBookCurl(g);clearPendingSelection();
      }
      e.preventDefault();queueBookCurlPoint(g,e.clientX,e.clientY);
    });
    function endPointerBookCurl(e){
      var g=bookCurl;if(!g||g.pointerId!==e.pointerId||g.settling)return;
      if(g.pointRaf){cancelAnimationFrame(g.pointRaf);g.pointRaf=null;}if(g.pendingPoint){var point=g.pendingPoint;g.pendingPoint=null;pointBookCurl(g,point.x,point.y);}
      if(!g.moved){cleanupBookCurl(g);return;}
      e.preventDefault();pagedSuppressClickUntil=Date.now()+520;
      /* Velocity is a flick only while it is fresh; holding a partly folded page still
         for a beat must let it fall back instead of unexpectedly completing the turn. */
      var velocityAge=Math.max(0,e.timeStamp-g.lastT),directionalVelocity=(g.direction>0?-g.vx:g.vx)*Math.exp(-velocityAge/90),commit=g.progress>=.42||(g.progress>=.13&&directionalVelocity>=.55);
      settleBookCurl(g,commit);
    }
    pane.addEventListener('pointerup',endPointerBookCurl);
    pane.addEventListener('pointercancel',function(e){if(bookCurl&&bookCurl.pointerId===e.pointerId)cancelActiveBookCurl(true);});
    pane.addEventListener('lostpointercapture',function(e){if(bookCurl&&bookCurl.pointerId===e.pointerId&&!bookCurl.settling)cancelActiveBookCurl(true);});
    pane.addEventListener('pointerleave',function(){if(!bookCurl)clearCurlReady();});
    function touchById(list,id){
      if(!list)return null;for(var i=0;i<list.length;i++)if(list[i].identifier===id)return list[i];return null;
    }
    function touchCurlPlan(direction){
      var leaf=activeBookHolder(direction>0?-1:1);if(!leaf)return null;
      var target=direction>0?pagedNextTarget():pagedPreviousTarget();
      return makeCurlPlan(direction,target,leaf,leaf.getBoundingClientRect());
    }
    function touchCurlGeometryReady(){return pagedCurlGeometryReady();}
    function touchEdgeDirection(touch){
      var pages=pagedPageNos(currentPage),spread=bookSpread();
      for(var i=0;i<pages.length;i++){
        var leaf=pdfViews[pages[i]-1]&&pdfViews[pages[i]-1].holder;if(!leaf)continue;
        if(!leaf.classList.contains('book-active'))continue;
        var rect=leaf.getBoundingClientRect();
        if(touch.clientY<rect.top||touch.clientY>rect.bottom||touch.clientX<rect.left||touch.clientX>rect.right)continue;
        var edge=Math.min(72,Math.max(44,rect.width*.1));
        if((!spread||leaf.classList.contains('book-spread-right'))&&touch.clientX>=rect.right-edge)return 1;
    ...
    function beginTouchBookCurl(swipe,touch,timeStamp){
      if(bookCurl||pagedTurning||!pagedPdfFlow()||!touchCurlGeometryReady()||touchCurlBlockedTarget(swipe.target))return null;
      var direction=swipe.direction||(touch.clientX<swipe.x?1:-1),plan=touchCurlPlan(direction);if(!plan)return null;
      var sourceView=pdfViews[plan.sourceNo-1];
      if(!sourceView||sourceView.buildId!==pdfBuildId||!sourceView.holder.isConnected||!sourceView.holder.classList.contains('book-active'))return null;
      var g=Object.assign(plan,{serial:++curlSerial,pointerId:-2,touchId:swipe.id,inputType:'touch',startX:swipe.x,startY:swipe.y,lastX:swipe.x,lastY:swipe.y,lastT:Number.isFinite(swipe.startT)?swipe.startT:timeStamp,vx:0,moved:true,staged:false,settling:false,automatic:false,doc:pdfDoc,documentId:currentId,buildId:pdfBuildId,sourceView:sourceView});
      bookCurl=g;bookCurlOwned=true;g.preparePromise=warmCurlPlan(g);stageBookCurl(g);frame.dataset.curlOrigin='finger';
      tapStart=null;lastTapAt=0;clearPendingSelection();moveTouchBookCurl(g,touch,timeStamp);
      return g;
    }
    function moveTouchBookCurl(g,touch,timeStamp){
      if(!g||g.inputType!=='touch'||g.touchId!==touch.identifier||g.settling)return;
      var dt=Math.max(1,timeStamp-g.lastT),instant=Math.max(-3,Math.min(3,(touch.clientX-g.lastX)/dt));
      g.vx=g.vx*.5+instant*.5;g.lastX=touch.clientX;g.lastY=touch.clientY;g.lastT=timeStamp;
      queueBookCurlPoint(g,touch.clientX,touch.clientY);
    }
    function releaseTouchBookCurl(e){
      var g=bookCurl;if(!g||g.inputType!=='touch'||touchById(e.touches,g.touchId))return false;
      var touch=touchById(e.changedTouches,g.touchId);
      /* A touchend coordinate is geometry, not a fresh velocity sample: Safari may send
         it long after the last move. Keep the last real motion time so a held fold can
         settle back instead of being mistaken for a new flick. */
      var motionLastT=g.lastT,motionVx=g.vx;if(touch)queueBookCurlPoint(g,touch.clientX,touch.clientY);
      if(g.pointRaf){cancelAnimationFrame(g.pointRaf);g.pointRaf=null;}
      if(g.pendingPoint){var point=g.pendingPoint;g.pendingPoint=null;pointBookCurl(g,point.x,point.y);}
      pagedSuppressClickUntil=Date.now()+520;
      var velocityAge=Math.max(0,e.timeStamp-motionLastT),directionalVelocity=velocityAge>140?0:(g.direction>0?-motionVx:motionVx)*Math.exp(-velocityAge/110);
      /* A finger has less travel than a mouse across an iPad. A deliberate third-page
         pull, or a shorter fresh flick, completes; a held shallow fold falls back. */
      var commit=g.progress>=.36||(g.progress>=.09&&directionalVelocity>=.32);
      settleBookCurl(g,commit);return true;
    }
    function handlePanMove(e){
      var t=e.touches[0],g=panGesture;
      var sel=window.getSelection&&window.getSelection();
      if(sel&&!sel.isCollapsed){panGesture=null;return;}
      var totalX=t.clientX-g.startX,totalY=t.clientY-g.startY;
      if(!g.claimed){
      ...
      if(pagedPdfFlow()){
        var liveMaxLeft=Math.max(0,pane.scrollWidth-pane.clientWidth),atNextEdge=pane.scrollLeft>=liveMaxLeft-2&&dx<0,atPrevEdge=pane.scrollLeft<=2&&dx>0;
        var claimedDirection=atNextEdge&&g.edgeDirection===1?1:atPrevEdge&&g.edgeDirection===-1?-1:0;
        if(claimedDirection){
          if(!g.edgePull){g.edgeStartX=g.lastX;g.edgeStartY=g.lastY;g.edgeStartT=g.lastT;}
          g.edgePull+=dx;
          if(Math.abs(g.edgePull)>10&&Math.abs(g.edgePull)>Math.abs(t.clientY-g.edgeStartY)*1.08){
            var swipe={id:t.identifier,target:g.target||t.target,direction:claimedDirection,x:g.startX,y:g.startY,startT:g.startT,lastX:g.startX,lastY:g.startY,moved:true};
            if(beginTouchBookCurl(swipe,t,e.timeStamp)){bookSwipe=swipe;panGesture=null;return;}
          }
    ...
    pane.addEventListener('touchstart',function(e){
      ...
      e.preventDefault();tapStart=null;lastTapAt=0;panGesture=null;bookSwipe=null;if(bookCurl||pagedTurning)invalidatePagedTurns();
      var rect=pane.getBoundingClientRect(),m=touchMid(e.touches);
      pinch={d:Math.max(20,touchDist(e.touches)),zoom:currentZoom(),k:1,x:m.x,y:m.y,curX:m.x,curY:m.y,fitInvalidated:false};
    ...
    pane.addEventListener('touchmove',function(e){
      ...
      if(bookSwipe&&!pinch&&e.touches.length===1){
        ...
        if(Math.abs(bdx)>10&&Math.abs(bdx)>Math.abs(bdy)*1.08&&(!bookSwipe.direction||directional>10)){
          ...
          var selection=window.getSelection&&window.getSelection();
          if(selection&&!selection.isCollapsed){bookSwipe=null;return;}
          e.preventDefault();bookSwipe.moved=true;if(bookSwipe.direction)beginTouchBookCurl(bookSwipe,bt,e.timeStamp);
        }
      ...
      e.preventDefault();
      var m=touchMid(e.touches),target=Math.max(pagedPdfFlow()?.05:.5,Math.min(4,pinch.zoom*(touchDist(e.touches)/pinch.d)));
      pinch.k=target/pinch.zoom;pinch.curX=m.x;pinch.curY=m.y;
      if(pagedPdfFlow()&&!pinch.fitInvalidated&&(Math.abs(pinch.k-1)>.005||Math.hypot(m.x-pinch.x,m.y-pinch.y)>2)){invalidatePagedFit();pinch.fitInvalidated=true;}
    ...
    function cancelTouchInteraction(reason){
      var refit=!!(pinch&&pinch.fitInvalidated),lifecycle=reason==='pagehide'||reason==='hidden';pinch=null;panGesture=null;bookSwipe=null;tapStart=null;lastTapAt=0;stopPanMomentum();clearPinchPreview();
      if(lifecycle&&pagedPdfFlow())invalidatePagedTurns();
    ...
    function endPinch(e){
      ...
      if(commit.k===1&&Math.hypot(shiftX,shiftY)<2){clearPinchPreview();if(commit.fitInvalidated)fitPagedPages(currentPage,false,0,pagedFitEpoch);return;}
    ...
    addEventListener('pagehide',function(){cancelTouchInteraction('pagehide');});
    document.addEventListener('visibilitychange',function(){
      if(document.visibilityState==='hidden')cancelTouchInteraction('hidden');
      else if(pdfDoc&&pagedPdfFlow()&&!pagedCurlGeometryReady())fitPagedPages(currentPage,false,0,pagedFitEpoch);
    });
  ...
  document.addEventListener('keydown',function(e){
    ...
    if(e.key==='Tab'&&temporaryNotebookMode()&&byId('notebook').classList.contains('sheet-open')){
    ...
    if(e.key===' '&&!e.shiftKey&&pagedPdfFlow()){
    ...
    else if(e.key==='PageUp'){e.preventDefault();if(pagedPdfFlow())byId('prevPage').click();else pane.scrollBy({top:-pane.clientHeight*.85,behavior:'smooth'});}
  ...
  addEventListener('resize',function(){
    ...
    syncTabletReaderUi();
    var rotatedNotebook=byId('notebook'),sheetTucked=temporaryNotebookMode()&&!rotatedNotebook.classList.contains('sheet-open');
    try{rotatedNotebook.inert=sheetTucked;}catch(e){}
    rotatedNotebook.setAttribute('aria-hidden',String(sheetTucked));
    if(byId('readerPage').classList.contains('hidden'))return;
    var resizePosition=readerMode==='pdf'&&pdfDoc?stablePdfPositionForRebuild():null;
    setNotebookWidth(notebookWidth,false);
    if(!temporaryNotebookMode())setNotebookCollapsed(notebookCollapsed,false);
    clearTimeout(refitTimer);refitTimer=setTimeout(function(){if(readerMode==='pdf'&&pdfDoc)renderPdfPage(resizePosition);},220);
  });
  ...
  if(window.ResizeObserver){
    var paneRefitTimer=null,paneLastWidth=0,paneLastHeight=0;
    new ResizeObserver(function(entries){
      var width=Math.round(entries[0].contentRect.width),height=Math.round(entries[0].contentRect.height);
      if(Math.abs(width-paneLastWidth)<2&&Math.abs(height-paneLastHeight)<2)return;
      var resizePosition=readerMode==='pdf'&&pdfDoc?stablePdfPositionForRebuild():null;
      paneLastWidth=width;paneLastHeight=height;
      if(readerMode==='pdf'&&pdfDoc&&pagedPdfFlow())setPagedReady(false);
  ...
  function renderText(ch){
    ...
    var reviewsButton=byId('openReaderReviews');if(reviewsButton)reviewsButton.onclick=function(){switchTab('reviewsPanel');if(temporaryNotebookMode())toggleSheet(true);else setNotebookCollapsed(false,true);};
  ...
  function reviewCommentSimilarity(left,right){
    var a=reviewNormalizedText(left&&left.text!==undefined?left.text:left),b=reviewNormalizedText(right&&right.text!==undefined?right.text:right);if(!a||!b)return 0;if(a===b)return 1;
    var short=a.length<=b.length?a:b,long=a.length<=b.length?b:a,contained=long.indexOf(short)>=0,aw=a.split(' ').filter(function(word){return word.length>2;}),bw=b.split(' ').filter(function(word){return word.length>2;}),aset={},bset={},intersection=0;aw.forEach(function(word){aset[word]=1;});bw.forEach(function(word){bset[word]=1;});Object.keys(aset).forEach(function(word){if(bset[word])intersection++;});var aCount=Object.keys(aset).length,bCount=Object.keys(bset).length;if(!aCount||!bCount)return contained ? .84 : 0;var containment=intersection/Math.min(aCount,bCount),union=aCount+bCount-intersection,jaccard=union?intersection/union:0,score=containment*.72+jaccard*.28;if(contained&&short.length>=28)score=Math.max(score,.84+.14*(short.length/long.length));return Math.min(1,score);
  }
  function reviewAuthorsCompatible(left,right){
    var a=reviewNormalizedText(left&&left.author||''),b=reviewNormalizedText(right&&right.author||'');if(!a||!b||a==='reviewer'||b==='reviewer')return true;return a===b;
  }
  /* Re-imports can split or trim a reviewer paragraph. Match the revised concern back
     to its old record before replacing anything, so the writer's note is not tied to
     brittle byte-for-byte comment text. Each old comment may be claimed only once. */
  function matchPriorReviewComments(incoming,existing){
    incoming=incoming||[];existing=existing||[];var matches=new Array(incoming.length),used={},candidates=[],incomingSources={},existingSources={};
    incoming.forEach(function(comment){var source=String(comment.sourceId||'');if(source)incomingSources[source]=(incomingSources[source]||0)+1;});existing.forEach(function(comment){var source=String(comment.sourceId||'');if(source)existingSources[source]=(existingSources[source]||0)+1;});
    function assign(i,j){if(matches[i]||used[j])return false;matches[i]=existing[j];used[j]=1;return true;}
    incoming.forEach(function(comment,i){for(var j=0;j<existing.length;j++){if(used[j]||!comment.id||!existing[j].id||String(comment.id)!==String(existing[j].id))continue;var source=String(comment.sourceId||''),stableSource=source&&source===String(existing[j].sourceId||'')&&incomingSources[source]===1&&existingSources[source]===1;if(stableSource||reviewCommentSimilarity(comment,existing[j])>=.72){assign(i,j);break;}}});
    incoming.forEach(function(comment,i){if(matches[i])return;var source=String(comment.sourceId||'');if(!source||incomingSources[source]!==1||existingSources[source]!==1)return;for(var j=0;j<existing.length;j++){if(!used[j]&&String(existing[j].sourceId||'')===source){assign(i,j);break;}}});
    incoming.forEach(function(comment,i){if(matches[i])return;var text=reviewNormalizedText(comment.text);for(var j=0;j<existing.length;j++){if(used[j]||!reviewAuthorsCompatible(comment,existing[j]))continue;if(text&&text===reviewNormalizedText(existing[j].text)){assign(i,j);break;}}});
    incoming.forEach(function(comment,i){if(matches[i])return;existing.forEach(function(prior,j){if(used[j]||!reviewAuthorsCompatible(comment,prior))return;var score=reviewCommentSimilarity(comment,prior);if(score>=.72)candidates.push({incoming:i,existing:j,score:score});});});
    candidates.sort(function(a,b){return b.score-a.score;});candidates.forEach(function(candidate){assign(candidate.incoming,candidate.existing);});return matches;
  }
  function restorePriorReviewWork(comment,prior){
    if(!comment||!prior)return comment;if(prior.id)comment.id=prior.id;if(prior.responseWrittenByUser&&String(prior.response||'').trim()){comment.response=String(prior.response);comment.responseWrittenByUser=true;}comment.resolved=!!prior.resolved;if(Array.isArray(prior.replies)&&prior.replies.length)comment.replies=prior.replies;comment.updatedAt=prior.updatedAt||comment.updatedAt||0;preserveManualReviewLocation(comment,prior);return comment;
  }
  ...
  function reviewTrimStructuralPrefix(text,range){
    text=String(text||'');if(!range||!text)return range;
    var start=Math.max(0,+range.start||0),end=Math.min(text.length,+range.end||0);if(end<=start)return range;
    var excerpt=text.slice(start,end),heading=/(?:^|\s)(?:\d+(?:\.\d+)*\s+)?(?:Abstract|Introduction|Background|Materials and Methods|Methods|Results|Discussion|Conclusions?|Acknowledg(?:e)?ments|References)\s+/g,match,proseStart=null;
    while((match=heading.exec(excerpt))){var after=start+match.index+match[0].length;if(end-after>=22)proseStart=after;}
    if(proseStart===null)return range;
    var sentence=reviewSentenceRanges(text.slice(proseStart,end)).find(function(item){return reviewNormalizedText(item.quote).split(' ').filter(Boolean).length>=4;});
    if(!sentence)return{start:proseStart,end:end,quote:text.slice(proseStart,end).trim()};
    var trimmedStart=proseStart+sentence.start,trimmedEnd=proseStart+sentence.end;return{start:trimmedStart,end:trimmedEnd,quote:text.slice(trimmedStart,trimmedEnd)};
  }
  function reviewSentenceRange(text,range,comment){
    text=String(text||'');if(!range||!text)return range;range=reviewTrimStructuralPrefix(text,range);var terms=reviewSearchTerms(comment),best=null,bestScore=-1;
    reviewSentenceRanges(text).forEach(function(sentence){var overlap=Math.max(0,Math.min(range.end,sentence.end)-Math.max(range.start,sentence.start));if(!overlap)return;var normalized=reviewNormalizedText(sentence.quote),score=overlap;terms.forEach(function(term){if(normalized.indexOf(term)>=0)score+=24+Math.min(18,term.length*1.5);});if(score>bestScore){best=sentence;bestScore=score;}});return reviewTrimStructuralPrefix(text,best||range);
  }
  ...
  function reviewCommentPage(ch,comment){
    if(!reviewHasDisplayablePassage(ch,comment))return null;
  ...
  function pdfReviewSpanMatch(view,quote,occurrence){
    ...
    var wanted=reviewNormalizedText(quote),at=-1,positions=[],from=0;if(wanted){while((from=joined.indexOf(wanted,from))>=0){positions.push(from);from+=Math.max(1,wanted.length);}if(positions.length)at=positions[Math.max(0,Math.min(positions.length-1,+occurrence||0))];}if(at<0){var words=wanted.split(' ');for(var width=Math.min(12,words.length);width>=Math.min(4,words.length)&&at<0;width--)for(var i=0;i+width<=words.length;i++){var phrase=words.slice(i,i+width).join(' '),found=joined.indexOf(phrase);if(found>=0){wanted=phrase;at=found;break;}}}
  ...
  function reviewRectOverlap(left,right){var x=Math.max(left.x,right.x),y=Math.max(left.y,right.y),r=Math.min(left.x+left.w,right.x+right.w),b=Math.min(left.y+left.h,right.y+right.h),intersection=Math.max(0,r-x)*Math.max(0,b-y),small=Math.min(left.w*left.h,right.w*right.h);return small?intersection/small:0;}
  function reviewMarkerEntriesBelongTogether(left,right){
    if(left.pageMarker||right.pageMarker)return!!left.pageMarker&&!!right.pageMarker;var a=reviewNormalizedText(left.anchor&&left.anchor.quote),b=reviewNormalizedText(right.anchor&&right.anchor.quote),short=a.length<=b.length?a:b,long=a.length<=b.length?b:a;if(a&&b&&(a===b||short.length>28&&long.indexOf(short)>=0))return true;return(left.rects||[]).some(function(one){return(right.rects||[]).some(function(two){return reviewRectOverlap(one,two)>=.58;});});
  }
  function reviewMarkerLabel(ch,comments){var numbers=(comments||[]).map(function(comment){return(ch.reviewComments||[]).indexOf(comment)+1;}).filter(function(number){return number>0;}).sort(function(a,b){return a-b;}),shown=numbers.slice(0,7),label='R '+shown.join(', ');if(numbers.length>shown.length)label+=' +'+(numbers.length-shown.length);return label;}
  function renderPdfReviewMarkers(ch,pageNum,view){
    view.reviewHits=[];var comments=(ch.reviewComments||[]).filter(function(comment){return !comment.resolved&&reviewHasDisplayablePassage(ch,comment)&&reviewPdfAnchorOnPage(ch,comment,pageNum);}),entries=[],groups=[];
    comments.forEach(function(comment){reviewPdfAnchors(ch,comment).filter(function(anchor){return anchor.page===+pageNum;}).forEach(function(anchor){var spans=anchor.quote?pdfReviewSpanMatch(view,anchor.quote):[],rects=pdfReviewRects(view,spans),pageMarker=!rects.length;if(pageMarker)rects=[{x:.91,y:.026,w:.077,h:.034}];entries.push({comment:comment,anchor:anchor,page:+pageNum,rects:rects,pageMarker:pageMarker});});});
    entries.forEach(function(entry){var group=groups.find(function(candidate){return candidate.entries.some(function(existing){return reviewMarkerEntriesBelongTogether(existing,entry);});});if(!group){group={entries:[],comments:[],rects:[],pageMarker:entry.pageMarker};groups.push(group);}group.entries.push(entry);if(group.comments.indexOf(entry.comment)<0)group.comments.push(entry.comment);group.rects=mergeHighlightRects(group.rects.concat(entry.rects));});
    groups.forEach(function(group){var levels={};group.comments.forEach(function(comment){levels[normalizeReviewLevel(comment.level,comment)]=1;});var level=Object.keys(levels).length===1?Object.keys(levels)[0]:'mixed',label=reviewMarkerLabel(ch,group.comments),ids=group.comments.map(function(comment){return comment.id;}).join(',');view.reviewHits.push({comment:group.comments[0],comments:group.comments,anchor:group.entries[0].anchor,page:+pageNum,rects:group.rects});group.rects.forEach(function(rect,index){var marker=document.createElement('div');marker.className=(group.pageMarker?'review-page-marker':'review-comment-highlight')+' review-level-'+level+(index===0?' review-marker-first':'');marker.dataset.reviewLabel=label;marker.dataset.reviewCommentIds=ids;marker.dataset.reviewPage=pageNum;marker.title='Open reviewer comments '+label;marker.style.left=(rect.x*100)+'%';marker.style.top=(rect.y*100)+'%';marker.style.width=(rect.w*100)+'%';marker.style.height=(rect.h*100)+'%';view.highlights.appendChild(marker);});});
  }
  ...
  async function focusReviewerPassage(ch,id,pageOverride,anchorOverride){
    ...
    renderText(ch);
    jumpToParagraph(+textAnchor.para);requestAnimationFrame(function(){var anchors=byId('textDocument').querySelectorAll('[data-review-comment-id="'+CSS.escape(id)+'"]'),anchor=anchors[textIndex]||anchors[0];if(anchor)anchor.scrollIntoView({block:'center',behavior:'smooth'});});if(temporaryNotebookMode())toggleSheet(false);
  }
  function showReviewerComment(ch,id,pageOverride,keepPassageGroup){
    var comment=reviewerById(ch,id);if(!comment)return;if(!keepPassageGroup)reviewPassageGroupIds=[];if(reviewFilter!=='all'&&reviewFilter!==normalizeReviewLevel(comment.level,comment))reviewFilter='all';reviewFocusId=id;reviewFocusPage=+pageOverride||reviewCommentPage(ch,comment)||0;reviewFocusAnchorIndex=reviewPdfAnchors(ch,comment).findIndex(function(anchor){return anchor.page===reviewFocusPage;});renderReviewerPanel(ch);switchTab('reviewsPanel');if(temporaryNotebookMode())toggleSheet(true);else setNotebookCollapsed(false,true);
  ...
  function showReviewerPassageGroup(ch,hit){
    var comments=hit&&Array.isArray(hit.comments)&&hit.comments.length?hit.comments:(hit&&hit.comment?[hit.comment]:[]);if(!ch||!comments.length)return;reviewPassageGroupIds=comments.map(function(comment){return comment.id;});showReviewerComment(ch,comments[0].id,hit.page,true);
  }
  ...
  function reviewLocalPdfQuote(text,comment){
    ...
    var terms=reviewSearchTerms(comment),targets=reviewExplicitDocumentTargets(comment),hardTargets=targets.filter(function(target){return target.kind!=='section';});if(!terms.length&&!targets.length)return'';
    ...
    candidates.forEach(function(candidate){var normalized=reviewNormalizedText(candidate),hits=0,score=0,targetHit=reviewPageTargetKeys(candidate,hardTargets).length>0;terms.forEach(function(term){if(normalized.indexOf(term)>=0){hits++;score+=1+Math.min(2.5,term.length/7);}});targets.forEach(function(target){if(reviewTargetPattern(target).test(candidate))score+=target.kind==='section'?5:9;});if(candidate.length>360)score-=1;if(score>bestScore){best=candidate;bestScore=score;bestHits=hits;bestTarget=targetHit;}});
    ...
    if(hardTargets.length&&!bestTarget)return'';if(!hardTargets.length&&!bestTarget&&(bestHits<3||bestScore<6))return'';return best.slice(0,460).trim();
  }
  function repairPdfReviewQuotes(ch,pageOnly){
    ...
    ch.reviewComments.forEach(function(comment){
      if(!reviewCanAutoLocate(comment))return;var anchors=Array.isArray(comment.pdfAnchors)?comment.pdfAnchors:[],kept=[],constraints=reviewPdfLocationConstraints(ch,comment);
      anchors.forEach(function(anchor){
        var page=+anchor.page||0,manual=!!anchor.manual||String(anchor.method||'').indexOf('manual-')===0;if(!page||pageOnly&&page!==+pageOnly){kept.push(anchor);return;}if(!manual&&!reviewPdfPageMeetsConstraints(ch,page,constraints)){changed=true;return;}var full=String(ch.pageTexts[page-1]||''),localMethod=anchor.method==='local-pdf-quote'||anchor.method==='review-page-reference',range=null;
      ...
      if(kept.length!==anchors.length){comment.pdfAnchors=kept;changed=true;}var hasManual=kept.some(function(anchor){return!!anchor.manual||String(anchor.method||'').indexOf('manual-')===0;});if(kept.length&&!hasManual&&!reviewPdfAnchorsMeetConstraints(ch,comment,kept,constraints)){kept=[];comment.pdfAnchors=[];changed=true;}if(!kept.length){if(comment.anchored){resetReviewLocation(comment);comment.locationStatus='needs-checking';changed=true;}return;}var primary=kept[0],confidence=Math.min.apply(null,kept.map(function(anchor){return Math.max(0,Math.min(1,+anchor.confidence||0));}));if(comment.page!==primary.page||comment.quote!==primary.quote||comment.anchorMethod!==primary.method||comment.matchConfidence!==confidence){comment.page=primary.page;comment.quote=primary.quote;comment.anchorMethod=primary.method;comment.matchConfidence=confidence;comment.anchored=true;changed=true;}
    });if(changed)ch.reviewUpdatedAt=now();return changed;
  }
  function reviewExplicitPages(comment,pageCount){
    var raw=[comment&&comment.locationHint,comment&&comment.text].filter(Boolean).join(' '),pages=[],seen={},match,re=/(?:\bp{1,2}\.?|\bpages?)\s*(\d{1,3}(?:\s*[-–—]\s*\d{1,3})?(?:(?:\s*,\s*|\s+and\s+)\d{1,3}(?:\s*[-–—]\s*\d{1,3})?){0,8})/gi;
    while((match=re.exec(raw))){var context=raw.slice(Math.max(0,match.index-90),match.index).toLowerCase(),supplementAt=Math.max(context.lastIndexOf('supplement'),context.lastIndexOf('appendix'),context.lastIndexOf('supporting information')),mainAt=Math.max(context.lastIndexOf('main manuscript'),context.lastIndexOf('main text'));if(supplementAt>mainAt)continue;var part,partRe=/(\d{1,3})(?:\s*[-–—]\s*(\d{1,3}))?/g;while((part=partRe.exec(match[1]))){var first=+part[1],last=+part[2]||first;if(last-first>12)last=first;for(var n=first;n<=last;n++)if(n>=1&&n<=pageCount&&!seen[n]){seen[n]=1;pages.push(n);}}}
    return pages;
  }
  function reviewExplicitDocumentTargets(comment){
    var raw=[comment&&comment.locationHint,comment&&comment.text].filter(Boolean).join(' '),targets=[],seen={},match,re=/\b(fig(?:ure)?|table|eq(?:uation)?|section)\.?\s*\(?([sS]?\d+(?:\.\d+)*(?:[a-z])?)\)?/gi;
    while((match=re.exec(raw))){var kind=/^fig/i.test(match[1])?'figure':/^eq/i.test(match[1])?'equation':String(match[1]).toLowerCase(),number=String(match[2]).toLowerCase(),key=kind+':'+number;if(!seen[key]){seen[key]=1;targets.push({kind:kind,number:number,key:key,label:(kind==='equation'?'Equation':kind[0].toUpperCase()+kind.slice(1))+' '+match[2]});}}
    return targets;
  }
  function reviewTargetPattern(target){
    var number=String(target&&target.number||'').replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),suffix=/[a-z]$/i.test(number)?'':'[a-z]?';if(target.kind==='section')return new RegExp('(?:\\bsection\\.?\\s*'+number+suffix+'\\b|(?:^|\\n)\\s*'+number+suffix+'(?=\\s|$))','i');var prefix=target.kind==='figure'?'(?:figure|fig)':target.kind==='equation'?'(?:equation|eq)':'table';return new RegExp('\\b'+prefix+'\\.?\\s*\\(?'+number+suffix+'\\)?\\b','i');
  }
  function reviewPageTargetKeys(text,targets){var keys=[];(targets||[]).forEach(function(target){if(reviewTargetPattern(target).test(String(text||'')))keys.push(target.key);});return keys;}
  function reviewPdfLocationConstraints(ch,comment){var targets=reviewExplicitDocumentTargets(comment);return{explicitPages:reviewExplicitPages(comment,(ch&&ch.pageTexts||[]).length),targets:targets,hardTargets:targets.filter(function(target){return target.kind!=='section';})};}
  function reviewPdfPageMeetsConstraints(ch,page,constraints){
    constraints=constraints||reviewPdfLocationConstraints(ch,null);page=+page||0;if(!page)return false;var text=String((ch.pageTexts||[])[page-1]||'');if(constraints.explicitPages.length)return constraints.explicitPages.indexOf(page)>=0;if(constraints.targets.length&&!reviewPageTargetKeys(text,constraints.targets).length)return false;return true;
  }
  function reviewPdfAnchorsMeetConstraints(ch,comment,anchors,constraints){
    constraints=constraints||reviewPdfLocationConstraints(ch,comment);anchors=anchors||[];if(!anchors.length)return false;if(anchors.some(function(anchor){return!!anchor.manual||String(anchor.method||'').indexOf('manual-')===0;}))return true;if(!anchors.every(function(anchor){return reviewPdfPageMeetsConstraints(ch,anchor.page,constraints);}))return false;if(!constraints.hardTargets.length)return true;var covered={};anchors.forEach(function(anchor){reviewPageTargetKeys(String((ch.pageTexts||[])[(+anchor.page||0)-1]||''),constraints.hardTargets).forEach(function(key){covered[key]=1;});});return constraints.hardTargets.every(function(target){return!!covered[target.key];});
  }
  function reviewPageExcerpt(text,terms,targets){
    text=String(text||'');if(text.length<=1500)return text;var best=0,bestScore=-1;
    for(var start=0;start<text.length;start+=650){var rawSample=text.slice(start,start+1500),sample=reviewNormalizedText(rawSample),score=0;terms.forEach(function(term){if(sample.indexOf(term)>=0)score+=1+Math.min(2,term.length/8);});(targets||[]).forEach(function(target){if(reviewTargetPattern(target).test(rawSample))score+=target.kind==='section'?8:40;});if(score>bestScore){bestScore=score;best=start;}}
    return text.slice(Math.max(0,best-100),Math.min(text.length,best+1500));
  }
  function reviewCandidatePdfPages(ch,comment){
    var pages=ch.pageTexts||[],terms=reviewSearchTerms(comment),constraints=reviewPdfLocationConstraints(ch,comment),explicitMap={};constraints.explicitPages.forEach(function(page){explicitMap[page]=1;});
    var ranked=pages.map(function(text,index){var normalized=reviewNormalizedText(text),score=explicitMap[index+1]?120:0;terms.forEach(function(term){if(normalized.indexOf(term)>=0)score+=1+Math.min(2,term.length/8);});constraints.targets.forEach(function(target){if(reviewTargetPattern(target).test(String(text||'')))score+=target.kind==='section'?12:45;});return{page:index+1,text:String(text||''),score:score};}).filter(function(item){return reviewPdfPageMeetsConstraints(ch,item.page,constraints);});
    ranked.sort(function(a,b){return b.score-a.score||a.page-b.page;});return ranked.slice(0,10).map(function(item){return{page:item.page,text:reviewPageExcerpt(item.text,terms,constraints.targets),score:item.score};});
  }
  ...
  async function locateReviewBatchWithAi(ch,comments,onProgress,routeOverride){
    ...
    if(ch.kind==='pdf'){
      prepared=comments.map(function(comment,index){var id=String(comment.id||'review-'+(index+1)),constraints=reviewPdfLocationConstraints(ch,comment),candidates=reviewCandidatePdfPages(ch,comment);return{id:id,comment:comment,candidates:candidates,explicit:constraints.explicitPages,constraints:constraints};}).filter(function(item){return item.candidates.length;});if(!prepared.length)return 0;
      system='You locate manuscript passages for several independent reviewer comments. Return exactly one item for every commentId. For each comment, choose only PDF pages listed inside that comment block; never borrow a candidate from another comment. Explicit PDF page ranges are mandatory. Numbered figure, table, and equation references are mandatory: if a comment names Figure 6, never substitute Figure 1 or any other numbered object. Cover every required numbered reference among the returned locations. A comment may cite multiple distinct locations, so return every necessary location up to four. Section references guide the choice but may span pages. Anchor the claim or method being criticized, not merely nearby words. Every match must include one complete exact sentence copied verbatim from its page; use two complete adjacent sentences only when both are necessary. Never begin or end mid-sentence, and never return a page with an empty quote. If you cannot identify an exact passage, or the concern is manuscript-wide or only targets supplementary material not shown, return an empty matches array. Never invent, paraphrase, or choose a merely related passage. Return only JSON: {"items":[{"commentId":"exact supplied id","matches":[{"page":1,"quote":"required complete verbatim sentence","confidence":0.0}]}]}.';
      user=prepared.map(function(item){var hard=item.constraints.hardTargets.map(function(target){return target.label;});return '[COMMENT '+item.id+']\nReviewer comment:\n'+item.comment.text+'\n\nLocation hint:\n'+(item.comment.locationHint||'none')+'\n\nRequired PDF pages:\n'+(item.constraints.explicitPages.join(', ')||'none')+'\n\nRequired numbered references:\n'+(hard.join(', ')||'none')+'\n\nAllowed candidate pages for '+item.id+':\n'+item.candidates.map(function(candidate){return '[COMMENT '+item.id+' · PDF page '+candidate.page+']\n'+candidate.text;}).join('\n\n');}).join('\n\n===== NEXT COMMENT =====\n\n');
      ...
      if(prepared.length>1)for(var p=0;p<prepared.length;p++)if(!passed[prepared[p].id])found+=await locateReviewBatchWithAi(ch,[prepared[p].comment],onProgress,routeOverride);return found;
    }
  ...
  function reviewHasVerifiedPassage(ch,comment){if(!comment||!comment.anchored)return false;if(ch&&ch.kind==='pdf'){var anchors=reviewPdfAnchors(ch,comment);return!!anchors.length&&anchors.every(function(anchor){return!!String(anchor.quote||'').trim();})&&reviewPdfAnchorsMeetConstraints(ch,comment,anchors);}return!!String(comment.quote||'').trim()&&comment.para!==null&&comment.para!==undefined&&Number.isInteger(+comment.para);}
  ...
  function reviewHasDisplayablePassage(ch,comment){if(!reviewHasVerifiedPassage(ch,comment))return false;if(reviewHasManualAnchor(ch,comment)||comment.anchorMethod==='word')return true;return comment.locationStatus!=='needs-checking'&&Number.isFinite(+comment.matchConfidence)&&+comment.matchConfidence>=.72;}
  ...
  function beginReviewerPassageLink(ch,id){
    var comment=reviewerById(ch,id);if(!comment)return;reviewLinkTargetId=id;setHighlightMode(false);clearPendingSelection();if(ch.kind==='pdf'&&readerMode!=='pdf')gotoPdfPage(reviewCommentPage(ch,comment)||currentPage,'auto');renderReviewerPanel(ch);if(temporaryNotebookMode())toggleSheet(false);showReaderToast('Select the correct passage for R'+reviewOrdinal(ch,comment));
  }
  function removeReviewerPassage(ch,id,index){
    ...
    setManualReviewPrimary(ch,comment);comment.manualReviewLink=true;comment.manualLocationRejected=true;reviewFocusId='';reviewFocusPage=0;reviewFocusAnchorIndex=-1;reviewLinkTargetId=id;ch.reviewUpdatedAt=now();touch(ch);if(readerMode==='text')renderText(ch);refreshPdfReviewMarkers();clearPdfReviewFocus();renderReviewerPanel(ch);updateReviewBadge();if(temporaryNotebookMode())toggleSheet(false);showReaderToast('AI passage rejected · this comment is manual until you link the correct text');
  }
  ...
  function locateReviewFromExactQuote(ch,comment){
    ...
    if(ch.kind==='pdf'){
      var pageTexts=ch.pageTexts||[],constraints=reviewPdfLocationConstraints(ch,comment),explicit=constraints.explicitPages,pages=(explicit.length?explicit:pageTexts.map(function(_,index){return index+1;})).filter(function(page){return reviewPdfPageMeetsConstraints(ch,page,constraints);}),matches=[];
      ...
      if(!matches.length)return false;var candidates=matches.map(function(match){return{page:match.page,text:String(pageTexts[match.page-1]||'')};});return applyPdfReviewMatches(ch,{comment:comment,candidates:candidates,explicit:explicit,constraints:constraints},matches,'Exact manuscript text');
    }
  ...
  function reviewLocationSummary(ch,comments){
    var matchable=(comments||[]).filter(function(comment){return reviewCanAutoLocate(comment)||comment.manualReviewLink;}),confident=matchable.filter(function(comment){return reviewHasDisplayablePassage(ch,comment);}).length,exact=matchable.filter(function(comment){return comment.locatedProvider==='Exact manuscript text'&&reviewHasDisplayablePassage(ch,comment);}).length;
  ...
  function trimReviewRecommendationTail(value){
    var text=String(value||''),match=/(?:^|\n)\s*In conclusion\b/i.exec(text);if(!match||match.index<120)return text.trim();var tail=text.slice(match.index);if(!/\b(?:recommend|recommended|recommendation)\b[\s\S]{0,180}\b(?:major|minor) revision\b/i.test(tail)||!/\bmanuscript\b/i.test(tail))return text.trim();return text.slice(0,match.index).trim();
  }
  function reviewReportParagraphs(text){
    var output=[],minorMode=false;
    function tagged(role,content){return(role?'['+role+'] ':'')+String(content||'').trim();}
    paras(text).forEach(function(paragraph){
      var roleMatch=paragraph.match(/^\[(REVIEWER TEXT|AUTHOR RESPONSE)\]\s*/i),role=roleMatch?roleMatch[1].toUpperCase():'',body=paragraph.replace(/^\[(?:REVIEWER TEXT|AUTHOR RESPONSE)\]\s*/i,'').trim(),lines=body.split(/\n+/).map(function(line){return line.trim();}).filter(Boolean),current='';
      function flush(){if(current){output.push(tagged(role,current));current='';}}
      lines.forEach(function(line){
        var section=/^(?:major concerns|minor comments|recommendations for strengthening the analysis|internal inconsistencies and corrections|closing comment|overall recommendation|confidential comments? to the editor)\b/i.test(line),numbered=/^\d{1,3}\.(?:\s+\S|$)/.test(line);
        if(/^In conclusion\b/i.test(line)&&/\b(?:recommend|recommended|recommendation)\b[\s\S]{0,180}\b(?:major|minor) revision\b/i.test(line))return;
        if(section){flush();output.push(tagged(role,line));minorMode=/^minor comments\b/i.test(line);return;}
        if(numbered){flush();current=line;minorMode=false;return;}
        if(minorMode){flush();output.push(tagged(role,line));return;}
        current+=(current?'\n':'')+line;
      });flush();
    });return output;
  }
  function reviewReportUnits(text){
    var paragraphs=reviewReportParagraphs(text),units=[],current=null,reviewer='Reviewer',group='',serial=0,hasRoles=/\[(?:REVIEWER TEXT|AUTHOR RESPONSE)\]/i.test(text),numbered=0;
    ...
    paragraphs.forEach(function(paragraph){
      ...
      section=content.match(/^(major concerns|minor comments|recommendations for strengthening the analysis|internal inconsistencies and corrections|closing comment|overall recommendation|confidential comments? to the editor)\b\s*[:.-]?\s*(.*)$/i);
  ...
  function reviewUnitRecord(unit,classification,provider){var text=String(unit.text||'').trim(),checked=reviewClassificationAudit(unit,classification);return{author:String(unit.reviewer||'Reviewer'),sourceGroup:String(unit.group||''),sourceNumber:String(unit.number||''),text:text,level:checked.level,topic:checked.topic,locationHint:checked.locationHint,classificationAudit:checked.audit,classifiedProvider:provider||'Local review rules'};}
  ...
  async function importReviewerFile(ch,file,button,replaceCurrent){
    ...
    try{
      ...
      ch.reviewReports=Array.isArray(ch.reviewReports)?ch.reviewReports:[];var existingComments=ch.reviewComments||[],existingReports=ch.reviewReports,replaceCount=replaceCurrent?existingComments.length:0,priorReport=existingReports.find(function(report){return String(report.name||'').toLowerCase()===String(file.name||'').toLowerCase();}),reportId=priorReport?priorReport.id:uid('report'),priorComments=replaceCurrent?existingComments:existingComments.filter(function(comment){return comment.sourceId===reportId;}),priorMatches=matchPriorReviewComments(extracted,priorComments);
      var comments=extracted.map(function(item,index){var record={id:uid('rr'),sourceId:reportId,author:item.author,sourceGroup:item.sourceGroup||'',sourceNumber:item.sourceNumber||'',date:'',text:item.text,level:normalizeReviewLevel(item.level),topic:normalizeReviewTopic(item.topic),locationHint:item.locationHint||'',classificationAudit:item.classificationAudit||null,classifiedProvider:item.classifiedProvider||plan.classification.label,para:null,page:null,start:0,end:0,quote:'',anchors:[],pdfAnchors:[],anchored:false,replies:[],response:'',responseWrittenByUser:false,resolved:false,sourceName:file.name,addedAt:now()},prior=priorMatches[index];if(prior)restorePriorReviewWork(record,prior);return record;});var matchable=comments.filter(reviewCanAutoLocate),locationStarted=now(),location=await locateReviewsBalanced(ch,matchable,function(message,progress){status.textContent=message;setTaskProgress('reviewerProgress',55+(Number.isFinite(progress)?progress:0)*45);},plan),locationMs=now()-locationStarted;
      ch.reviewComments=replaceCurrent?comments:existingComments.filter(function(comment){return comment.sourceId!==reportId;}).concat(comments);ch.reviewReports=replaceCurrent?[]:existingReports.filter(function(report){return report.id!==reportId;});var levels={},adjusted=0,stamp=now(),totalMs=stamp-started;comments.forEach(function(comment){levels[comment.level]=(levels[comment.level]||0)+1;if(comment.classificationAudit&&comment.classificationAudit.adjusted)adjusted++;});ch.reviewReports.push({id:reportId,name:file.name,addedAt:stamp,comments:comments.length,levels:levels,extractorVersion:4,classifierVersion:3,locatorVersion:6,provider:plan.classification.label,classificationProvider:plan.classification.label,locationProvider:plan.location.label,exactMatches:location.exactMatched,needsChecking:location.needsChecking,adjustedClassifications:adjusted,timing:{classificationMs:classificationMs,locationMs:locationMs,totalMs:totalMs}});ch.reviewUpdatedAt=stamp;ch.reviewClearedAt=0;placeInReviewWorkspace(ch);touch(ch);if(readerMode==='text')renderText(ch);renderReviewerPanel(ch);refreshPdfReviewMarkers();renderPdfReviewFocus(currentPage);updateReviewBadge();revealRevisionDesk();setTaskProgress('reviewerProgress',100);status.textContent=(replaceCurrent?'Replaced '+replaceCount+' old comment'+(replaceCount===1?'':'s')+' with ':'Imported ')+comments.length+' reviewer concern'+(comments.length===1?'':'s')+' · '+location.confident+' passage'+(location.confident===1?'':'s')+' linked · '+location.needsChecking+' without a passage'+(location.exactMatched?' · '+location.exactMatched+' exact-text link'+(location.exactMatched===1?'':'s'):'')+' · '+elapsedLabel(totalMs)+'.';await finishSharedReviewPass(plan.classification);
  ...
  async function locateUnlinkedReviews(ch,button){
    ...
    if(!hasReviewAiRoute()){status.textContent='Set up the built-in or a cloud AI in Desk settings first.';return;}
    var plan=await reviewAiPlan(),started=now();button.disabled=true;setTaskProgress('reviewerProgress',0);
    try{var result=await locateReviewsBalanced(ch,pending,function(message,progress){status.textContent=message;setTaskProgress('reviewerProgress',(Number.isFinite(progress)?progress:0)*100);},plan),elapsed=now()-started;ch.reviewUpdatedAt=now();touch(ch);if(result.needsChecking)reviewFilter='needs';renderText(ch);renderReviewerPanel(ch);refreshPdfReviewMarkers();setTaskProgress('reviewerProgress',100);status.textContent=result.confident+' passage'+(result.confident===1?'':'s')+' linked · '+result.needsChecking+' still without a passage'+(result.exactMatched?' · '+result.exactMatched+' exact-text link'+(result.exactMatched===1?'':'s'):'')+' · '+elapsedLabel(elapsed)+'.';await finishSharedReviewPass(plan.location);}
  ...
  function reviewerPassageMarkup(ch,comment){
    var displayable=reviewHasDisplayablePassage(ch,comment),addLabel=displayable?'Add another passage':comment.manualLocationRejected?'Select the correct passage':'Link a passage yourself',add='<button class="reviewer-add-passage" type="button" data-review-add-passage="'+esc(comment.id)+'">＋ '+addLabel+'</button>';
    if(!displayable){var empty=comment.manualLocationRejected?'AI location rejected · automatic matching is off for this comment':comment.level==='general'?'Manuscript-wide · no single passage':comment.level==='section'&&!reviewHasLocationEvidence(comment)?'Section-wide · no single passage named':'No verified passage yet';return'<span class="reviewer-unlinked'+(comment.manualLocationRejected?' manual-only':'')+'">'+empty+'</span>'+add;}
  ...
  var REVIEW_SHARE_SAFE_LINK_LENGTH=12000;
  function reviewSharePayload(ch){
    var comments=(ch&&ch.reviewComments||[]).slice(0,160).map(function(comment){return{
      author:String(comment.author||'Reviewer').slice(0,160),sourceGroup:String(comment.sourceGroup||'').slice(0,200),sourceNumber:String(comment.sourceNumber||'').slice(0,30),date:String(comment.date||'').slice(0,80),text:String(comment.text||'').slice(0,30000),level:normalizeReviewLevel(comment.level,comment),topic:normalizeReviewTopic(comment.topic),locationHint:String(comment.locationHint||'').slice(0,500),response:comment.responseWrittenByUser?String(comment.response||'').slice(0,30000):'',resolved:!!comment.resolved,replies:(comment.replies||[]).slice(0,40).map(function(reply){return{author:String(reply.author||'Reply').slice(0,160),date:String(reply.date||'').slice(0,80),text:String(reply.text||'').slice(0,12000)};}),pdfAnchors:(reviewHasDisplayablePassage(ch,comment)?reviewPdfAnchors(ch,comment):[]).slice(0,4).map(function(anchor){return{page:+anchor.page,quote:String(anchor.quote||'').slice(0,4000),confidence:Math.max(0,Math.min(1,+anchor.confidence||0))};})
    };});
    return{kind:'phloem-review',version:1,layerId:String(ch.reviewShareId||''),createdAt:now(),paper:{contentHash:String(ch.contentHash||''),title:String(ch.title||'Untitled').slice(0,300),sourceName:String(ch.sourceName||'').slice(0,240),pageCount:+ch.pageCount||0},comments:comments};
  }
  function reviewShareBase64(bytes){var binary='',chunk=0x8000;for(var at=0;at<bytes.length;at+=chunk)binary+=String.fromCharCode.apply(null,bytes.subarray(at,Math.min(bytes.length,at+chunk)));return btoa(binary).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');}
  function reviewShareBytes(value){value=String(value||'').replace(/-/g,'+').replace(/_/g,'/');while(value.length%4)value+='=';var binary=atob(value),bytes=new Uint8Array(binary.length);for(var i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);return bytes;}
  function validateSharedReview(payload){if(!payload||payload.kind!=='phloem-review'||payload.version!==1||!payload.paper||!/^[a-f0-9]{64}$/i.test(payload.paper.contentHash||'')||!Array.isArray(payload.comments)||!payload.comments.length||payload.comments.length>160)throw new Error('This is not a valid Phloem review layer.');return payload;}
  async function encodeSharedReview(payload){
    var raw=new TextEncoder().encode(JSON.stringify(payload));
    if(typeof CompressionStream!=='undefined')try{var packed=await new Response(new Blob([raw]).stream().pipeThrough(new CompressionStream('gzip'))).arrayBuffer();return'g.'+reviewShareBase64(new Uint8Array(packed));}catch(e){}
    return'j.'+reviewShareBase64(raw);
  }
  async function decodeSharedReview(value){
    value=String(value||'');if(value.length>900000)throw new Error('This review link is too large to open safely. Ask the sender for a .phloem-review file instead.');
    try{var dot=value.indexOf('.'),mode=dot>0?value.slice(0,dot):'j',bytes=reviewShareBytes(dot>0?value.slice(dot+1):value),text='';
      if(mode==='g'){if(typeof DecompressionStream==='undefined')throw new Error('This browser cannot unpack the shared review. Open it in a current Safari, Chrome, or Edge.');text=await new Response(new Blob([bytes]).stream().pipeThrough(new DecompressionStream('gzip'))).text();}else if(mode==='j')text=new TextDecoder().decode(bytes);else throw new Error('Unknown review-link format.');
      if(text.length>2000000)throw new Error('This shared review is larger than Phloem can safely import.');return validateSharedReview(JSON.parse(text));
    }catch(e){if(/browser cannot unpack|larger than Phloem|too large to open safely/i.test(e&&e.message||''))throw e;var damaged=new Error('This review link is incomplete or was changed by the app it was sent through. Ask the sender to download and send the .phloem-review file instead.');damaged.reviewLinkDamaged=true;throw damaged;}
  }
  async function decodeSharedReviewFile(file){if(!file||file.size>4000000)throw new Error('That review file is too large to open safely.');var text=await file.text();if(text.length>4000000)throw new Error('That review file is too large to open safely.');try{return validateSharedReview(JSON.parse(text));}catch(e){throw new Error('That is not a valid Phloem review file. Ask the sender to download it again.');}}
  function reviewShareTarget(payload){var hash=String(payload&&payload.paper&&payload.paper.contentHash||'');return state.chapters.find(function(ch){return ch.kind==='pdf'&&String(ch.contentHash||'')===hash;})||null;}
  function sharedReviewRecords(payload){
    var layerId=String(payload.layerId||'shared-review').slice(0,160);return(payload.comments||[]).slice(0,160).map(function(item,index){var anchors=(item.pdfAnchors||[]).slice(0,4).map(function(anchor){return{page:Math.max(1,+anchor.page||1),quote:String(anchor.quote||'').slice(0,4000),confidence:Math.max(0,Math.min(1,+anchor.confidence||0)),method:'shared-pdf-quote',manual:false};}).filter(function(anchor){return!!anchor.quote;}),primary=anchors[0]||null,confidence=anchors.length?Math.min.apply(null,anchors.map(function(anchor){return anchor.confidence;})):0,response=String(item.response||'').slice(0,30000);return{id:layerId+'-'+(index+1),sourceId:layerId,author:String(item.author||'Reviewer').slice(0,160),sourceGroup:String(item.sourceGroup||'').slice(0,200),sourceNumber:String(item.sourceNumber||'').slice(0,30),date:String(item.date||'').slice(0,80),text:String(item.text||'').slice(0,30000),level:normalizeReviewLevel(item.level,item),topic:normalizeReviewTopic(item.topic),locationHint:String(item.locationHint||'').slice(0,500),page:primary&&primary.page||null,para:null,start:0,end:0,quote:primary&&primary.quote||'',anchors:[],pdfAnchors:anchors,anchored:!!anchors.length,anchorMethod:primary?'shared-pdf-quote':'',matchConfidence:confidence,locatedProvider:primary?'Shared review':'',locationStatus:primary&&confidence>=.72?'confident':'needs-checking',manualReviewLink:false,manualLocationRejected:false,replies:(item.replies||[]).slice(0,40).map(function(reply){return{author:String(reply.author||'Reply').slice(0,160),date:String(reply.date||'').slice(0,80),text:String(reply.text||'').slice(0,12000)};}),response:response,responseWrittenByUser:!!response,resolved:!!item.resolved,sourceName:'Shared review link',addedAt:now(),updatedAt:now()};});
  }
  function copyPlainText(value){
    if(navigator.clipboard&&navigator.clipboard.writeText)return navigator.clipboard.writeText(value);
    return new Promise(function(resolve,reject){var area=document.createElement('textarea');area.value=value;area.setAttribute('readonly','');area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();try{if(!document.execCommand('copy'))throw new Error();area.remove();resolve();}catch(e){area.remove();reject(e);}});
  }
  function reviewShareStats(ch){var comments=ch&&ch.reviewComments||[],linked=0,responses=0,resolved=0;comments.forEach(function(comment){if(reviewHasDisplayablePassage(ch,comment))linked+=reviewPdfAnchors(ch,comment).length;if(String(comment.response||'').trim())responses++;if(comment.resolved)resolved++;});return{comments:comments.length,linked:linked,responses:responses,resolved:resolved};}
  async function ensureReviewShareFingerprint(ch){if(!ch||ch.kind!=='pdf')return'';if(!ch.contentHash){var stored=await getPdf(ch.id);if(stored)await rememberPdfFingerprint(ch,stored);}if(!ch.contentHash)throw new Error('The original PDF is not available here, so Phloem cannot verify its fingerprint. Re-add the PDF first.');return ch.contentHash;}
  function reviewShareFilename(ch){var stem=String(ch&&ch.title||ch&&ch.sourceName||'shared review').replace(/[\u0000-\u001f\u007f/\\<>:"|?*]+/g,' ').replace(/\s+/g,' ').trim().slice(0,100)||'shared review';return stem+' — Phloem review.phloem-review';}
  function ensureReviewShareId(ch){if(!ch.reviewShareId){ch.reviewShareId='review-layer-'+uid('').replace(/[^a-z0-9-]/gi,'');persist(false);}return ch.reviewShareId;}
  async function downloadReviewShareFile(ch){var button=byId('reviewShareDownload'),status=byId('reviewShareStatus'),old=button.textContent;button.disabled=true;button.textContent='Preparing…';status.textContent='';try{await ensureReviewShareFingerprint(ch);ensureReviewShareId(ch);var blob=new Blob([JSON.stringify(reviewSharePayload(ch))],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=reviewShareFilename(ch);document.body.appendChild(a);a.click();a.remove();setTimeout(function(){URL.revokeObjectURL(url);},5000);status.textContent='Review file downloaded. Send that file; it includes the review layer, never the PDF or an AI key.';}catch(e){status.textContent=e.message||'Phloem could not make the review file.';}finally{button.disabled=false;button.textContent=old;}}
  async function copyReviewShareLink(ch){
    var button=byId('reviewSharePrimary'),status=byId('reviewShareStatus'),old=button.textContent;button.disabled=true;button.textContent='Preparing…';status.textContent='';
    try{
      if(location.protocol==='file:')throw new Error('Open the published Phloem site before making a share link.');
      await ensureReviewShareFingerprint(ch);
      ensureReviewShareId(ch);var encoded=await encodeSharedReview(reviewSharePayload(ch)),link=location.href.split('#')[0]+'#phloem-review='+encoded;if(link.length>REVIEW_SHARE_SAFE_LINK_LENGTH)throw new Error('This '+(ch.reviewComments||[]).length+'-comment layer is too large for a reliable chat link. Use “Download review file” and send that file instead.');await copyPlainText(link);status.textContent='Snapshot link copied. After later changes, copy a new link; it will replace this layer for recipients.';button.textContent='Copied';setTimeout(function(){if(button.textContent==='Copied')button.textContent=old;},1800);
    }catch(e){status.textContent=e.message||'Phloem could not make the review link.';button.textContent=old;}button.disabled=false;
  }
  function openReviewShare(ch){
    if(!ch||ch.kind!=='pdf'||!(ch.reviewComments||[]).length)return;pendingSharedReview=null;var stats=reviewShareStats(ch),dialog=byId('reviewShareDialog');byId('reviewShareTitle').textContent='Share review layer';byId('reviewShareIntro').textContent='Send the comments and their exact highlighted passages to someone who has the same PDF.';byId('reviewSharePaper').textContent=ch.title||ch.sourceName||'Untitled paper';byId('reviewShareMatch').textContent='The PDF itself is not included. Phloem matches it by its exact file fingerprint.';byId('reviewShareIncludes').innerHTML='<span>'+stats.comments+' comment'+(stats.comments===1?'':'s')+'</span><span>'+stats.linked+' linked passage'+(stats.linked===1?'':'s')+'</span>'+(stats.responses?'<span>'+stats.responses+' revision note'+(stats.responses===1?'':'s')+'</span>':'')+(stats.resolved?'<span>'+stats.resolved+' resolved</span>':'');byId('reviewSharePrivacy').textContent='This is a snapshot, not a live link. Links are convenient for small layers; for a large review, download the review file so chat apps cannot truncate it. Neither format includes the PDF, reading notes, or AI/API settings.';byId('reviewShareStatus').textContent='';var download=byId('reviewShareDownload'),primary=byId('reviewSharePrimary');download.classList.remove('hidden');download.onclick=function(){downloadReviewShareFile(ch);};primary.textContent='Copy review link';primary.onclick=function(){copyReviewShareLink(ch);};if(!dialog.open)dialog.showModal();
  }
  function refreshIncomingReviewShare(){
    var payload=pendingSharedReview,target=reviewShareTarget(payload),count=payload&&payload.comments&&payload.comments.length||0,linked=0;(payload&&payload.comments||[]).forEach(function(comment){linked+=(comment.pdfAnchors||[]).length;});byId('reviewShareTitle').textContent='Open shared review';byId('reviewShareIntro').textContent='This carries a review layer for a specific PDF. Nothing is added until you confirm below.';byId('reviewSharePaper').textContent=payload&&payload.paper&&(payload.paper.title||payload.paper.sourceName)||'Shared paper';byId('reviewShareMatch').textContent=target?'Exact PDF found in your library: '+(target.title||target.sourceName||'Untitled paper'):'That exact PDF is not in this browser yet.';byId('reviewShareIncludes').innerHTML='<span>'+count+' comment'+(count===1?'':'s')+'</span><span>'+linked+' linked passage'+(linked===1?'':'s')+'</span>';byId('reviewSharePrivacy').textContent='The shared layer contains comments, highlighted excerpts, user-written revision notes, and resolved state—but not the unpublished PDF or anyone’s AI key.';byId('reviewShareDownload').classList.add('hidden');var primary=byId('reviewSharePrimary');primary.textContent=target?'Add shared review':'Choose matching PDF';primary.onclick=function(){var exact=reviewShareTarget(pendingSharedReview);if(exact)applySharedReview(pendingSharedReview,exact);else byId('reviewSharePdfFile').click();};
  }
  async function openIncomingSharedReview(payload){pendingSharedReview=validateSharedReview(payload);var dialog=byId('reviewShareDialog');refreshIncomingReviewShare();byId('reviewShareStatus').textContent='';if(!dialog.open)dialog.showModal();if(!reviewShareTarget(pendingSharedReview)&&state.chapters.some(function(ch){return ch.kind==='pdf'&&!ch.contentHash;})){byId('reviewShareStatus').textContent='Checking the PDFs already on this device…';await backfillPdfFingerprints();refreshIncomingReviewShare();byId('reviewShareStatus').textContent=reviewShareTarget(pendingSharedReview)?'Exact PDF verified.':'Choose the exact PDF to attach this review layer.';}}
  async function applySharedReview(payload,ch){
    var status=byId('reviewShareStatus'),button=byId('reviewSharePrimary');if(!payload||!ch||String(ch.contentHash||'')!==String(payload.paper.contentHash||'')){status.textContent='This is not the exact PDF used for the shared review.';return false;}button.disabled=true;status.textContent='Adding the shared review…';
    try{var records=sharedReviewRecords(payload),layerId=String(payload.layerId||'shared-review').slice(0,160),previous=(ch.reviewComments||[]).filter(function(comment){return comment.sourceId===layerId;}),priorMatches=matchPriorReviewComments(records,previous),stamp=now();records.forEach(function(record,index){if(priorMatches[index])restorePriorReviewWork(record,priorMatches[index]);});ch.reviewComments=(ch.reviewComments||[]).filter(function(comment){return comment.sourceId!==layerId;}).concat(records);ch.reviewReports=(ch.reviewReports||[]).filter(function(report){return report.id!==layerId;});ch.reviewReports.push({id:layerId,name:'Shared review layer',addedAt:stamp,comments:records.length,extractorVersion:4,classifierVersion:3,locatorVersion:6,shared:true});ch.reviewUpdatedAt=stamp;ch.reviewClearedAt=0;placeInReviewWorkspace(ch);touch(ch);byId('reviewShareDialog').close();pendingSharedReview=null;var opened=await openReader(ch.id);if(opened&&currentId===ch.id){revealRevisionDesk();showReaderToast(records.length+' shared reviewer comment'+(records.length===1?'':'s')+' added · your local revision notes were kept.');}return true;}catch(e){status.textContent=e.message||'Phloem could not add the shared review.';return false;}finally{button.disabled=false;}
  }
  byId('reviewSharePdfFile').onchange=async function(){
    var file=this.files&&this.files[0],payload=pendingSharedReview,status=byId('reviewShareStatus'),button=byId('reviewSharePrimary');this.value='';if(!file||!payload)return;button.disabled=true;status.textContent='Checking the PDF fingerprint…';
    try{var bytes=await file.arrayBuffer(),hash=await pdfFingerprint(bytes);if(hash!==payload.paper.contentHash)throw new Error('That is a different PDF. Choose the exact file used for this review.');var added=await importPdf(file,'','',button,true,bytes);if(!added)throw new Error('The PDF could not be added.');var target=reviewShareTarget(payload);if(!target)throw new Error('The PDF was added, but its fingerprint could not be verified.');await applySharedReview(payload,target);}catch(e){status.textContent=e.message||'Phloem could not open that PDF.';}button.disabled=false;
  };
  function reviewNeedsPassage(ch,comment){if(!reviewCanAutoLocate(comment)&&!comment.manualReviewLink)return false;return!reviewHasDisplayablePassage(ch,comment);}
  function renderReviewerPanel(ch){
    ch=ch||find(currentId);var comments=ch&&ch.reviewComments||[],list=byId('readerReviewList'),count=byId('readerReviewerCount'),locate=byId('locateReviewsBtn'),filters=byId('reviewerFilters'),qualityBox=byId('reviewQualitySummary'),importButton=byId('importReviewFileBtn'),shareButton=byId('shareReviewBtn'),clearButton=byId('clearReviewCommentsBtn'),replaceNote=byId('reviewReplaceNote'),status=byId('reviewerLocateStatus'),legacyReport=(ch&&ch.reviewReports||[]).some(function(report){return !report.extractorVersion||report.extractorVersion<4;});count.textContent=comments.length?'· '+comments.filter(function(comment){return !comment.resolved;}).length:'';importButton.textContent=comments.length?(legacyReport?'↻ Re-import structured reviewer file':'↻ Replace reviewer Word file'):'＋ Import reviewer Word file';shareButton.classList.toggle('hidden',!comments.length||!ch||ch.kind!=='pdf');clearButton.textContent='Clear all '+comments.length+' comment'+(comments.length===1?'':'s');clearButton.classList.toggle('hidden',!comments.length);replaceNote.classList.toggle('hidden',!comments.length);syncReviewLinkControls(ch);if(legacyReport&&!status.textContent)status.textContent='Re-import the same reviewer file to restore its numbered concern hierarchy. Your revision notes and manual links will be kept.';
    if(!comments.length){reviewPassageGroupIds=[];list.innerHTML='<div class="notebook-empty">This document has no reviewer comments.</div>';filters.innerHTML='';locate.classList.add('hidden');qualityBox.classList.add('hidden');return;}
    var quality=reviewLocationSummary(ch,comments),broad=comments.length-quality.matchable;qualityBox.textContent=quality.matchable?quality.confident+' passage'+(quality.confident===1?'':'s')+' linked'+(quality.needsChecking?' · '+quality.needsChecking+' without a passage':'')+(broad?' · '+broad+' broad-scope':''):(broad+' broad-scope comment'+(broad===1?'':'s')+' · no single passage expected');qualityBox.classList.remove('hidden');qualityBox.classList.toggle('needs-passage',quality.needsChecking>0);
    var commentIds={};comments.forEach(function(comment){commentIds[comment.id]=1;});reviewPassageGroupIds=reviewPassageGroupIds.filter(function(id){return!!commentIds[id];});var passageOnly=reviewPassageGroupIds.length>0,passageIds={};reviewPassageGroupIds.forEach(function(id){passageIds[id]=1;});
    var missing=comments.filter(function(comment){return reviewNeedsPassage(ch,comment);}),aiPending=missing.filter(function(comment){return reviewCanAutoLocate(comment);}),unlinked=missing.length,levelOrder=['all','needs','general','section','specific','editorial'],levelCounts={all:comments.length,needs:unlinked};comments.forEach(function(comment){comment.level=normalizeReviewLevel(comment.level,comment);comment.topic=normalizeReviewTopic(comment.topic);levelCounts[comment.level]=(levelCounts[comment.level]||0)+1;});if(reviewFilter!=='all'&&!levelCounts[reviewFilter])reviewFilter='all';
    if(passageOnly){filters.innerHTML='<span class="reviewer-filter passage-filter">This passage · '+reviewPassageGroupIds.length+'</span><button class="reviewer-filter" type="button" data-review-show-all>Show all comments</button>';filters.querySelector('[data-review-show-all]').onclick=function(){reviewPassageGroupIds=[];reviewFilter='all';renderReviewerPanel(ch);};}
    else{filters.innerHTML=levelOrder.filter(function(level){return level==='all'||levelCounts[level];}).map(function(level){return '<button class="reviewer-filter" type="button" data-review-filter="'+level+'" aria-pressed="'+String(reviewFilter===level)+'">'+(level==='all'?'All':level==='needs'?'Needs passage':reviewLevelLabel(level))+' · '+(levelCounts[level]||0)+'</button>';}).join('');filters.querySelectorAll('[data-review-filter]').forEach(function(button){button.onclick=function(){reviewPassageGroupIds=[];reviewFilter=button.dataset.reviewFilter;renderReviewerPanel(ch);};});}
    locate.classList.toggle('hidden',!aiPending.length||passageOnly);locate.textContent='✦ Locate '+aiPending.length+' passage'+(aiPending.length===1?'':'s')+' with AI';var visible=comments.map(function(comment,index){return{comment:comment,index:index};}).filter(function(item){return passageOnly?!!passageIds[item.comment.id]:(reviewFilter==='all'||(reviewFilter==='needs'?reviewNeedsPassage(ch,item.comment):item.comment.level===reviewFilter));});
    list.innerHTML=visible.map(function(item){var comment=item.comment,index=item.index,replies=(comment.replies||[]).map(function(reply){return '<div class="reviewer-reply"><b>'+esc(reply.author||'Reply')+'</b><p>'+esc(reply.text||'')+'</p></div>';}).join(''),where=reviewerPassageMarkup(ch,comment),location=comment.locationHint?'<p class="reviewer-location">⌖ '+esc(comment.locationHint)+'</p>':'',needs=reviewNeedsPassage(ch,comment),displayable=reviewHasDisplayablePassage(ch,comment),longComment=String(comment.text||'').length>460,commentCopy='<p class="reviewer-comment-text'+(longComment?' is-collapsed':'')+'" id="review-comment-copy-'+esc(comment.id)+'">'+esc(comment.text||'')+'</p>'+(longComment?'<button class="text-button reviewer-comment-toggle" type="button" data-review-comment-toggle="'+esc(comment.id)+'" aria-expanded="false">Show full comment</button>':'');
      return '<article class="reviewer-comment-card review-level-'+esc(comment.level)+(displayable?' has-passage':'')+(comment.resolved?' resolved':'')+(comment.id===reviewFocusId?' focused':'')+(comment.id===reviewLinkTargetId?' linking':'')+'" data-review-card="'+esc(comment.id)+'"><div class="reviewer-card-head"><span>R'+(index+1)+' · '+esc(comment.author||'Reviewer')+(reviewSourceLabel(comment)?' · '+esc(reviewSourceLabel(comment)):'')+'</span><span>'+esc(reviewerDate(comment.date))+'</span></div><div class="reviewer-classification"><span class="reviewer-chip level review-level-'+esc(comment.level)+'">'+esc(reviewLevelLabel(comment.level))+'</span><span class="reviewer-chip">'+esc(reviewTopicLabel(comment.topic))+'</span>'+(reviewHasManualAnchor(ch,comment)?'<span class="reviewer-chip review-linked-by-user">Linked by you</span>':'')+(comment.manualLocationRejected?'<span class="reviewer-chip review-manual-only">AI location rejected</span>':'')+(needs?'<span class="reviewer-chip review-needs-passage">Needs passage</span>':'')+'</div>'+location+(comment.quote&&displayable?'<blockquote>'+esc(comment.quote)+'</blockquote>':'')+commentCopy+replies+where+'<label class="field-label" for="review-response-'+esc(comment.id)+'">Your revision note · only what you type</label><textarea class="reviewer-response" id="review-response-'+esc(comment.id)+'" data-review-response="'+esc(comment.id)+'" placeholder="Write your own revision note here…">'+esc(comment.response||'')+'</textarea><button class="reviewer-resolve" type="button" data-resolve-review="'+esc(comment.id)+'">'+(comment.resolved?'Reopen comment':'Mark resolved')+'</button></article>';
    }).join('');
  ...
  function setRecall(on){
    ...
    if(recallActive){if(temporaryNotebookMode())toggleSheet(true);requestAnimationFrame(function(){var note=byId('pageNote');note.focus({preventScroll:true});note.setSelectionRange(note.value.length,note.value.length);});}
  }
  ...
  function gdriveArmGestureSync(){
    ...
    if(!state.chapters.length){libraryDriveRestoreArmed=true;if(!libraryDriveRestoreMessage)libraryDriveRestoreMessage='Google needs one tap before Phloem can restore it on this device.';if(!libraryHydrating)renderShelf();}
    syncUi('☁ tap to sync');
    ...
    addEventListener('click',function(){gdriveArmed=false;if(zenOn){gdriveArmGestureSync();return;}if(!gdriveOn())return;var restoring=!state.chapters.length;if(restoring){libraryDriveRestoreArmed=false;libraryHydrating=true;renderShelf();}Promise.resolve(gdriveSync(true)).finally(function(){if(restoring){libraryHydrating=false;renderShelf();updateReviewBadge();}});},{capture:true,once:true});
  }
  function loadGis(){
  ...
  async function gdriveSync(interactive,retried){
    ...
    try{
      ...
      libraryDriveRestoreArmed=false;libraryDriveRestoreMessage='';
      if(fileId){
        var got=await fetch('https://www.googleapis.com/drive/v3/files/'+fileId+'?alt=media',{headers:{Authorization:'Bearer '+token}});
        if(!got.ok){var readErr=new Error('Drive library download failed ('+got.status+')');if(got.status===401)readErr.auth=true;throw readErr;}
        var remote;try{remote=await got.json();}catch(parseError){throw new Error('The Drive library could not be read safely. Phloem did not overwrite it.');}
        if(!remote||!Array.isArray(remote.chapters))throw new Error('The Drive library has an unexpected format. Phloem did not overwrite it.');
        if(mergeState(remote)){persist(false);renderShelf();updateReviewBadge();}
      }
  ...
  function fillSettings(){
    byId('syncRepo').value=syncCfg?syncCfg.repo||'':'';byId('syncToken').value=syncCfg?syncCfg.token||'':'';byId('syncPass').value=syncCfg?syncCfg.pass||'':'';byId('syncNowBtn').classList.toggle('hidden',!syncCfg);byId('syncOffBtn').classList.toggle('hidden',!syncCfg);fillAiSettings();fillPassSettings();
  ...
  async function importSharedReview(){
    var match=location.hash.match(/^#phloem-review=(.+)$/);if(!match)return;
    try{
      var payload=await decodeSharedReview(match[1]);history.replaceState(history.state&&history.state.phloem?{phloem:history.state.phloem}:null,'',location.pathname+location.search);await openIncomingSharedReview(payload);
    }catch(e){pendingSharedReview=null;showError(e.message||'Phloem could not read this shared review.',e&&e.reviewLinkDamaged?'Shared review link is incomplete':'Could not open shared review');}
  }
  function importSharedAiPass(){
    var match=location.hash.match(/^#phloem-ai-pass=(.+)$/);if(!match)return;history.replaceState(history.state&&history.state.phloem?{phloem:history.state.phloem}:null,'',location.pathname+location.search);
    try{
      var payload=decodeSetup(match[1]),endpoint=normalizePassEndpoint(payload.endpoint),token=String(payload.token||''),claimBefore=+payload.claimBefore||0;if(payload.kind!=='phloem-ai-pass'||payload.version!==1||!endpoint||!/^[A-Za-z0-9_-]{24,}$/.test(token)||claimBefore<=now())throw new Error('This review pass is invalid or has expired.');
      if(confirm('Use this shared high-accuracy review pass?\n\nIt permits one reviewer workflow. Extracted reviewer text and candidate manuscript excerpts—not the PDF file—will be sent through the pass owner’s DeepSeek account.')){saveSharedAiPass({endpoint:endpoint,token:token,claimBefore:claimBefore,activeUntil:0,jobId:''});if(byId('settingsDialog').open)fillSettings();}
    }catch(e){showError(e.message||'Phloem could not open this review pass.','Could not open review pass');}
  }
  importSetup();
  importSharedReview();
  importSharedAiPass();
  addEventListener('hashchange',importSetup);
  addEventListener('hashchange',importSharedReview);
  addEventListener('hashchange',importSharedAiPass);
  ...
  byId('aiPassCreate').onclick=async function(){
    ...
    try{
      var response=await fetch(endpoint+'/passes',{method:'POST',headers:{'Authorization':'Bearer '+ownerCode,'Content-Type':'application/json'},body:'{}'});if(!response.ok){var detail='';try{detail=(await response.json()).error||'';}catch(e){}throw new Error(detail||'Pass service returned '+response.status);}
      var pass=await response.json();if(!pass||!/^[A-Za-z0-9_-]{24,}$/.test(String(pass.token||''))||!+pass.claimBefore)throw new Error('The pass service returned an invalid grant.');var payload={kind:'phloem-ai-pass',version:1,endpoint:endpoint,token:pass.token,claimBefore:+pass.claimBefore},publicPage=location.protocol==='file:'?(document.querySelector('meta[property="og:url"]')||{}).content:location.href.split('#')[0],link=publicPage+'#phloem-ai-pass='+encodeSetup(payload);await copyPlainText(link);try{localStorage.setItem(AI_PASS_SERVICE_KEY,endpoint);}catch(e){}byId('aiPassOwnerCode').value='';status.textContent='One-review link copied. It can be claimed once, then runs for up to ten minutes within its hard usage limit.';
    }catch(e){status.textContent=e.message||'Phloem could not create that pass.';}
    finally{button.disabled=false;button.textContent='Copy one-review pass';}
  };
