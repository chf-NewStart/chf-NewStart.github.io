(function(){
  ...
  async function reviewAiPlan(){
    aiSettings=loadAiSettings();var selected=aiSettings.provider||'auto',chosen=selected==='auto'?null:cloudAiRoute(selected),local=browserLanguageModel()?{id:'browser',label:'Gemini Nano (on device)',cfg:{}}:null;
    if(selected!=='auto'){
      var readyLocal=null;if(local&&chosen)try{var api=browserLanguageModel(),availability=api.availability?await api.availability():api.capabilities?(await api.capabilities()).available:'available';if(availability==='available'||availability==='readily'){readyLocal=local;readyLocal.fallback=chosen;}}catch(e){}
      return chosen?{mode:readyLocal?'hybrid':'cloud',classification:readyLocal||chosen,location:readyLocal||chosen,escalation:readyLocal?chosen:null}:{mode:'none',classification:null,location:null,escalation:null};
    }
    return{mode:local?'local':'none',classification:local,location:local,escalation:null};
  }
  ...
  async function runAiMessages(messages,maxTokens,onProgress,routeOverride){
    ...
    if(route.id==='browser'){try{return await runBrowserAi(messages,onProgress);}catch(e){var fallback=route.fallback;if(!fallback)throw e;aiProgress(onProgress,'On-device Gemini is unavailable; asking '+fallback.label+'…');return runCloudAi(fallback,messages,maxTokens);}}
  ...
  function normalize(ch){
    ...
    ch.reviewComments.forEach(function(comment){comment.replies=Array.isArray(comment.replies)?comment.replies:[];comment.anchors=Array.isArray(comment.anchors)?comment.anchors:(comment.anchored&&comment.para!==null&&comment.para!==undefined?[{para:comment.para,start:comment.start,end:comment.end,quote:comment.quote||''}]:[]);comment.response=String(comment.response||'');comment.locationHint=String(comment.locationHint||'');comment.level=normalizeReviewLevel(comment.level,comment);comment.topic=normalizeReviewTopic(comment.topic);comment.page=+comment.page||null;comment.pdfAnchors=Array.isArray(comment.pdfAnchors)?comment.pdfAnchors.map(function(anchor){return{page:+anchor.page||null,quote:String(anchor.quote||''),confidence:Math.max(0,Math.min(1,+anchor.confidence||0)),method:String(anchor.method||'ai-pdf-page')};}).filter(function(anchor){return anchor.page;}):[];if(!comment.pdfAnchors.length&&comment.page)comment.pdfAnchors=[{page:comment.page,quote:String(comment.quote||''),confidence:Math.max(0,Math.min(1,+comment.matchConfidence||0)),method:String(comment.anchorMethod||'ai-pdf-page')}];comment.legacyImport=!!legacyReviewReports[comment.sourceId];if(comment.legacyImport){comment.anchored=false;comment.page=null;comment.quote='';comment.pdfAnchors=[];}comment.resolved=!!comment.resolved;});
  ...
  function reviewPdfAnchors(ch,comment){
    if(!ch||ch.kind!=='pdf'||!comment)return[];var raw=Array.isArray(comment.pdfAnchors)&&comment.pdfAnchors.length?comment.pdfAnchors:(comment.page?[{page:comment.page,quote:comment.quote||'',confidence:comment.matchConfidence||0,method:comment.anchorMethod||'ai-pdf-page'}]:[]),seen={};return raw.map(function(anchor){return{page:+anchor.page||null,quote:String(anchor.quote||''),confidence:Math.max(0,Math.min(1,+anchor.confidence||0)),method:String(anchor.method||'ai-pdf-page')};}).filter(function(anchor){var key=anchor.page+'|'+reviewNormalizedText(anchor.quote);if(!anchor.page||seen[key])return false;seen[key]=1;return true;});
  }
  ...
  function reviewLocationIsDifficult(ch,comment){
    if(!reviewCanAutoLocate(comment))return false;var audit=comment.classificationAudit||{},confidence=Number(audit.confidence),pages=ch&&ch.kind==='pdf'?reviewExplicitPages(comment,(ch.pageTexts||[]).length):[],text=String([comment.text,comment.locationHint].filter(Boolean).join(' '));
    return!reviewHasVerifiedPassage(ch,comment)||!Number.isFinite(+comment.matchConfidence)||+comment.matchConfidence<.72||!Number.isFinite(confidence)||confidence<.72||!!audit.adjusted||normalizeReviewLevel(comment.level,comment)==='section'||pages.length>1||/\b(?:throughout|elsewhere|both|multiple|several|compare|consistent with|conflicts? with)\b/i.test(text);
  }
  function reviewShouldEscalateBeforeLocation(ch,comment){
    var audit=comment&&comment.classificationAudit||{},confidence=Number(audit.confidence),pages=ch&&ch.kind==='pdf'?reviewExplicitPages(comment,(ch.pageTexts||[]).length):[],text=String([comment&&comment.text,comment&&comment.locationHint].filter(Boolean).join(' '));
    return!Number.isFinite(confidence)||confidence<.72||!!audit.adjusted||normalizeReviewLevel(comment&&comment.level,comment)==='section'||pages.length>1||/\b(?:throughout|elsewhere|both|multiple|several|compare|consistent with|conflicts? with)\b/i.test(text);
  }
  ...
  function reviewLocationSummary(ch,comments){
    var matchable=(comments||[]).filter(reviewCanAutoLocate),confident=matchable.filter(function(comment){return reviewHasVerifiedPassage(ch,comment)&&comment.locationStatus!=='needs-checking'&&+comment.matchConfidence>=.72;}).length,cloudChecked=matchable.filter(function(comment){return!!comment.locationEscalated&&reviewHasVerifiedPassage(ch,comment)&&comment.locationStatus!=='needs-checking';}).length;
    return{matchable:matchable.length,confident:confident,needsChecking:Math.max(0,matchable.length-confident),cloudChecked:cloudChecked};
  }
  async function locateReviewsHybrid(ch,comments,onProgress,plan){
    ...
    var direct=plan.escalation?comments.filter(function(comment){return reviewShouldEscalateBeforeLocation(ch,comment);}):[],primary=comments.filter(function(comment){return direct.indexOf(comment)<0;});
    if(primary.length)await locateReviewsWithAi(ch,primary,function(message,progress){if(onProgress)onProgress(message,(Number.isFinite(progress)?progress:0)*(plan.escalation ? .55 : 1));},plan.location);
    var difficult=plan.escalation?direct.concat(primary.filter(function(comment){return reviewLocationIsDifficult(ch,comment);})):[];
    if(difficult.length){var cloudStart=primary.length ? .55 : 0;difficult.forEach(function(comment){resetReviewLocation(comment);comment.locationEscalated=true;});await locateReviewsWithAi(ch,difficult,function(message,progress){if(onProgress)onProgress('High-accuracy recheck · '+message,cloudStart+(Number.isFinite(progress)?progress:0)*(1-cloudStart));},plan.escalation);}
    finalizeReviewLocationConfidence(ch,comments);var summary=reviewLocationSummary(ch,comments);summary.escalated=difficult.length;summary.primaryLabel=plan.location.label;summary.cloudLabel=plan.escalation&&difficult.length?plan.escalation.label:'';return summary;
  }
  ...
  function reviewCanAutoLocate(comment){if(comment&&comment.legacyImport)return false;var level=normalizeReviewLevel(comment&&comment.level,comment);return level==='specific'||level==='editorial'||(level==='section'&&reviewHasLocationEvidence(comment));}
  ...
  function reviewClassificationNeedsCloud(record){var audit=record&&record.classificationAudit||{},confidence=Number(audit.confidence);return!!audit.adjusted||!Number.isFinite(confidence)||confidence<.72;}
  async function refineReviewClassificationsWithAi(records,onProgress,routeOverride){
    var candidates=(records||[]).map(function(record,index){return{record:record,unit:{inputId:'recheck-'+index,reviewer:record.author||'Reviewer',group:'',number:'',text:record.text||'',response:record.response||''}};}).filter(function(item){return reviewClassificationNeedsCloud(item.record);}),batches=reviewClassificationBatches(candidates,6),next=0,done=0,changed=0;
    if(!batches.length||!routeOverride)return 0;if(onProgress)onProgress('Rechecking '+candidates.length+' uncertain label'+(candidates.length===1?'':'s')+' with '+routeOverride.label+'…',0);
    async function worker(){for(;;){var index=next++;if(index>=batches.length)return;var batch=batches[index],units=batch.map(function(item){return item.unit;}),result=await classifyReviewBatchWithAi(units,reviewClassificationPrompt(),function(message){if(onProgress)onProgress('Label recheck '+(index+1)+' of '+batches.length+' · '+message,done/candidates.length);},routeOverride);batch.forEach(function(item){var classification=result.byId[item.unit.inputId];if(!classification)return;var checked=reviewUnitRecord(item.unit,classification,result.provider||routeOverride.label);item.record.level=checked.level;item.record.topic=checked.topic;item.record.locationHint=checked.locationHint;item.record.classificationAudit=checked.classificationAudit;item.record.classifiedProvider=checked.classifiedProvider;item.record.classificationEscalated=true;changed++;});done+=batch.length;if(onProgress)onProgress('Rechecked '+Math.min(done,candidates.length)+' of '+candidates.length+' uncertain labels…',done/candidates.length);}}
    var workers=[],count=Math.min(4,batches.length);for(var i=0;i<count;i++)workers.push(worker());await Promise.all(workers);return changed;
  }
  async function importReviewerFile(ch,file,button,replaceCurrent){
    var status=byId('reviewerLocateStatus'),old=button.textContent,plan=await reviewAiPlan();if(!plan.classification||!plan.location){status.textContent='Set up the built-in or a cloud AI in Desk settings first.';return;}button.disabled=true;button.textContent='Reading review…';
    try{
      ...
      var report=await reviewReportText(file);if(!report.trim())throw new Error('That reviewer file contains no readable text.');status.textContent='Separating the reviewer comments with '+plan.classification.label+'…';setTaskProgress('reviewerProgress',10);var extracted=await extractReviewerReportWithAi(report,function(message,progress){status.textContent=message;setTaskProgress('reviewerProgress',10+(Number.isFinite(progress)?progress:0)*30);},plan.classification);if(!extracted.length)throw new Error('AI could not find actionable reviewer comments in that file.');if(plan.escalation)await refineReviewClassificationsWithAi(extracted,function(message,progress){status.textContent=message;setTaskProgress('reviewerProgress',40+(Number.isFinite(progress)?progress:0)*15);},plan.escalation);setTaskProgress('reviewerProgress',55);
      ...
      var comments=extracted.map(function(item){var record={id:uid('rr'),sourceId:reportId,author:item.author,date:'',text:item.text,level:normalizeReviewLevel(item.level),topic:normalizeReviewTopic(item.topic),locationHint:item.locationHint||'',classificationAudit:item.classificationAudit||null,classifiedProvider:item.classifiedProvider||plan.classification.label,classificationEscalated:!!item.classificationEscalated,para:null,page:null,start:0,end:0,quote:'',anchors:[],pdfAnchors:[],anchored:false,replies:[],response:item.response||'',resolved:false,sourceName:file.name,addedAt:now()},prior=priorByText[reviewNormalizedText(item.text)];if(prior){record.id=prior.id;record.response=prior.response||record.response;record.resolved=!!prior.resolved;record.replies=prior.replies||[];}return record;});var matchable=comments.filter(reviewCanAutoLocate),location=await locateReviewsHybrid(ch,matchable,function(message,progress){status.textContent=message;setTaskProgress('reviewerProgress',55+(Number.isFinite(progress)?progress:0)*45);},plan);
      ch.reviewComments=replaceCurrent?comments:existingComments.filter(function(comment){return comment.sourceId!==reportId;}).concat(comments);ch.reviewReports=replaceCurrent?[]:existingReports.filter(function(report){return report.id!==reportId;});var levels={},adjusted=0,stamp=now(),providerLabel=plan.escalation?plan.classification.label+' + '+plan.escalation.label:plan.classification.label;comments.forEach(function(comment){levels[comment.level]=(levels[comment.level]||0)+1;if(comment.classificationAudit&&comment.classificationAudit.adjusted)adjusted++;});ch.reviewReports.push({id:reportId,name:file.name,addedAt:stamp,comments:comments.length,levels:levels,extractorVersion:2,classifierVersion:3,locatorVersion:3,provider:providerLabel,classificationProvider:plan.classification.label,locationProvider:plan.location.label,escalationProvider:location.cloudLabel,escalatedLocations:location.escalated,needsChecking:location.needsChecking,adjustedClassifications:adjusted});ch.reviewUpdatedAt=stamp;ch.reviewClearedAt=0;placeInReviewWorkspace(ch);touch(ch);if(readerMode==='text')renderText(ch);renderReviewerPanel(ch);refreshPdfReviewMarkers();renderPdfReviewFocus(currentPage);updateReviewBadge();setTaskProgress('reviewerProgress',100);status.textContent=(replaceCurrent?'Replaced '+replaceCount+' old comment'+(replaceCount===1?'':'s')+' with ':'Imported ')+comments.length+' reviewer concern'+(comments.length===1?'':'s')+' · '+location.confident+' confidently located · '+location.needsChecking+' need checking'+(location.escalated?' · '+location.escalated+' rechecked with '+location.cloudLabel:'')+' · filed in '+REVIEW_WORKSPACE_CATEGORY+'.';
  ...
  async function locateUnlinkedReviews(ch,button){
    var pending=(ch.reviewComments||[]).filter(function(comment){return comment.text&&reviewNeedsPassage(ch,comment);}),status=byId('reviewerLocateStatus');if(!pending.length)return;
    if(!hasAiRoute()){status.textContent='Set up the built-in or a cloud AI in Desk settings first.';return;}
    var plan=await reviewAiPlan();button.disabled=true;setTaskProgress('reviewerProgress',0);
    try{var result=await locateReviewsHybrid(ch,pending,function(message,progress){status.textContent=message;setTaskProgress('reviewerProgress',(Number.isFinite(progress)?progress:0)*100);},plan);ch.reviewUpdatedAt=now();touch(ch);renderText(ch);renderReviewerPanel(ch);refreshPdfReviewMarkers();setTaskProgress('reviewerProgress',100);status.textContent=result.confident+' confidently located · '+result.needsChecking+' still need checking'+(result.escalated?' · '+result.escalated+' rechecked with '+result.cloudLabel:'')+'.';}
  ...
  function reviewerPassageMarkup(ch,comment){
    if(!comment.anchored)return comment.level==='general'?'<span class="reviewer-unlinked">Manuscript-wide · no single passage</span>':comment.level==='section'&&!reviewHasLocationEvidence(comment)?'<span class="reviewer-unlinked">Section-wide · no single passage named</span>':'<span class="reviewer-unlinked">Needs checking · no confident passage yet</span>';
  ...
  function renderReviewerPanel(ch){
    ...
    list.innerHTML=visible.map(function(item){var comment=item.comment,index=item.index,replies=(comment.replies||[]).map(function(reply){return '<div class="reviewer-reply"><b>'+esc(reply.author||'Reply')+'</b><p>'+esc(reply.text||'')+'</p></div>';}).join(''),where=reviewerPassageMarkup(ch,comment),location=comment.locationHint?'<p class="reviewer-location">⌖ '+esc(comment.locationHint)+'</p>':'',needs=reviewNeedsPassage(ch,comment);
      return '<article class="reviewer-comment-card review-level-'+esc(comment.level)+(comment.anchored?' has-passage':'')+(comment.resolved?' resolved':'')+(comment.id===reviewFocusId?' focused':'')+'" data-review-card="'+esc(comment.id)+'"><div class="reviewer-card-head"><span>R'+(index+1)+' · '+esc(comment.author||'Reviewer')+'</span><span>'+esc(reviewerDate(comment.date))+'</span></div><div class="reviewer-classification"><span class="reviewer-chip level review-level-'+esc(comment.level)+'">'+esc(reviewLevelLabel(comment.level))+'</span><span class="reviewer-chip">'+esc(reviewTopicLabel(comment.topic))+'</span>'+(needs?'<span class="reviewer-chip review-needs-checking">Needs checking</span>':'')+'</div>'+location+(comment.quote&&comment.anchored?'<blockquote>'+esc(comment.quote)+'</blockquote>':'')+'<p class="reviewer-comment-text">'+esc(comment.text||'')+'</p>'+replies+where+'<label class="field-label" for="review-response-'+esc(comment.id)+'">Your response · written by you</label><textarea class="reviewer-response" id="review-response-'+esc(comment.id)+'" data-review-response="'+esc(comment.id)+'" placeholder="Write your response here…">'+esc(comment.response||'')+'</textarea><button class="reviewer-resolve" type="button" data-resolve-review="'+esc(comment.id)+'">'+(comment.resolved?'Reopen comment':'Mark resolved')+'</button></article>';
    }).join('');
  ...
  function aiProviderNote(id){
    ...
    return 'For reviewer files, Phloem starts locally when possible and sends only uncertain labels and difficult candidate excerpts to '+AI_PROVIDERS[id].label+'. Other AI questions use '+AI_PROVIDERS[id].label+' directly. The key is excluded from library sync and backups.';
  }
