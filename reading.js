(function(){
  ...
  function renderReviewWorkspaceCard(ch,isSelected){
    ...
    card.innerHTML='<div class="review-workspace-card-head"><div><span class="review-workspace-eyebrow">Manuscript revision</span><h3>'+esc(title)+'</h3><p>'+esc(ch.authors||'')+'</p></div><strong class="review-workspace-remaining">'+esc(remaining)+'</strong></div><div class="review-workspace-pair"><span>Paper</span><b>'+esc(paperName)+'</b><span>Reviewer '+(files.length===1?'file':'files')+'</span><b>'+(files.length?files.map(esc).join('<br>'):'No reviewer file attached yet')+'</b></div><div class="review-workspace-scopes" aria-label="Review progress by scope">'+levels.map(function(level){var item=stats.levels[level],done=item.total-item.open,progress=item.total?Math.round(done/item.total*100):0;return'<div class="review-workspace-scope review-level-'+level+'"><div><span>'+esc(reviewLevelLabel(level))+'</span><b>'+item.open+' open · '+done+' done</b></div><span class="review-workspace-progress" aria-hidden="true"><i style="--review-progress:'+progress+'%"></i></span></div>';}).join('')+'</div><div class="review-workspace-footer"><span>'+esc(shelfDate(ch))+'</span><button class="button review-workspace-continue" type="button">Continue review&nbsp; →</button></div>';
    card.querySelector('.review-workspace-continue').onclick=function(){openReader(ch.id).then(function(){switchTab('reviewsPanel');if(innerWidth>720)setNotebookCollapsed(false,true);});};return card;
  }
  ...
  async function importReviewerFile(ch,file,button,replaceCurrent){
    ...
    try{
      ...
      var comments=extracted.map(function(item){var record={id:uid('rr'),sourceId:reportId,author:item.author,date:'',text:item.text,level:normalizeReviewLevel(item.level),topic:normalizeReviewTopic(item.topic),locationHint:item.locationHint||'',classificationAudit:item.classificationAudit||null,classifiedProvider:item.classifiedProvider||reviewRoute.label,para:null,page:null,start:0,end:0,quote:'',anchors:[],pdfAnchors:[],anchored:false,replies:[],response:item.response||'',resolved:false,sourceName:file.name,addedAt:now()},prior=priorByText[reviewNormalizedText(item.text)];if(prior){record.id=prior.id;record.response=prior.response||record.response;record.resolved=!!prior.resolved;record.replies=prior.replies||[];}return record;});var matchable=comments.filter(reviewCanAutoLocate),linked=await locateReviewsWithAi(ch,matchable,function(message,progress){status.textContent=message;setTaskProgress('reviewerProgress',55+(Number.isFinite(progress)?progress:0)*45);},reviewRoute);
      ch.reviewComments=replaceCurrent?comments:existingComments.filter(function(comment){return comment.sourceId!==reportId;}).concat(comments);ch.reviewReports=replaceCurrent?[]:existingReports.filter(function(report){return report.id!==reportId;});var levels={},adjusted=0,stamp=now();comments.forEach(function(comment){levels[comment.level]=(levels[comment.level]||0)+1;if(comment.classificationAudit&&comment.classificationAudit.adjusted)adjusted++;});ch.reviewReports.push({id:reportId,name:file.name,addedAt:stamp,comments:comments.length,levels:levels,extractorVersion:2,classifierVersion:2,locatorVersion:2,provider:reviewRoute.label,adjustedClassifications:adjusted});ch.reviewUpdatedAt=stamp;ch.reviewClearedAt=0;placeInReviewWorkspace(ch);touch(ch);if(readerMode==='text')renderText(ch);renderReviewerPanel(ch);refreshPdfReviewMarkers();renderPdfReviewFocus(currentPage);updateReviewBadge();setTaskProgress('reviewerProgress',100);status.textContent=(replaceCurrent?'Replaced '+replaceCount+' old comment'+(replaceCount===1?'':'s')+' with ':'Imported ')+comments.length+' reviewer concern'+(comments.length===1?'':'s')+' · '+linked+' linked to the '+(ch.kind==='pdf'?'PDF':'manuscript')+' · '+(adjusted?adjusted+' label'+(adjusted===1?'':'s')+' corrected by local checks':'AI labels agreed with local checks')+' · '+reviewRoute.label+' · filed in '+REVIEW_WORKSPACE_CATEGORY+'.';
  ...
  async function locateUnlinkedReviews(ch,button){
    ...
    try{found=await locateReviewsWithAi(ch,pending,function(message,progress){status.textContent=message;setTaskProgress('reviewerProgress',(Number.isFinite(progress)?progress:0)*100);},reviewRoute);ch.reviewUpdatedAt=now();touch(ch);renderText(ch);renderReviewerPanel(ch);refreshPdfReviewMarkers();setTaskProgress('reviewerProgress',100);status.textContent=found?'Linked '+found+' comment'+(found===1?'':'s')+' to the manuscript with '+reviewRoute.label+'.':'No confident passage matches were found.';}
