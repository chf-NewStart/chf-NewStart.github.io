(function(){
  ...
  function normalize(ch){
    ...
    if(!Array.isArray(ch.reviewReports))ch.reviewReports=[];ch.reviewUpdatedAt=+ch.reviewUpdatedAt||0;ch.reviewClearedAt=+ch.reviewClearedAt||0;var legacyReviewReports={};ch.reviewReports.forEach(function(report){if(!report.extractorVersion||report.extractorVersion<3)legacyReviewReports[report.id]=1;});
  ...
  function restorePriorReviewWork(comment,prior){
    if(!comment||!prior)return comment;if(prior.id)comment.id=prior.id;if(String(prior.response||'').trim())comment.response=String(prior.response);comment.resolved=!!prior.resolved;if(Array.isArray(prior.replies)&&prior.replies.length)comment.replies=prior.replies;comment.updatedAt=prior.updatedAt||comment.updatedAt||0;preserveManualReviewLocation(comment,prior);return comment;
  }
