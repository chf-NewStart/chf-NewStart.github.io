(function(){
  ...
  async function copyReviewShareLink(ch){
    ...
    try{
      ...
      if(!ch.reviewShareId){ch.reviewShareId='review-layer-'+uid('').replace(/[^a-z0-9-]/gi,'');persist(false);}var encoded=await encodeSharedReview(reviewSharePayload(ch)),link=location.href.split('#')[0]+'#phloem-review='+encoded;await copyPlainText(link);status.textContent='Review link copied. It contains the review layer, not the PDF.';button.textContent='Copied';setTimeout(function(){if(button.textContent==='Copied')button.textContent=old;},1800);
